...
export const PRO_FEATURE_CATALOG: ReadonlyArray<{ id: string; name: string }> = [
  ...
  // 유료공개 — 맵을 값을 매겨 판다 (27a · 27b, 2026-09-21).
  //
  // ★ **코어에 판매 경로를 뚫지 않는다.** 27b §4.2 는 "pro 가 없으면
  //   코어가 402 를 답한다" 였는데, 그러려면 코어에 `/sales/*` 같은
  //   유료 경로가 생긴다 — 이 파일 맨 위의 규칙(기능별 경로는 유료
  //   모듈이 자기가 가져온다)을 깨는 일이고, 유료 기능이 늘 때마다
  //   공개 코어를 고치게 된다.
  //
  //   402 로 하려던 일("404 가 *원래 없다* 인지 *못 판다* 인지 구분되게")은
  //   **이 목록이 이미 한다** — 화면은 `/v1/features` 에서 `map-sales` 가
  //   꺼져 있는 것과 **그 이유**를 읽어, 값 매기기·구매 단추를 아예 그리지
  //   않거나 이유를 적는다. 자르기(코어)와 파는 일(pro)의 경계는 그대로다.
  { id: 'map-sales', name: '유료공개(맵 판매)' },
];
