...
export function OutlinePanel({ t, outline }: PanelProps) {
  return (
    <div style={{ padding: '10px 6px 12px' }}>
      ...
      {outline.map(node => <OutlineRow key={node.id} t={t} node={node} />)}
    </div>
  );
}
    ...
    <div>
      <div style={{
        ...
        <button
          ...
          {expanded ? <I.ChevronDown size={12} stroke={2} /> : <I.ChevronRight size={12} stroke={2} />}
        </button>
        <span style={{
          width: 7, height: 7, borderRadius: '50%',
          background: node.depth === 0 ? t.primary : (node.selected ? t.primary : t.borderStrong),
          flexShrink: 0,
        ...
        {node.selected && (
          <span style={{ marginLeft: 'auto', fontSize: 9, color: t.primary, fontWeight: 700 }}>●</span>
        )}
      </div>
      {expanded && hasChildren && node.children!.map(c => (
        <OutlineRow key={c.id} t={t} node={c} />
      ))}
    </div>
  );
}
