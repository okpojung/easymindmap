...
//   · 지금 보이는 영역이 사각형으로 — 크기 = 캔버스 창 px ÷ 배율
...
export function Minimap({ t, nodes, W, H, CX, CY, onClose }: Props) {
  ...
  const geom = useMemo(() => {
    ...
    return minimapGeometry(worldBounds(nodes), maxW, maxH);
  }, [nodes, W, H]);
  const view = worldToMini(geom, viewportWorldRect(W, H, CX, CY, panX, panY, zoom));
