...
/**
 * EMM 표준 소개 — 출처는 `docs/04-extensions/emm-spec.md` §1 이다.
 * 비교표의 값도 그 문서의 표를 그대로 옮겼다(§1.1 시장 공백).
 */
export default function Emm() {
  return (
    <>
      <section className="wrap head">
        <p className="eyebrow">EMM — EasyMindMap Markdown</p>
        <h1>마인드맵을 담는 <b>표준 텍스트 형식</b></h1>
        <p className="lead">
          마인드맵을 표현하는 텍스트 포맷의 사실상 표준은 아직 없습니다.
          EMM 은 <b>본문은 순수 GFM</b>, 충실도 정보는 <b>본문 밖 메타데이터</b>에
          두는 2계층 구조로 그 빈자리를 채웁니다.
        </p>
      </section>
        ...
        <div className="two">
          <article>
            ...
            <p>
              ...
              레벨</b>이라는 약속 하나뿐입니다. 그래서 EMM 파일은 GitHub·
              Obsidian·Notion·VS Code 에서 <b>그냥 정상적인 문서</b>로 보입니다.
            </p>
          </article>
          <article>
            <h3>메타데이터 — 도구가 읽는 층</h3>
            <p>
              레이아웃·도형·색·사진 위치처럼 <b>그림에만 필요한 것</b>은 본문 밖에
              둡니다. 덕분에 맵 → MD → 맵을 반복해도 <b>무손실</b>이고, 메타데이터가
              없어도 문서는 그대로 유효합니다.
            </p>
          </article>
        </div>
      ...
      <section className="wrap">
        ...
        <div className="tablewrap">
          <table className="cmp">
            <thead>
              <tr><th>항목</th><th>Mermaid</th><th>Markmap</th><th>FreeMind</th><th>EMM</th></tr>
            </thead>
            <tbody>
              <tr><td>기반</td><td>MD 코드블록</td><td>MD 헤딩</td><td>XML</td><td><b>GFM 본문 + 메타데이터</b></td></tr>
              ...
              <tr><td>사진(위치 보존)</td><td>✗</td><td>✗</td><td>제한</td><td className="ok">✅</td></tr>
              <tr><td>일반 MD 뷰어에서 가독</td><td>코드블록으로 보임</td><td>✅</td><td>✗</td><td className="ok">✅ 본문이 순수 GFM</td></tr>
              <tr><td>무손실 왕복</td><td>✗</td><td>✗</td><td>자체 포맷 내</td><td className="ok">✅</td></tr>
              <tr><td>AI 생성 친화</td><td>중간</td><td>높음</td><td>낮음</td><td className="ok">높음</td></tr>
            </tbody>
          </table>
        </div>
        <p className="foot-note">
          출처: EMM Specification §1.1 — 이 표는 스펙 문서의 비교표를 그대로 옮긴 것입니다.
        </p>
      </section>
...
`}</pre>
            <p className="small">
              ...
              일반 Markdown 만 내놓아도 유효한 EMM 문서입니다.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="wrap">
        <h2 className="sec">EMM 이 지키려는 것 여섯</h2>
        <ol className="goals">
          ...
          <li><b>무손실 왕복</b> — 맵 → MD → 맵을 반복해도 구조·노트·스타일·사진 위치가 남는다.</li>
          <li><b>AI 입출력 친화</b> — 메타데이터는 선택 사항이다.</li>
      ...
      <section className="wrap cta">
        <h2>직접 열어 보세요</h2>
        <p>맵을 만들어 Markdown 으로 내보내면, 그 파일이 곧 EMM 입니다.</p>
