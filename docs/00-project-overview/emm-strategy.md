...
> 마인드맵 Markdown 포맷(mmd)의 표준을 정의하고
...
- 이 공백에서 승부는 "더 좋은 앱"이 아니라 **"파일 포맷을 가진 쪽"**이
  ...
  포맷이 없어서다.
- mmd의 차별 구조 — "본문 100% GFM + 파일 끝 메타데이터 1줄" — 는
...
- [x] mmd v1.0 Draft 스펙 공개 (`emm-spec.md`)
...
- mmd RFC 공개 + 변경 제안 프로세스(GitHub Discussions/PR) 운영
...
| 대상 | 이유 |
|---|---|
| mmd 스펙 + 적합성 테스트 스위트 | 표준의 신뢰 기반 — 이것 없이는 표준이 아님 |
...
| 단독 벤더 표준이라는 경계심 | 스펙 리포 분리, 공개 변경 프로세스, 파서 Apache-2.0, "mmd-Basic = 그냥 GFM" 강조 |
