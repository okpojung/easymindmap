...
>   없음, 노트 블록 4종(table 포함·warning/tip 폐기), 자동 저장=주기 스냅샷(기본 5분)+로컬 초안,
...
### 4. 자동 저장 · 히스토리 · 편집 잠금
- 서버 맵에 연결된 문서는 **주기(기본 5분)·미저장 편집 50개·탭 전환/창 닫기** 시점에 문서 스냅샷 자동 저장. 그 사이 편집은 **로컬 초안(IndexedDB)** 에 즉시 보관 (2026-08-06 개편 — 14-save.md §0)
  (`PUT /maps/:id/document`) — 무변경 저장은 스킵(버전 미생성)
- 저장 배지 6종 — `저장됨 · N분 전` / `미저장 편집 N개` / `저장 중…` /
  `재시도 중…` / `저장 실패` / `저장 안 됨` (자동 재시도는 무기한 + 온라인
  복귀 즉시 재개)
- **명시 저장(☁ 저장·맵 닫기·다른 이름)에서만** 버전 히스토리 생성
  (히스토리 패널에서 조회·새 탭 열기, 첨부 개수·용량 표시)
...
| Autosave 타이밍 | 주기(3·5·10분, 기본 5분) + 미저장 편집 50개 + 탭 전환·창 닫기·온라인 복귀, 전체 스냅샷 (무변경 스킵) |
