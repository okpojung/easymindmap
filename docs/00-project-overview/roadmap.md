...
| 버전 히스토리 | 맵 변경 이력 조회 |
| Diff Viewer | 버전 간 변경 내용 시각화 (VH-07) |
| **노드 배경 이미지** | preset / 업로드, fit/position/overlay 설정 (IMG-01~20) |
...
| CHAT-04 | 전송 대상 지정 | 전체 협업자 또는 특정 1명에게 메시지 전송 (DM) |
...
| 노드 노트 | 높음 | 낮음 | MVP |
| 노드 링크 (URL 첨부) | 높음 | 낮음 | MVP |
| 노드 첨부파일 (일반/멀티미디어) | 높음 | 중간 | MVP |
| 노드 배경 이미지 | 중간 | 중간 | V1 |
| 자동 저장 | 매우 높음 | 중간 | MVP |
| AI 맵 생성 | 높음 | 중간 | MVP |
