...
| 152 | AI          | AI-01       | Generate Mindmap       | 주제 입력으로 자동 생성                    | 1. 주제 입력2. AI 요청 및 수신3. 맵 적용                                                                     |
| 153 | AI          | AI-02       | Expand Node            | 하위 아이디어 자동 확장                    | 1. 노드 선택2. 확장 요청3. 자식 생성                                                                         |
| 154 | AI          | AI-03       | Thread Summarize       | 댓글 스레드 AI 요약                     | 1. 스레드 선택2. 요약 분석 및 텍스트 생성                                                                       |
| 155 | AI          | AI-04       | Task Extraction        | 태스크 자동 추출                        | 1. 분석 요청2. 텍스트 분석3. 태스크 목록 반환                                                                    |
| 156 | AI          | AI-05       | Task Node Gen          | 추출된 태스크를 노드로 생성                  | 1. 결과 확인2. 적용 클릭 및 삽입                                                                            |
| 157 | AI 워크플로우    | WFLOW-01    | Workflow Generate      | AI Workflow 노드 구조 자동 생성          | 1. 요청 입력2. step 노드 설계 및 생성                                                                       |
| 158 | AI 워크플로우    | WFLOW-02    | Step Node 구조           | 단계별 step node 정의                 | 1. 순서 정의2. 실행 조건·내용 저장                                                                           |
| 159 | AI 워크플로우    | WFLOW-03    | Step Status            | 실행 상태 추적                         | 1. 트리거2. stepState 변경 및 UI 갱신                                                                    |
| 160 | AI 워크플로우    | WFLOW-04    | Step Progress          | 실행 진행 상태 표시                      | 1. 배지 표시2. 진행률 시각화                                                                               |
| 161 | AI 워크플로우    | WFLOW-05    | Error Input            | 오류 시 사용자 입력 요청                   | 1. blocked 감지2. 입력 필드 활성화 및 재시도                                                                  |
| 162 | AI 워크플로우    | WFLOW-06    | AI Resolution          | 오류 자동 해결 시도                      | 1. blocked 감지2. AI 분석 및 재시도                                                                      |
| 163 | AI 워크플로우    | WFLOW-07    | Cleanup                | 완료 후 임시 데이터 정리                   | 1. 상태 확인2. 임시 데이터 삭제                                                                             |
| 164 | AI 워크플로우    | WFLOW-08    | Structured Note        | 결과를 구조화된 노트로 저장                  | 1. 결과 수신2. 파싱 및 node_notes 저장                                                                    |
| 165 | AI 워크플로우    | WFLOW-09    | Code Block             | 결과 코드 블록 렌더링 지원                  | 1. 코드 감지2. 언어 식별 및 UI 렌더링                                                                        |
| 166 | AI 워크플로우    | WFLOW-10    | Copy Button            | 코드 블록 복사 버튼 제공                   | 1. 버튼 표시2. 클립보드 복사                                                                               |
| 167 | AI 워크플로우    | WFLOW-11    | Solo-only AI           | 개인 맵에서만 사용 허용                    | 1. map_type 확인2. personal 여부 검증                                                                  |
| 168 | AI 워크플로우    | WFLOW-12    | Collab Restriction     | 협업 맵에서 사용 제한                     | 1. 협업 맵 감지2. UI 비활성화 안내                                                                          |
| 169 | 내보내기        | EXPORT-01   | Export Markdown        | 맵을 Markdown 내보내기                 | 1. 모드 선택2. 변환 및 다운로드                                                                             |
| 170 | 내보내기        | EXPORT-02   | Export HTML            | 맵을 standalone HTML 내보내기          | 1. HTML 생성2. 다운로드                                                                                |
| 171 | 가져오기        | IMPORT-01   | Import Markdown        | Markdown 파일을 맵으로 가져오기            | 1. 파일 선택2. 파싱 및 트리 생성                                                                            |
| 172 | 가져오기        | IMPORT-02   | Outline Mode           | 아웃라인 모드 import                   | 1. 모드 선택2. 리스트를 자식 노드로 변환                                                                        |
| 173 | 가져오기        | IMPORT-03   | Document Mode          | 문서 모드 import (기본)                | 1. 모드 적용2. 본문/리스트를 콘텐츠로 유지                                                                       |
| 174 | 가져오기        | IMPORT-04   | Import Preview         | import 전 파싱 결과 미리보기              | 1. 파일 선택2. 미리보기 표시 및 확인                                                                          |
| 175 | 대시보드        | DASH-01     | Dashboard Mode         | 대시보드 모드 활성화                      | 1. 모드 전환2. 대시보드 형태 표시                                                                            |
| 176 | 대시보드        | DASH-02     | Auto Refresh           | 외부 데이터 주기적 갱신                    | 1. 활성화2. interval마다 API 호출                                                                       |
| 177 | 대시보드        | DASH-03     | Change Highlight       | 갱신 시 변경 항목 강조                    | 1. 갱신 완료2. 비교 및 하이라이트 표시                                                                         |
| 178 | 대시보드        | DASH-04     | Refresh Interval       | 자동 갱신 간격 설정                      | 1. 값 입력 및 저장2. 반영                                                                                |
| 179 | 대시보드        | DASH-05     | External Update API    | 외부 업데이트 API                      | 1. API 호출2. 인증 및 데이터 갱신                                                                          |
| 180 | 번역          | TRANS-01    | Auto Translate         | 노드 텍스트 자동 번역                     | 1. 언어 설정 감지2. API 요청 및 저장                                                                        |
| 181 | 번역          | TRANS-02    | Translation Cache      | 번역 결과 캐시 저장                      | 1. 완료 후 키 기반 캐시 저장                                                                               |
| 182 | 번역          | TRANS-03    | Cache Invalidation     | 원문 변경 시 캐시 무효화                   | 1. 변경 감지2. 해시 무효화 및 재번역 트리거                                                                      |
| 183 | 번역          | TRANS-04    | Skeleton UI            | 번역 대기 중 Skeleton 표시              | 1. 요청 중 Skeleton 표시2. 완료 시 fadeIn 교체                                                             |
| 184 | 번역          | TRANS-05    | Original Toggle        | 번역본에서 원문 토글 조회                   | 1. 아이콘 클릭2. 원문 팝오버 표시                                                                            |
| 185 | 번역          | TRANS-06    | Batch Translate        | 여러 노드 일괄 번역                      | 1. 다중 선택2. 순차 처리 및 갱신                                                                            |
| 186 | 번역          | TRANS-07    | Translation Broadcast  | WebSocket 실시간 전파                 | 1. 완료 이벤트 발행2. 클라이언트 전파 및 갱신                                                                     |
| 187 | 번역          | TRANS-08    | Live Chat Translate    | 채팅 메시지 실시간 번역                    | 1. 메시지 수신2. 감지 및 번역 표시                                                                           |
| 188 | 번역          | TRANS-09    | Language-group Cache   | 언어 그룹별 캐시 공유                     | 1. 그룹 분류2. 캐시 공유 및 히트율 향상                                                                        |
| 189 | 번역          | TRANS-10    | Short Message Guard    | 짧은 메시지 번역 skip                   | 1. 길이 측정2. 임계값 미만 시 skip                                                                         |
| 190 | 번역          | TRANS-11    | Original + Translation | 원문과 번역문 동시 표시                    | 1. 동시 렌더링2. 비교 가능                                                                                |
| 191 | 협업          | COLLAB-01   | 맵 초대                   | 다른 사용자를 협업자 초대                   | 1. 이메일 입력2. 역할 선택3. 발송                                                                           |
| 192 | 협업          | COLLAB-02   | 권한 변경                  | 협업자 역할 변경                        | 1. 목록 확인2. 업데이트 및 적용                                                                             |
| 193 | 협업          | COLLAB-03   | 협업자 제거                 | 협업자 목록에서 삭제                      | 1. 선택2. 레코드 삭제 및 차단                                                                              |
| 194 | 협업          | COLLAB-04   | 실시간 동기화                | 동시 편집 실시간 반영                     | 1. WebSocket 이벤트 브로드캐스트                                                                          |
| 195 | 협업          | COLLAB-05   | LWW 충돌 정책              | 동시 편집 충돌 처리 (나중 승리)              | 1. 시각 기준 나중 값 적용 및 알림                                                                            |
| 196 | 협업          | COLLAB-06   | 변경 수신 및 반영             | 타인 변경 로컬 상태 적용                   | 1. 이벤트 수신2. store 및 렌더링 갱신                                                                       |
| 197 | 협업          | COLLAB-07   | 커서 공유                  | 다른 사용자 커서 위치 표시                  | 1. presence 이벤트 발행2. 아이콘 표시                                                                      |
| 198 | 협업          | COLLAB-08   | Soft Lock              | 편집 중인 노드 잠금 표시                   | 1. 시작 이벤트 발행2. 타 사용자 화면에 잠금 표시                                                                   |
| 199 | 협업          | COLLAB-09   | Lock 해제                | 편집 완료 후 잠금 자동 해제                 | 1. 완료 이벤트 발행2. 표시 해제                                                                             |
| 200 | 협업          | COLLAB-10   | Node Thread            | 노드에 댓글 스레드 추가                    | 1. 패널 열기2. 댓글 저장 및 알림                                                                            |
| 201 | 협업          | COLLAB-11   | Thread Reply           | 스레드에 답글 추가                       | 1. 답글 입력2. 저장 및 알림                                                                               |
| 202 | 협업          | COLLAB-12   | Thread Resolve         | 스레드 해결 완료 처리                     | 1. Resolve 클릭2. 상태 변경 및 표시                                                                       |
| 203 | 협업          | COLLAB-13   | Thread Mention         | @멘션 사용자 태그                       | 1. @입력 및 선택2. 알림 발송                                                                              |
| 204 | 협업          | COLLAB-14   | AI Thread Summary      | 스레드 내용 AI 요약                     | 1. 요약 버튼 클릭2. 분석 및 상단 표시                                                                         |
| 205 | 협업          | COLLAB-15   | AI Task Extraction     | 스레드에서 태스크 추출                     | 1. 분석 요청2. 결과 노드 생성                                                                              |
| 206 | 협업          | COLLAB-16   | 접속자 목록                 | 현재 접속자 실시간 표시                    | 1. presence 연결 및 수신2. 아바타 표시                                                                     |
| 207 | 협업          | COLLAB-17   | 접속자 수 표시               | 현재 동시 접속자 수 표시                   | 1. 집계 및 툴바 표시                                                                                    |
| 208 | 채팅          | CHAT-01     | 맵 채팅 패널                | 실시간 채팅 패널 열기                     | 1. 버튼 클릭2. 내역 로딩                                                                                 |
| 209 | 채팅          | CHAT-02     | 메시지 전송                 | 채팅 입력 및 전송                       | 1. 입력 및 전송2. 패널 표시                                                                               |
| 210 | 채팅          | CHAT-03     | 이전 메시지 로딩              | 스크롤 up 시 페이징 로딩                  | 1. 스크롤 감지2. API 호출 및 추가                                                                          |
| 211 | 채팅          | CHAT-04     | 전송 대상 지정               | 특정 사용자 지정 메시지                    | 1. 수신자 선택2. 해당 사용자에게만 표시                                                                         |
| 212 | 채팅          | CHAT-05     | @멘션                    | 채팅에서 특정 사용자 멘션                   | 1. @사용자 입력2. 알림 전송                                                                               |
| 213 | 채팅          | CHAT-06     | 미확인 @멘션 표시             | 읽지 않은 멘션 알림 배지                   | 1. 배지 표시2. 확인 시 해제                                                                               |
| 214 | 채팅          | CHAT-07     | 1:1 DM                 | 특정 사용자와 다이렉트 메시지                 | 1. 사용자 선택2. 전용 패널 메시지 전송                                                                         |
| 215 | 공유/게시       | PUBL-01     | 맵 게시                   | 외부 읽기 전용 공개                      | 1. 범위 선택2. URL 생성                                                                                |
| 216 | 공유/게시       | PUBL-02     | 게시 취소                  | 비공개 전환                           | 1. unpublish 선택2. URL 비활성화                                                                       |
| 217 | 공유/게시       | PUBL-03     | 공개 뷰 렌더링               | 읽기 전용 viewer 표시                  | 1. URL 접근2. viewer 렌더링                                                                           |
| 218 | 공유/게시       | PUBL-04     | 공유 링크 복사               | URL 클립보드 복사                      | 1. 버튼 클릭2. 클립보드 저장                                                                               |
| 219 | WBS         | WBS-01      | WBS 모드 전환              | WBS 보기 모드 활성화                    | 1. 토글 활성화2. 인디케이터 표시                                                                             |
| 220 | WBS         | WBS-02      | 일정 설정                  | 시작일·종료일 설정                       | 1. DatePicker 선택2. node_schedules 저장                                                             |
| 221 | WBS         | WBS-03      | 마일스톤 설정                | 노드 마일스톤 지정                       | 1. 토글 활성화2. ◆ 마커 표시                                                                              |
| 222 | WBS         | WBS-04      | 진척률 설정                 | 완료율 0~100% 입력                    | 1. 슬라이더 조정2. progress 저장                                                                         |
| 223 | WBS         | WBS-05      | WBS 인디케이터              | 일정·진척률 등 렌더링                     | 1. 데이터 확인2. 상태별 색상 표시                                                                            |
| 224 | 리소스         | RES-01      | 리소스 할당 패널              | 담당자 할당 패널 열기                     | 1. 패널 오픈2. 목록 표시                                                                                 |
| 225 | 리소스         | RES-02      | 담당자 검색/추가              | 검색 후 노드 추가                       | 1. 사용자 검색2. 선택 및 아바타 표시                                                                          |
| 226 | 리소스         | RES-03      | 역할 지정                  | 담당자 역할 지정                        | 1. 역할 선택 및 저장                                                                                    |
| 227 | 리소스         | RES-04      | 공수 입력                  | 예상 공수 입력                         | 1. 숫자 입력 및 저장                                                                                    |
| 228 | 리소스         | RES-05      | 아바타 인디케이터              | 노드에 담당자 아바타 표시                   | 1. 조건부 아바타 렌더링                                                                                   |
| 229 | Obsidian 연동 | OBS-01      | Obsidian 가져오기          | vault 파일을 맵으로 import             | 1. 파일 선택2. 파싱 및 적용                                                                               |
| 230 | Obsidian 연동 | OBS-02      | 맵 내보내기                 | 맵을 Obsidian vault 내보내기           | 1. .md 변환 및 저장                                                                                   |
| 231 | Obsidian 연동 | OBS-03      | Vault 연결 설정            | vault 경로 연결 설정                   | 1. 경로 입력 및 테스트                                                                                   |
| 232 | Obsidian 연동 | OBS-04      | 변경 감지 동기화              | 파일 변경 시 맵 자동 업데이트                | 1. 변경 감지 및 파싱                                                                                    |
| 233 | Obsidian 연동 | OBS-05      | Wikilink 처리            | Obsidian [[파일명]] 처리              | 1. 링크 감지 및 연결                                                                                    |
| 234 | Redmine 연동  | RDMN-01     | Redmine 연동 설정          | URL·API Key 설정                   | 1. 설정 저장 및 테스트                                                                                   |
| 235 | Redmine 연동  | RDMN-02     | Pull 동기화               | Issue 데이터를 맵으로 가져오기              | 1. API 호출 및 생성                                                                                   |
| 236 | Redmine 연동  | RDMN-03     | Push 동기화               | 맵 노드를 Issue로 내보내기                | 1. 변환 및 Redmine 생성                                                                               |
| 237 | Redmine 연동  | RDMN-04     | 노드 생성→Issue 생성         | 생성 시 자동 Issue 생성                 | 1. 이벤트 감지 및 API 호출                                                                               |
| 238 | Redmine 연동  | RDMN-05     | 노드 수정→Issue 업데이트       | 수정 시 자동 Issue 업데이트               | 1. 변경 감지 및 PATCH 호출                                                                              |
| 239 | Redmine 연동  | RDMN-06     | 노드 삭제→Issue 삭제         | 삭제 시 자동 Issue 삭제                 | 1. 삭제 이벤트 감지 및 API 호출                                                                            |
| 240 | Redmine 연동  | RDMN-07     | 동기화 상태 인디케이터           | sync_status 아이콘 표시               | 1. 상태별 아이콘 표시                                                                                    |
| 241 | Redmine 연동  | RDMN-08     | Redmine Plugin 탭       | 관리 전용 탭                          | 1. 탭 선택 및 관리                                                                                     |
| 242 | 설정          | SETT-01     | 프로필 설정                 | 프로필 정보 수정                        | 1. 정보 수정 및 저장                                                                                    |
| 243 | 설정          | SETT-02     | 테마 설정                  | 라이트·다크 테마 선택                     | 1. 테마 저장 및 적용                                                                                    |
| 244 | 설정          | SETT-03     | 언어/번역 설정               | UI 언어 및 기본 설정                    | 1. 언어 변경 및 저장                                                                                    |
| 245 | 설정          | SETT-04     | 기본 레이아웃 설정             | 신규 맵 기본 레이아웃                     | 1. 타입 선택 및 저장                                                                                    |
| 246 | 설정          | SETT-05     | UI 표시 설정               | 인디케이터 등 표시 설정                    | 1. 토글 저장 및 적용                                                                                    |
| 247 | 설정          | SETT-06     | 맵별 설정 오버라이드            | 개별 맵 설정 덮어쓰기                     | 1. 개별 저장 및 적용                                                                                    |
| 248 | 설정          | SETT-07     | API Key 관리             | 연동용 API Key 관리                   | 1. 암호화 저장 및 사용                                                                                   |
