...
| 19b | NODE 편집     | NE-013b     | 달력 노드 추가 (2026-09-22) | 우상단 `+` 메뉴 — 년도 노드 아래 1월~12월, 년월 노드 아래 주별(`[NN주] 시작 ~ 끝` → 날짜 노드 7개, 일요일·공휴일 빨강 · 토요일 파랑). "표로 붙여넣기" 체크 → 노드 내용에 그 달 달력 표 | 1. 노드 선택 → `+` → 달력 노드 추가2. 글·조상에서 년도·월 미리 채운 창 확인3. 확인 → addChildOutlineBulk(undo 1단계). 선택 없이 `+` = 중심 노드 추가 |
