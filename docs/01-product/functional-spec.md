...
| 19c | NODE 편집     | NE-013c     | 연결선 (2026-09-22) | 트리와 무관하게 노드↔노드를 잇는 선 — 각진/둥근, 두께·색(기본 파랑)·실선/파선/점선, 화살표 없음/끝/시작/양쪽, 가운데·위·아래·곁가지 라벨(도형 없음/둥근/사각/캡슐/원). 곁가지는 노드가 아니라 라벨 | 1. 노드 선택 → 툴바 [연결] → 끝 노드 클릭2. 선 클릭 = 선택 → 스타일 탭 연결선 패널3. Delete/휴지통 삭제 · 끝 노드 삭제 시 함께 정리4. mmd `connectors:`(노드 경로)·HTML 뷰어에 담김 — 10-canvas §31 |
