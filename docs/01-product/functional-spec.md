...
| 19a | NODE 편집     | NE-013      | 스타일 복사(붓) (2026-09-19) | 한 노드의 도형·색·글자맞춤을 내용 없이 다른 노드에 | 1. 노드 선택 → 붓 버튼2. 커서 옆 붓 표시3. 노드 클릭 또는 러버밴드 = 칠하기(undo 한 단계)4. ESC/빈 곳 클릭으로 해제 — 05-node-style §20 |
| 19b | NODE 편집     | NE-013b     | 달력 노드 추가 (2026-09-22) | 우상단 `+` 메뉴 — 년도 노드 아래 1월~12월, 년월 노드 아래 주별(`[NN주] 시작 ~ 끝` → 날짜 노드 7개, 일·토·공휴일 빨강 + 공휴일 `[이름]`, 앞뒤 달은 회색 점선). "표로 붙여넣기" 체크 → 노드 내용에 그 달 달력 표 | 1. 노드 선택 → `+` → 달력 노드 추가2. 글·조상에서 년도·월 미리 채운 창 확인3. 확인 → addChildOutlineBulk(undo 1단계). 선택 없이 `+` = 중심 노드 추가 |
| 19c | NODE 편집     | NE-013c     | 연결선 (2026-09-22) | 트리와 무관하게 노드↔노드를 잇는 선 — 각진/둥근, 두께·색(기본 파랑)·실선/파선/점선, 화살표 없음/끝/시작/양쪽, 가운데·위·아래·곁가지 라벨(도형 없음/둥근/사각/캡슐/원). 곁가지는 노드가 아니라 라벨. 시작 면·끝 면(자동/위/아래/왼쪽/오른쪽, 2026-09-23) — 같은 면은 고리, 마주 보면 가운데 꺾음, 직각은 모서리 | 1. 노드 선택 → 툴바 [연결] → 끝 노드 클릭2. 선 클릭 = 선택 → 스타일 탭 연결선 패널3. Delete/휴지통 삭제 · 끝 노드 삭제 시 함께 정리4. mmd `connectors:`(노드 경로)·HTML 뷰어에 담김 — 10-canvas §31 |
