...
> 상세 설계: `docs/03-editor-core/canvas/09-kanban.md`
...
| EXPORT-03 | Export PNG/SVG | 맵 캔버스를 이미지 파일로 내보내기 | **후순위** |
| EXPORT-04 | Export PDF | 맵을 PDF 파일로 내보내기 | **후순위** |
...
| **포함 메타** | 노드 텍스트·태그·메모·링크 (옵션) | title, map_id, owner, layout_type, **edge_default**, theme, node_count, tags, created_at, updated_at |
...
> **`edge_default` 허용값**: `curve` (방사형 레이아웃) | `orthogonal` (그 외 모든 레이아웃)  
> 레이아웃에서 자동 파생되며 사용자가 직접 지정하지 않는다.  
> 참조: `docs/03-editor-core/edge-policy.md §3`, `docs/04-extensions/import-export/20-export.md §4.2-B`
...
| CHAT-06 | 미확인 @멘션 표시 | 나중에 참여 시 본인 @멘션 이력 강조 표시 | V2 |
| CHAT-07 | 1:1 DM | 협업자와 1:1 비공개 대화 채널 | V3 |
...
| 단계 | 포함 기능 그룹 | 비고 |
|---|---|---|
| MVP | MAP, NODE, NODE STYLE (NS-01~06), NODE BG IMAGE (IMG MVP), NODE RENDERING (NR-01~08), EDGE POLICY (EDGE-01~06), LAYOUT, KANBAN, CANVAS, SELECTION, HISTORY, VERSION HISTORY (VH-01), SAVE, TAG, SEARCH, **AI (AI-01~02)**, EXPORT, IMPORT, PUBLISH, SETTINGS (SETT MVP) | 핵심 편집 기능 전체 — AI는 Generate·Expand(AI-01~02)만 포함 |
...
| V3 | DASHBOARD (DASH-01~05), AI 협업 요약 (COLLAB-14~15, **AI-03~05**), CHAT-07 (파일 첨부), SETTINGS (SETT-07) | 대시보드 맵 + AI Thread 요약·작업 추출·작업 노드 생성(AI-03~05) + Dashboard API Key |
...
| TAG | 4 | MVP | — |
| SEARCH | 2 | MVP | — |
| AI (AI-01~02) | 2 | MVP | Generate Mindmap, Expand Node |
| AI (AI-03~05) | 3 | V3 | ⭐ Thread 요약·작업 추출·작업 노드 생성 (V3, AI 협업 요약) |
...
| COLLAB Phase 2 + CHAT | 16 | V2~V3 | ⭐ 신규 (COLLAB-07~13 / CHAT-01~07) + CHAT-06(미확인 @멘션)·CHAT-07(1:1 DM V3) 추가 |
