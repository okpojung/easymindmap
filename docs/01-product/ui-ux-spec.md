...
- **상단 툴바 (실물 구성)** — 화면마다 다른 막대다 (2026-09-07). **편집**(`TopToolbar`): 내 문서(문서함) · Undo/Redo(+undo 카운터) · AI · 공유 · 퍼블리싱 · 아웃라인↔맵 모드 토글 · 다크모드 토글 · ☁ 저장 · ⧉ 다른 이름으로 저장 · ✕ 맵 닫기 · 내보내기 드롭다운(HTML/MD — 첨부 시 ZIP) · 계정. **문서함**(`DocsToolbar`): 로고 · 내 문서 · 다크모드 · 계정만 — 맵마다 할 일은 문서함 행의 관리 칸(정보 · 공유 · 퍼블리싱(단독맵만) · 이름 · 이동 · 삭제)에서 한다. 공통 부품(다크 토글·계정 메뉴·토스트)은 같은 것을 쓴다.
- **좌측 레일** — 검색 · 템플릿 · 히스토리 · 맵 설정 패널 + 속성 탭. **문서함이 열려 있으면 레일을 그리지 않는다**(2026-09-07). '새 맵'은 레일에서 빠졌다 — 문서함 도구줄 **[＋ 새 맵 ▾]**(`NewMapPanel` 을 팝오버로 재사용: 기본 맵 · 템플릿 · MD/HTML/ZIP)에서만 만든다. 문서함이 없는 Guest 체험에서만 레일에 '새 맵'이 남는다(`UnifiedSidebar.railNewMap`). (아웃라인은 좌측 탭이 아니라 **편집 영역 분할 모드**로 제공)
...
> ⚠️ **구현 현황(2026-08-02)**: 앱에 라우터가 없어 `/login` 경로 대신
> **로그인 게이트**로 구현했다 — 인증이 켜진 배포에서 미로그인이면
> 에디터 대신 **소개(슬로건)+로그인 화면**만 렌더한다(`WelcomeScreen`).
> 가입은 같은 화면의 "가입" 버튼(SCR-02 별도 화면 없음), 성공 시
> 대시보드가 아니라 **에디터**로 들어간다(대시보드는 미구현 —
> 문서 목록이 그 역할). SNS 로그인은 제공자 목록에 자리만 있고
> "준비 중"이다. 상세: [auth-session-ui.md](../04-extensions/auth-session-ui.md)
...
| Layout 선택 | 레이아웃 드롭다운 — 선택 시 연결선(Edge) 타입 자동 결정 (방사형→곡선, 그 외→직각선) |
| AI 생성 버튼 | AI 패널 토글 |
| 협업 (Share) | 공유 다이얼로그 (V1~) |
| 아웃라인/맵 모드 토글 | 편집 영역 전체를 아웃라인 전용 ↔ 맵 전용으로 통째 전환 (목록↔마인드맵 SVG 아이콘, 전환될 대상 표시). 아웃라인 분할 보기가 켜져 있으면 비활성 |
| 다크/라이트 토글 | 테마 전환 (🌙/☀) |
