...
## 0. 레벨(Level) 용어 정의 (2026-07 확정)

사용자에게 보이는 **레벨은 중심 주제가 1레벨**이다 (ThinkWise와 동일한
사용자 감각). 내부 코드는 0-기준 `depth`를 쓰므로 **표시 레벨 = depth + 1**.

| 사용자 표시 | 내부 depth | 노드 | 예 |
|---|---|---|---|
| **1레벨** | depth 0 | 중심 주제 (root) | "2026 제품 로드맵" |
| **2레벨** | depth 1 | 주제 (root 직계 브랜치, `map.branches`) | "Q1 · 기반 구축" |
| **3레벨** | depth 2 | 하위 주제 | "인증 시스템" |
| **4레벨+** | depth 3+ | 내용 | — |

규칙:
- **UI 문구·문서·사용자 대화는 반드시 1-기준 "N레벨"**을 쓴다
  (사이드바 "선택 · N레벨", 맵 설정 "1레벨 (중심)/2레벨/…" 등).
- **코드 내부(변수·타입·레이아웃 엔진)는 0-기준 `depth`** 그대로 둔다.
  UI 경계에서만 `depth + 1`로 변환한다.
- 문서에서 내부 값을 언급할 때는 `depth N`으로 표기해 표시 레벨과
  혼동하지 않는다. (예: "2레벨(depth 1) 브랜치")
