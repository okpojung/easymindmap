...
| 그룹 | 라벨 | 버튼 | 활성 조건 | 단축키 |
|---|---|---|---|---|
| 노드 | **노드** | `+` (`add-node`, 2026-09-22 사용자 요청) — 선택 없음: **중심 노드 추가**(새 중심주제와 같다) · 선택 있음: 메뉴(`add-menu`) `자식 노드 추가` / `달력 노드 추가…`(`CalendarNodeDialog` — 노드 글·조상에서 년도·월을 읽어 미리 채운 창, 년도 → 1월~12월 · 년월 → `NN주` → `YY/MM/DD(일) ~ YY/MM/DD(토)`, `utils/calendarNodes.ts` · `addChildOutlineBulk` 한 번 = undo 1단계). 사용자 가이드 03 | 항상 | — (다중 추가는 캔버스 `Ctrl+Space` 팝업) |
