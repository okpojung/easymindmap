...
| 그룹 | 라벨 | 버튼 | 활성 조건 | 단축키 |
|---|---|---|---|---|
| 노드 | **노드** | `+` (`add-node`, 2026-09-22 사용자 요청) — 선택 없음: **중심 노드 추가**(새 중심주제와 같다) · 선택 있음: 메뉴(`add-menu`) `자식 노드 추가` / `달력 노드 추가…`(`CalendarNodeDialog` — 노드 글·조상에서 년도·월을 읽어 미리 채운 창, 년도 → 1월~12월 · 년월 → `[NN주] YYYY/MM/DD(일) ~ MM/DD(토)` → 날짜 노드 7개(`YYYY/MM/DD(요일)`, 일요일·공휴일 `textColor` 빨강 `#DC2626` · 토요일 파랑 `#2563EB` — `utils/koreanHolidays.ts` 2024~2030 표 + 고정 공휴일), `utils/calendarNodes.ts` · `addChildOutlineBulk` 한 번 = undo 1단계(`OutlineItem.style` 을 노드에 복사). **표로 붙여넣기** 체크(`calendar-as-table`, 월을 골라야 켜짐) → 노드를 만들지 않고 `updateNodeText` 로 선택 노드 내용 끝에 `calendarTable()` GFM 표(일~토 7열 · 일요일·공휴일 `**N**` + 이름)를 붙인다, 2026-09-22 사용자 수정). 사용자 가이드 03 | 항상 | — (다중 추가는 캔버스 `Ctrl+Space` 팝업) |
