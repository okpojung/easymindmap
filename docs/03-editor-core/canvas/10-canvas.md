...
| 창 모드 | 그 배율로 맵이 다 안 들어가면(`fits = false`) 미니맵은 **화면 주변만** 보여 주는 창이 된다. 축마다: 맵이 창에 들어가면 맵 가운데, 아니면 표시창 중심을 가운데로 두되 맵 경계 밖으로는 안 나간다. **창은 표시창이 가장자리(6%) 안에 있는 동안 그대로**고, 끄는 동안(포인터 캡처)은 무조건 고정 — 끌 때마다 창이 따라오면 사각형이 제자리에 서고 맵이 흐르는 조이스틱이 된다 |
...
검증: 단위 `minimapMath.test.ts` 29항목 · 브라우저 e2e291 26항목(900 노드 맵의 창 모드 · 열 때 100% 포함).
