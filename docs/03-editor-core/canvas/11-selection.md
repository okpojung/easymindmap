...
* 포함 (구현 기준: **단일 선택 + 러버밴드 영역 다중 선택**):
  ...
  * 선택 해제 (ESC, 빈 영역 클릭)
  * 선택 시 노드 추가 인디케이터 표시
  * 다중 선택 상태에서 일괄 동작 (삭제, 스타일, 크기, **레이아웃** — 2026-09-21
    사용자 보고로 추가: `updateNodesLayoutType(ids, layout)` 한 번 = undo 1단계,
    'root'·중심주제 루트는 건너뛴다). 다중 선택 중에는 +/− 인디케이터가 없으므로
    **접힘 숫자 배지도 대표 노드에서 숨기지 않는다** (같은 보고)
