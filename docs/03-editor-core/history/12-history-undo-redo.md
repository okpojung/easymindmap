...
* 제외:
  ...
  * 영구 저장 (→ SAVE)
  * **되돌리기 스택의 IndexedDB 영구 보관 — 하지 않는다** (2026-08-15
    사용자 결정). 이유 셋. ⑴ 위의 **2026-07-31 결정("세션 한정")과
    어긋난다.** ⑵ "지난 상태로 돌아가고 싶다"는 **저장 버전 이력**이
    이미 덮는다(`13-version-history.md`) — 둘 다 두면 사용자가 어느 쪽을
    써야 하는지 알 수 없다. ⑶ **비용이 크다**: 지금 99단계가 싼 이유는
    스냅샷이 메모리에서 구조를 공유하기 때문인데, **직렬화하면 그 공유가
    깨져** 전체 문서 99벌이 된다. 문서에는 이미지가 data URL 로 들어 있어
    브라우저 저장소를 금방 밀어낸다.
...
#### 6.6-1 **되돌리기는 문서 경계를 넘지 않는다** (2026-08-06 실사용 보고)

> **"되돌리기 하면 새 맵이 아닌 예전 맵도 나오고 정체불명의 맵들이 표시됨"** (사용자)

##### 재현 (실측)

```
로그인 → 서버 맵 열기 → 편집 4회 → 맵 닫기 → 새 맵 → 편집 3회
  → Ctrl+Z ×3   (여기까지는 새 맵)
  → -3 을 0 으로 확정
  → Ctrl+Z 더   →  **'통계C-큼' 의 편집 상태가 튀어나온다**
```

실측값: 맵 닫기 직후 `past = 5`, 새 맵 직후 `past = 6` — **이전 문서의
스냅샷이 그대로 쌓여 있었다.**

##### 원인 — 두 겹이었다

1. `closeMap()` · `newMap()` 이 `past`/`future` 를 **비우지 않았다.**
   (`loadMap` 만 `resetHistory` 옵션으로 비우고 있었다)
2. **비우기만 해서는 부족하다.** zustand 구독은 `set()` 안에서 **동기**
   실행되므로, 전환 자체(`이전 문서 → 새 문서`)가 되돌리기에 한 건
   쌓인다. 비운 뒤에도 구독이 이전 문서를 도로 밀어 넣는다.

##### 고친 방법 — `asDocumentSwap`
...
// 되돌리기 구독:  if (swappingDocument) return;   ← ①
```

이 헬퍼를 쓰는 전환: `closeMap` · `newMap` · `setSample` ·
`loadMap(resetHistory)`. **개별 액션마다 `set({past:[],future:[]})` 를
흩뿌리는 대신 한 곳에 모았다** — 새 전환 경로가 생겨도 이 헬퍼만 쓰면
규칙이 자동으로 지켜진다 (`docOrigin` 을 구조로 막은 것과 같은 방식,
14-save.md §7.5).

##### 함께 바뀐 것

**맵 닫기는 이제 되돌리기로 되살아나지 않는다.** 예전 주석은 "실수로
닫아도 Ctrl+Z 로 복구된다"였는데, 그 편의가 바로 위 증상의 통로였다.
맵 닫기는 **저장한 뒤** 닫으므로 문서함에서 다시 열면 된다.

검증: e2e113 [1]~[5]. **수정 전 상태로 되돌려 [1][2][3] 이 실제로
실패하는 것을 확인한 뒤** 고쳤다.

##### 후속 — **새 문서를 만드는 `loadMap` 은 전부 `resetHistory`** (2026-08-06)

위를 배포한 뒤 같은 증상이 한 번 더 보고됐다.

> *"새 맵(진행트리-트리맵) → 편집 → -3 되돌리기 → 0 확정 → 되돌리기 몇 번
> 하니 **트리-진행트리맵**(고르지도 않은 기본 골격)이 표시됨"*

`asDocumentSwap` 은 `closeMap`·`newMap`·`setSample`·`loadMap(resetHistory)`
만 덮는다. 그런데 **템플릿으로 새 맵**을 만드는 경로는

```
replaceWithBlankDoc()   → newMap()  = 기본 골격 (스택 비움)
loadMap(templateSkeletonMap(tpl.map))   ← resetHistory 없음!
```

이라 **"기본 골격 → 템플릿 골격" 전환이 되돌리기에 한 건 남았다.**
되돌리기를 끝까지 하면 고르지도 않은 기본 골격이 나온다.

같은 모양의 경로를 전수로 찾아 셋 다 `resetHistory: true` 를 붙였다.

| 경로 | 무엇 |
| --- | --- |
| `NewMapPanel.applyChosenTpl` | 템플릿으로 새 맵 |
| `AITab` | API 키 AI 로 새 맵 |
| `WebAiPanel.runOpenAsNewMap` | 웹 AI 로 새 맵 |

**`keepOrigin: true` 로 부르는 곳(템플릿 스타일만 입히기)은 그대로 둔다** —
같은 문서를 바꾸는 것이라 되돌아가는 게 맞다.

> **판별 기준**: `loadMap` 이 *새 문서를 만드는가*(→ `resetHistory: true`)
> 아니면 *같은 문서를 바꾸는가*(→ 그대로). 헷갈리면 "Ctrl+Z 로 이전
> 문서가 나와도 되는가"를 물으면 된다.

검증: e2e114 [1][1a].

---

#### 6.6-2 새 맵 만들기 확인 게이트 — "닫을 맵이 열려 있는가" (2026-08-06)

§6.6-1 의 **부작용**으로 한 건이 더 보고됐다.

> *"서버 맵을 불러와 열려 있는 상태에서 새 맵 만들기를 하면, 기존 맵을
> 닫는다는 메시지 없이 바로 새 맵 만들기로 감"*

확인 게이트(`NewMapPanel.confirmThen`)가 **`past.length === 0` 하나로**
"잃을 것이 없다"를 판정하고 있었다. 되돌리기 경계를 고치면서 서버 맵을
연 직후 `past` 가 0 이 되자, 확인 없이 곧바로 넘어갔다.

**판정을 규칙 하나로 단순화했다 — "닫을 맵이 화면에 있는가".**

| 묻지 않는다 | 묻는다 |
| --- | --- |
| 문서가 비어 있다 (`문서 없음` — 맵 닫기 직후·로그인 직후) | **그 밖의 모든 경우** |

편집 여부도, 서버 맵인지 로컬 파일인지도, 편집 모드인지 읽기 전용인지도
보지 않는다.

##### 여기까지 두 번 헛짚었다 (조건을 더하는 방식의 한계)

| 시도 | 빠진 경우 |
| --- | --- |
| `past.length === 0` 만 | 되돌리기 경계를 고치자(§6.6-1) 서버 맵을 연 직후 past 가 0 이 되어 **확인이 사라졌다** |
| `+ cloudMapId` | **읽기 전용으로 연 맵**(링크 없음)·**로컬 파일로 불러온 맵**이 여전히 빠졌다 (2026-08-06 2차 보고) |
| `+ saveState === 'unsaved'` | 문서를 *불러오기만 해도* 그 상태라 **손대지 않은 첫 화면에서도** 확인창이 떴다 (e2e103 이 잡아냈다) |

조건을 하나씩 더하는 방식이 계속 구멍을 남겨, **"화면에 맵이 있으면
묻는다"** 는 규칙 하나로 바꿨다. 새 맵 골격만 있는 상태에서 또 새 맵을
만들 때도 묻지만, 그건 취소 한 번이면 되고 **예측 가능성이 더 중요하다**
(사용자 결정: *"편집이든 읽기 모드든 오픈된 맵이 있으면 무조건 물어야
한다"*).

검증: e2e114 [2][3] · e2e115 [1a][1b][1c].
