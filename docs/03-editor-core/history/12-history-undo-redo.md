...
> 셀렉트 적용됨, e2e66).
>
> 한도는 `HISTORY_LIMIT = 100` 스냅샷. 툴바 되돌리기·다시 실행 버튼
> 사이의 **단계 카운터**(`data-testid="undo-depth"`)는 `future.length`를
> 보여준다 — 최신 상태 0, undo마다 -1씩, redo로 복귀, 새 편집 시 0
> (future가 비워지는 표준 히스토리 규칙과 일치, e2e67).
