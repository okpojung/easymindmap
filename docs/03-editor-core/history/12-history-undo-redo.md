...
**판정을 바꿨다 — "편집했는가"가 아니라 "닫을 맵이 열려 있는가".**

| 묻지 않는다 | 묻는다 |
| --- | --- |
| 문서가 비어 있다 (`문서 없음` — 맵 닫기 직후·로그인 직후) | **서버 맵이 열려 있다** (편집 안 했어도) |
| 서버 맵과 무관한 문서를 **한 번도 안 건드렸다** (첫 화면의 손대지 않은 샘플·기본 골격) | 편집 이력이 있다 (`past > 0`) |

> `saveState === 'unsaved'` 는 **쓰지 않는다.** 문서를 *불러오기만 해도*
> 그 상태가 되므로, 손대지 않은 첫 화면에서도 확인창이 떠 버린다
> (실제로 한 번 그렇게 만들었다가 e2e103 이 잡아냈다).
