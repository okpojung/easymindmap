...
`trust proxy` 를 고친 뒤에도 IP 가 `192.168.94.74` 그대로였다.
`/v1/health/ip` 가 원인을 정확히 짚어 줬다.

```
remoteAddress : ::ffff:10.0.1.6      ← 우리 앞 프록시(Traefik)
xForwardedFor : 192.168.94.74        ← 항목이 **하나**, 그것도 사설
```

우리 앞 프록시가 "접속자는 192.168.94.74"라고 적어 보냈고, 그 값이 **서로
...
조치는 runbook §6.4 참조 — `forwardedHeaders.trustedIPs` 에 NPM 대역. 즉 진짜 접속자 IP 는 **우리 API 에 닿기
