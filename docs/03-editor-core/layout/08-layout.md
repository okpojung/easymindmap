...
* 새 노드는 `layoutType` **미지정** 상태로 생성 — 렌더링 시 상위(부모→루트) 상속으로 해석한다 (값 복사 아님). **단, 그 레벨에 정해진 서브트리 레이아웃이 있으면 그것을 박는다** (§6.2 — 2026-09-04)
...
* **예외 — 레벨에 정해진 서브트리 레이아웃** (2026-09-04 사용자 보고).
  템플릿·`emm` 선언으로 만든 맵은 2레벨 노드마다 `tree-right` 같은 값이
  박혀 있다(#362). 그 레벨에 **손으로 노드를 하나 더 만들면** 미지정이라
  부모(1레벨 진행트리)를 물려받고, 거기에 AI 답변을 삽입하자 형제들과
  달리 진행트리로 펼쳐졌다. 그래서 새 노드(자식·형제·상위 추가·일괄
  추가)와 AI 삽입 노드는 **그 레벨의 것**을 박는다 — 순서는
  ⑴ `settings.levelLayouts[depth]`(설정 패널·선언) ⑵ 없으면 같은 깊이에
  이미 있는 첫 노드의 `layoutType`. 둘 다 없으면 예전대로 미지정이다.
  AI 삽입은 층마다 적용하고, **받는 노드**도 미지정이면 그 레벨의 것을
  박는다(그 노드의 자식 배치가 곧 형제와 같아지는 자리). 이미 지정된
  값은 건드리지 않는다 (`documentStore.levelLayoutFor` · e2e189).
...
* **윗변 정렬 행은 윗변을 늘린다** (2026-09-18, e2e278). 진행트리(모든
  레벨)와 트리·아래(1레벨)는 자식 행을 **윗변 기준**으로 나란히 놓는다.
  세로 배율을 노드 중심점에 곱하면 높이가 다른 형제의 윗변이 제각각
  벌어져, 부모에서 내려오는 가로 줄기(`midY`)가 높이 종류만큼 여러 줄로
  갈라졌다 — 세로 115% 진행트리에서 중심의 연결선이 6줄로 보였다. 그래서
  `applySpacing` 은 **부모의 배치가 윗변 정렬 행이면 그 노드의 윗변**을,
  아니면(형제를 세로로 쌓는 트리·오른쪽·계층형·방사형) 예전대로 중심점을
  루트 기준으로 늘린다. 판정은 배치가 남긴 부모의 `layoutType` 이다 —
  1레벨에 트리·오른쪽 오버라이드가 걸려 있어도 1레벨 행 자체는 루트
  (진행트리)가 놓은 것이므로 윗변 기준이 맞다.
...
## 서브트리 오버라이드의 크기 변화는 **레벨마다 길목 서브트리로** 민다 (2026-09-08)

> 실사용 보고: 트리·오른쪽 맵에서 발표순서(1단)에 진행트리를 걸고 그 밑
> 목차 여섯 개가 각각 개요형인데, 다음 가지(발표 슬라이드)가 수천 px 아래에
> 놓였다. "레이아웃을 바꿨는데 반영이 안 된 것 같다 — 맵 배치 버튼을 두면
> 어떨까" 라는 제안이 함께 왔다.

자동 배치는 그릴 때마다 문서에서 새로 계산한다(저장된 좌표가 없다). 그래서
"다시 배치" 버튼은 같은 답을 다시 낼 뿐이다 — 문제는 계산 자체에 있었다.

`SubtreeStrategy.propagateDelta` 는 오버라이드한 노드 **하나의** bbox 변화
(after − before)를 조상 체인의 **모든 레벨**에 그대로 적용했다. 진행트리처럼
형제가 가로로 나란한 부모 아래에 개요형 자식이 셋이면, 셋이 **각각** 아래로
자란 만큼이 루트 레벨의 다음 형제에 **세 번 더해진다** — 실제로 늘어난 것은
셋 중 가장 큰 것 하나뿐인데. 재현(발표순서=진행트리, 목차 3개=개요형)에서
다음 가지까지의 틈이 **622px**(정상 18px)이었다.

이제 `propagateByLevels` — 체인의 각 레벨이 **길목 노드**(`pathNode`, 그
레벨에서 오버라이드로 내려가는 조상)를 들고 있고, 바꾸기 전에 그 서브트리의
bbox 를 레벨마다 재 둔다. 바꾼 뒤에는 **깊은 레벨부터** 올라오며 길목
서브트리를 다시 재어 "정말 커진(줄어든) 만큼"만 형제를 민다. 아래 레벨에서
민 것이 위 레벨의 bbox 에 이미 들어 있으므로 중복이 없다. 줄어들면(개요형
자리에 진행트리를 걸어 납작해짐) 그만큼 당겨 온다.

검증 e2e242 — 루트 레이아웃 × 오버라이드 조합 10가지에서 겹침 0, 다음
형제까지의 틈이 그 레이아웃의 기본 간격(트리·오른쪽 18 · 방사형 62 · 계층형
12 · 진행트리 x 28 · 트리 아래 x 172). 옛 코드는 같은 조합에서 틈이 최대
622px(사용자 보고 구조).


## 서브트리 오버라이드 — 맵이 엉망이 된 사고 조사와 재발 방지 (2026-09-22)

> 사용자 보고: 진행트리 맵에서 달력 노드 `[36주] …` (3레벨)을 골라 레이아웃 탭에서
> 진행트리·오른쪽을 누르자 맵 전체가 흐트러졌다 — 주 노드들이 위·오른쪽으로 튀고,
> 날짜 노드 하나가 화면 밖 멀리 놓이고, 표 노드 위에 다른 가지가 겹쳤다.
> "원인을 조사하고 재발 방지 대책을 세워 달라".

### 원인 — 두 겹이었다

**① 근본 원인: 노드 id 충돌.** `documentStore.createNodeId()` 가 `node-<ms>-<0~999 난수>`
였다. 달력 노드 추가(#544)는 주 5개 + 날짜 35개 = **40개를 한 틱에** 만든다 — 같은
ms 안에서 1000개 값 중 40개를 뽑으면 **절반 넘는 확률로 같은 id** 가 나온다
(생일 문제). 기본 배치(전략)는 id 를 보지 않아 멀쩡해 보였지만, **서브트리
오버라이드는 id 로 노드를 찾아 떼었다 붙인다** (`out.find`, `descendantIds` Set,
`splice`). 겹친 id 가 있으면 다른 가지의 노드가 함께 떼어져 엉뚱한 자리에 놓이거나
사라진다 — 헤드리스 재현: 날짜 노드 둘의 id 만 같게 해도 노드 수 65 → 64, 겹침 9,
한 노드가 y = −30 (화면 밖). 사용자가 본 그림과 같다. 오버라이드를 걸기 전엔 안
보이고 거는 순간 터지는 이유다. 확률적이라 같은 조작이 어떤 때는 멀쩡했다.

**② 그 밑에 깔려 있던 배치 결함 9종.** 무작위 맵 1,500개와 사슬 행렬(1·2·3레벨
오버라이드 10×10×10, 기준 레이아웃 7종)로 겹침을 세어 찾았다 (아래 표). 사용자
보고 하나만으로는 못 찾았을 것들이다.

| # | 결함 | 고침 |
|---|---|---|
| A | 계층형: 첫 자식이 부모보다 크면(표·코드) 중심을 맞춰 **위로 삐져나와** 앞 형제와 겹침 | 첫 자식 윗변을 부모 윗변까지만 (`HierarchyStrategy`, 맵·오버라이드 공통) |
| B | `clearRootCollision` 이 아래로 늘어지는 서브트리(진행트리·트리·아래·트리·오른쪽)를 루트 위로 **들어 올려** 앵커와 겹침 | 그런 서브트리는 루트를 **옆으로** 비킨다 |
| C | 옆에 두는 부모(계층형·방사형·시간배치) 아래 가운데 정렬 오버라이드(트리·아래 등)가 앵커 **부모 쪽으로 삐져나와** 부모 상자 안에 | 앵커의 실제 놓인 쪽(`side`)을 보고 바깥쪽 모서리까지만 (`relayoutSubtree` 옆 보정) |
| D | 방사형·왼쪽/양쪽 왼쪽 가지에 계층형을 걸면 자식이 **오른쪽(부모·루트 쪽)** 으로 | 앵커가 왼쪽이면 거울상(`hierarchy-left` 로 표시, EdgeRenderer 가 왼쪽 변에 선) |
| E | 시간배치 안에서 자란 서브트리가 뒤 주제를 **밀지 않았다** (axis null) · 위쪽 가지 스택은 **위로** 쌓이는데 아래로 밀었다 · 왼쪽 앵커의 축은 부모로 되돌아갔다 | 축 시작 노드의 자식은 x축, 축 위/스택 노드의 자식은 y축(위쪽이면 반대 방향) — `axisOf(effective, laidParent, selfIsTimelineAnchor)` · `ChainLevel.reversed` · 왼쪽 앵커면 축을 뒤집고 `_timelineDir` 로 표시 |
| F | 시간배치 오버라이드는 위로 넘쳐도 안 내렸다(2026-08-07 결정) → 진행트리 윗줄과 겹침 | **실제로 겹칠 때만** 내린다 (`collidesOutside`) |
| G | `propagateByLevels` 가 위로 자란 만큼 **앞 형제를 올렸다** — 윗변 정렬 스택(트리·오른쪽/계층형)에선 루트·윗줄과 겹침 | 가운데 정렬 부모(방사형)에서만 (`ChainLevel.centered`) |
| H | `separateBranchGroups` 연쇄 분리가 **안 밀린 가지끼리도** 8px 여유로 갈라 패스마다 조금씩 밀어 루트 위까지 | 이번 패스에 밀린 가지가 낀 쌍만 · 루트는 장애물(닿으면 반대쪽) |
| I | `branchIdOf` 가 id 가 겹친 문서에서 parent 사슬을 **무한히** 돌 수 있었다 | 횟수 guard |

### 고친 것

- `createNodeId`/`createSubId`: 프로세스 일련번호 + 36진수 6자리 난수 (`node-<ms>-<seq><rand>`).
- `dedupeNodeIds(map)`: **여는 순간**(`loadMap`·`applyRemoteMap`) 겹친 id 의 뒤 노드에 새 id — 예전
  생성기로 만든 맵(이미 저장된 달력 맵)도 다시 열면 낫는다. 첫 노드는 그대로라 참조가 안 흔들린다.
- `SubtreeStrategy`·`HierarchyStrategy` 위 표 A~I.

### 재발 방지 — **불변식을 기계가 지킨다**

1. `src/layout/layoutInvariants.test.ts` (단위 체인 `test:unit`, CI 매 PR) — 노드 상자 **겹침 0**,
   **id 유일·노드 수 = 문서 노드 수**(`checkIdentity`) 를 ① 결함마다 고정 사례 ② 보고 모양 사슬
   400조합 ③ 고정 씨앗 무작위 200맵(기준 7종)에서 확인한다. 약 1초.
2. `src/stores/nodeIds.test.ts` — 한 틱에 200개 만들어도 id 유일 · `loadMap` 이 겹침을 고친다.
3. 넓게 볼 때: `npx tsx scripts/layout/fuzz.ts 1 1500 [base]` · `scripts/layout/matrix.ts <base> [o1/o2/o3]`
   (같은 생성기 `src/layout/invariantGen.ts`, 조합을 주면 좌표까지 찍는다).
4. 브라우저: `scripts/doc-shots/guide04-layout-nested.mjs` — 보고와 같은 조작(레이아웃 탭 버튼)을
   실제 캔버스에서 하고 **엔진 결과와 화면 상자** 모두 겹침 0 을 잰다 (17항목).
5. 규칙: `src/layout/**` 을 고치면 위 1이 체인에서 돈다. 새 레이아웃·오버라이드를 더할 때는
   `SUBTREE_SUPPORTED` 와 함께 ②의 조합에 자동으로 들어간다. 노드를 여럿 만드는 새 기능은
   `createNodeId` 만 쓴다 (직접 `Date.now()` id 를 만들지 않는다).

| J | `separateBranchGroups` 가 **가지 bbox 끼리** 닿으면 밀었다 — 시간배치의 위쪽 스택이 앞 가지 bbox 의 빈 구석에 들어가도(노드는 안 닿아도) 앞 가지를 위로 밀어 그 위 가지와 겹침 | 노드끼리 실제로 닿을 때만 (`nodesCollide`) |

**아직 남은 조합** (사슬 행렬 7,000조합 중 10): 트리·오른쪽 맵 2(1레벨 진행트리 →
2레벨 시간배치), 방사형·양쪽 2(1레벨 시간배치 → 2레벨 진행트리 → 3레벨 트리·아래),
방사형·왼쪽 6(왼쪽 가지 안의 시간배치 중첩). 모두 **시간배치가 다른 오버라이드 안에
겹쳐 걸린** 드문 경우다. 진행트리·계층형·방사형·오른쪽·트리·아래 맵은 1,000조합 전부
0, 무작위 1,500맵 0.

## 23. 여러 중심주제 배치 (2026-09-15, 1단계 — 읽기)

mmd 표준 세션 결정(emm-spec §3.1 "여러 중심주제"): `#` 가 둘이면 한 장의
맵에 **중심주제가 둘**이다. 모델은 `SampleMap.root`/`branches` (첫 중심) +
`SampleMap.centers[]` (`{ root, branches, pos? }`, 두 번째 이후).

`computeLayout(map, …)` 은 이렇게 놓는다 (`LayoutEngine.ts`).

1. 첫 중심을 (CX, CY) 에 예전 그대로 배치한다 — `computeCenterLayout`.
   전략(방사형·트리·계층형·진행트리·시간배치)과 서브트리 오버라이드·간격
   배율은 **중심 하나 안에서** 돈다. 전략 코드는 중심이 여럿인 것을 모른다.
2. 두 번째 이후의 중심은 각각 **자기 루트의 레이아웃**(없으면 맵 레이아웃)
   으로 원점(0,0)에 배치한 뒤 통째로 옮긴다.
   - `pos` 가 있으면 첫 중심 루트 기준 상대 좌표 `(CX+dx, CY+dy)` 에.
   - 없으면 **자동 배치** — 지금까지 놓인 것의 테두리(bbox) 오른쪽에
     `CENTER_GAP`(160px) 을 두고, 루트 높이는 첫 중심과 나란히(`y = CY`).
3. 돌려주는 배열은 중심 순서대로 이어 붙인 것. 중심 루트는 모두
   `depth 0 · parent null · side 'center'` 라 렌더러(`depth === 0` 판정)·
   엣지(부모 없음 → 선 없음)는 손대지 않았다.
   ★ 전략들은 최상위 가지의 부모를 글자 그대로 `'root'` 로 박는다 — 둘째
   이후의 중심은 `computeCenterLayout` 끝에서 `'root'` → 그 중심 루트 id 로
   바꿔 준다. 빠뜨리면 그 가지의 연결선이 **첫 중심에서** 그어진다
   (PR #490 Codex 지적).

칸반은 예전처럼 첫 중심만 그린다(빈 배열 반환은 그대로).

**스토어**: `findNodeInMap`·`getNodeDepth`·`findParentId`·`buildParentIndex`·
`nodePathInMap`·`mutateNode`(글·스타일 등 노드 하나 고치기) 는 중심을 가리지
않는다. 중심 루트의 부모는 `null`, 그 가지의 부모는 그 중심 루트의 id.
### 23.1 편집 (2단계, 2026-09-15)

**스토어는 "중심주제 하나를 보는 창"(`CenterView`)으로 돈다.** 추가·형제·
상위·삭제·이동 액션은 "루트 + 가지" 를 전제로 쓰여 있는데, 중심이 여럿이면
그 전제가 **중심마다** 성립한다. 그래서 노드가 속한 중심을 찾아
(`centerOfNode`) 그 중심의 `(rootId, root, branches)` 를 주고 결과를
되돌려 쓴다(`withCenter`). 액션 본문은 `'root'`→`view.rootId`,
`map.branches`→`view.branches` 로 바뀐 것뿐이다. id 로 찾는 것(삭제·접기·
레벨 레이아웃)은 모든 중심에 같은 변환을 건다(`mapAllBranches`). 노드는
**중심을 건너** 옮겨진다 — 떼어 내는 것은 전체(`extractFromMap`), 붙이는 것은
대상 중심의 창.

| 동작 | 어디서 | 규칙 |
|---|---|---|
| 새 중심주제 | 우상단 툴바 [중심 ▸ 새 중심주제] | `addCenter` — 맨 뒤 순서, 자동 배치(앞 테두리 오른쪽). 새 루트가 선택된다 |
| 중심 옮기기 | 둘째 이후의 중심 루트를 **끌어서** | 드롭존 없이 자리만 옮긴다 — `setCenterPos(dx, dy)` = 첫 중심 루트 기준 상대 좌표(반올림). 첫 중심('root')은 원점이라 끌 수 없다 |
| 중심 지우기 | 중심 루트 선택 + `Del` / 툴바 🗑 | `deleteNode(centerRootId)` — 그 중심을 **가지째** 지운다(노드 삭제와 같은 규칙). 첫 중심은 못 지운다. 되돌리기 가능 |
| 묶기 | 중심 루트 선택 + [중심 ▸ 묶기] | `mergeCentersInto(rootId)` — 다른 중심 전부가 이 중심의 **가지 하나씩**이 되고 그 가지들이 자식이 된다(문서 순서). 남는 중심이 둘째 이후면 그것이 `'root'` 자리로 오고 옛 첫 중심은 새 id 의 가지가 된다 |
| 올리기 | 1레벨 가지 선택 + [중심 ▸ 올리기] | `promoteToCenter(branchId)` — 가지를 떼어 새 중심(맨 뒤)으로, 자식들이 그 중심의 가지가 된다(id 유지) |
| 노드 편집 | 어느 중심에서든 | 추가(+버튼·Tab·Enter)·삭제·드래그 이동·다중 이동·접기·좌우·레벨 레이아웃·루트 레이아웃 전부 첫 중심과 같다 |

**레이아웃 탭**: 둘째 이후의 중심 루트를 고르면 **그 중심의 레이아웃**만 바뀐다
(맵 전역·첫 중심의 오버라이드는 건드리지 않는다). 새 노드의 레벨 레이아웃 폴백
(`levelLayoutFor` ⑵ "같은 깊이의 형제")은 **그 노드가 들어갈 중심의 가지**에서
찾는다. 묶기로 둘째 이후의 중심이 첫 중심이 되면 그 루트의 레이아웃을 전역
(editorUiStore)에 맞춘다 — 셋 다 PR #493 Codex 지적.

검증 e2e268 — `src/stores/centers.test.ts` 81항목.

> 선언 블록 위치·머리말 처리·묶기/올리기의 **세부**는 mmd 표준 세션에서
> 정하는 중이다(2026-09-15 사용자). 여기 묶기/올리기는 `mindmap-markdown-
> alignment.md` A-1 의 문구를 앱의 트리 편집으로 옮긴 것 — 결과는 어느 쪽이든
> 표준 트리(`# … ## …`)라 표준이 정해지면 **세부만** 맞춘다.

### 23.2 둘째 중심 이후를 보는 곳 (3단계, 2026-09-16)

| 곳 | 규칙 |
|---|---|
| 아웃라인 | 중심주제마다 최상위 행 하나(`EditorPage.outline` = `mapCenters`). 중심 행에서 Enter/＋자식 = 그 중심의 가지 추가, 형제·들여쓰기 없음. 첫 중심만 못 지운다 |
| 검색 | 모든 중심의 루트·가지·태그. 경로는 그 중심의 이름에서 |
| 칸반 | 컬럼 = 모든 중심의 1레벨 가지(중심 순서 → 문서 순서). 컬럼 id 가 가지 id 라 카드 이동이 중심을 건넌다 |
| HTML 뷰어 | 내보내기 `data.centers[]`(루트와 같은 모양, `pos` 구움). 뷰어는 `ROOTS()` 로 뿌리 전부를 그린다 — 검색·아웃라인·펼치기/접기·노드 수·포커스도 뿌리 전부. 접기·펼치기 뒤(동적 재배치)는 둘째 중심을 앞 중심의 오른쪽 160px 에 새로 잰다 |
| MCP | `findByPath`: `parent`/`nodes` 에 중심주제 이름을 쓰면 그 중심(가지는 "중심 > 가지"), `id:` 는 둘째 중심 루트도. `append_to_map` 은 그 중심의 가지로 붙이고, `check_items` 는 그 중심 루트의 체크를 바꾼다. `create_map` 은 템플릿 레벨 레이아웃을 모든 중심에 |
| 파서 | `#` 없는 문서의 최상위 항목이 각각 중심(A-1 ②) — emm-spec §3.1 |
