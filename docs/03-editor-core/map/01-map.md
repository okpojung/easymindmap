...
2. (편집 중인 맵이 있으면) **닫아도 되는지 확인**한 뒤 **템플릿 선택 단계**를 연다.
   이 시점까지 **문서는 아직 그대로다** — 여기서 ✕(취소)를 누르면 아무 일도 없었던
   것이 된다 (2026-08-05: 예전에는 누른 즉시 문서를 갈아 끼운 뒤 템플릿을 물어,
   그 단계의 '취소'가 "이미 지운 맵 되살리기"가 됐다)
3. 템플릿을 고르거나 `건너뛰기(기본 그대로)` 를 누른 **그때** 새 문서로 갈아 끼운다
   — 제목은 묻지 않고 **'새 맵'** 으로 시작한다 (이름 변경은 저장 시)
4. 시스템은 root + 기본 골격(주제 1~3, 11노드)을 자동 생성한다 (템플릿을 골랐으면
   그 템플릿의 4레벨 골격)
5. 기본 layout 및 초기 설정을 적용한다
6. 생성 완료 후 편집 화면으로 진입한다. `map_id` 는 첫 저장 때 서버가 만든다
