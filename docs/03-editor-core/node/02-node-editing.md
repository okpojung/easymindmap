...
* 노트 문단의 `html`(리치 붙여넣기)은 **같은 sanitizer(`sanitizeRichHtml`)로
  다시 걸러** 통과시킨다 (2026-09-11 — 전에는 버려서 사진이 사라졌다)
...
## 드롭존은 **실제 배치 기하**로 정한다 (2026-09-08)

> 실사용 보고: 트리·오른쪽 맵에서 한 줄(발표순서)만 진행트리로 두었더니,
> 그 아래 2단 노드(목차(정))의 **왼쪽에 놓으면 형제가 아니라 하위로**
> 붙었고, 하위/상위 존은 보이지 않았다.

위 2026-08-04 규칙은 "실효 레이아웃 이름 → 자식 축" 표 하나(`childDirOf`)로
네 존을 정했다. 두 가지가 어긋났다. ⑴ 표가 개요형(트리 오른쪽 —
`layoutTreeRight` 는 자식을 **아래**에 들여써 놓는다)을 "오른쪽"으로 적고
있었다. ⑵ 형제 축을 **부모의** 표에서 가져와, 진행트리 줄(형제 가로) 아래에
개요형 노드가 오면 실제 배치와 달랐다.

이제 `editor/canvas/dropGeometry.ts` 가 **laid-out 좌표**에서 읽는다
(`zoneAxesFor` · `zoneAt`, 판정과 바 그리기가 같은 함수를 쓴다):

| 축 | 어떻게 정하나 |
|---|---|
| 형제 축 | **같은 쪽(side)의** 가장 가까운 형제와의 dx·dy 비교. 형제가 없으면 부모 실효 레이아웃의 관례(`convSibAxis` — 트리 아래·진행트리·시간배치 축은 가로, 나머지는 세로) |
| 자식 방향 | 자식 무게중심이 있는 쪽. 자식이 없으면 그 노드 실효 레이아웃의 관례(`convChildDir` — 개요형은 **아래**) |
| 부모 방향 | 자식의 반대. **단, 자식 축이 형제 축과 겹치면**(개요형·시간배치 스택) 형제 축이 이기고 부모/자식은 수직 축으로 — 부모가 있는 쪽(들여쓰기의 반대)이 상위, 그 반대가 하위 |
| 이전/다음 | 형제 축에서 **앞 형제가 있는 쪽**이 이전(시간배치 위 스택은 축 쪽이 이전) |

존 판정은 "박스 밖으로 **가장 많이 벗어난 변** 하나"로 한다 — 네 변이 서로
다른 뜻을 가지므로 어느 변이든 반드시 한 존에 닿는다(예전엔 축 판정이
겹쳐 두 존만 닿는 노드가 있었다). 박스 안 = 하위, 루트 = 어디든 하위는
그대로다. 레이아웃별 결과 표는 사용자 설명서 03-노드-편집 "노드 이동".

검증 e2e239 — 단위 16 + Chromium 68(레이아웃 7종 × 노드 2~3 × 네 변,
사용자 맵 구조 포함). 옛 코드로 같은 검증을 돌리면 61/68 — 실패 7건이
사용자가 겪은 목차(정) 네 방향 + 시간배치 스택 셋이다.
