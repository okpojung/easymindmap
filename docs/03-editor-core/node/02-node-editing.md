...
> 달려 있다.** ThinkWise 는 그림을 복사해도 표준 이미지 대신 자기
> 형식과 `[--iThinkWise--]` 표시만 넣어, 브라우저가 읽을 수 없다.
> 그 표시만 온 붙여넣기는 노드로 만들지 않고 캔버스 위에 안내를 띄운다
> (`canvas-notice`) — 예전에는 그 글자가 그대로 노드가 됐다.
> 그림을 옮기려면 ThinkWise 에서 이미지 파일로 저장하거나 화면을
> 캡처해 붙여넣는다.
