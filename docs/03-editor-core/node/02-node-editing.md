...
- 문서 버전: v1.1
- 작성일: 2026-04-05
- 최종 업데이트: 2026-08-04 — 실제 구현 기준으로 현행화(더블클릭 편집 진입·Enter=형제 추가, 새 노드 자동 편집 없음, 길이 제한 없음, 수동 좌표 없음, 저장은 맵 전체 문서 스냅샷, 폰트 기본 18/14/13).
...
| NE-002 | 자식 노드 추가 | 선택된 노드 아래에 자식 노드 추가 | Space/Tab/인디케이터 + |
| NE-003 | 형제 노드 추가 | 선택된 노드와 같은 부모 아래 형제 노드 추가 | Enter, 인디케이터 + |
| NE-004 | 노드 제목 편집 | 노드의 기본 텍스트(label/title) 수정 | 더블클릭으로 인라인 편집 진입 (Enter는 형제 추가) |
...
| NE-012 | 수동 위치 보정 [미구현 — 후속 과제] | 좌표 수동 조정은 미구현 — 현재 수동 조정은 크기(sizeW/sizeH)만 지원 | drag to resize |
| NE-013 | 인라인 생성 흐름 | 새 노드는 '새 노드' 텍스트로 생성·선택만 (자동 편집 진입 없음) | create → select (편집은 더블클릭) |
...
- 노드는 제목(label)을 직접 수정할 수 있다. 새 노드는 '새 노드' 텍스트로 생성·선택되며, 편집은 더블클릭으로 진입한다.
...
- 선택된 노드에서 다음 동작을 수행할 수 있다.
  ...
  - 복제
  - 접기/펼치기
- 더블클릭 시 노드 텍스트 편집 모드로 진입한다. (Enter는 편집 진입이 아니라 **형제 추가**, Space/Tab은 자식 추가, Ctrl+←는 부모 추가.)
- 새 노드는 '새 노드' 텍스트로 생성되고 **선택만** 된다. 자동 편집 진입은 없다.
- **빈 텍스트도 저장된다** (2026-08-05). 편집 중 전체 선택 후 `Del`·`Ctrl+X`
  로 지우고 Enter 를 치면 노드 내용이 정말 비워진다. 예전에는 편집창과
  스토어(`updateNodeText`) 양쪽에 "빈 문자열이면 무시" 가드가 있어 지운
  글자가 되살아났고, 사용자에게는 "지워지지 않는다"로 보였다 —
  **내용을 비우는 것**과 **노드를 지우는 것**은 다른 조작이다
  (노드 삭제는 선택 상태에서 `Del`).
- 드래그 앤 드롭 시 시스템은 “같은 부모 내 순서 변경”인지, “부모 변경”인지, “금지된 구조 변경”인지를 판별한다.

#### 5.2 Clipboard / Paste 정책
##### 기본 원칙
Canvas 또는 선택 노드 상태에서 일반 텍스트를 붙여넣는 경우,
붙여넣은 전체 텍스트는 자동 구조 변환하지 않고 **하나의 노드 content로 저장한다.**

즉, 줄바꿈 / 들여쓰기 / Markdown list가 포함되어 있더라도
기본 Paste 동작에서는 여러 노드로 분해하지 않는다.

##### 기본 Paste 동작
| 상황                  | 동작                        |
| ------------------- | ------------------------- |
| 노드 선택 상태에서 텍스트 붙여넣기 | 선택 노드 기준으로 단일 노드 생성       |
| 빈 Canvas에 텍스트 붙여넣기  | 현재 focus 위치에 단일 노드 생성     |
| 여러 줄 텍스트 붙여넣기       | 하나의 노드 content에 줄바꿈 포함 저장 |
| Markdown list 붙여넣기  | 하나의 노드 content에 원문 그대로 저장 |

##### 자동 구조 변환 제외
기본 Paste에서는 다음 동작을 수행하지 않는다.
* 줄바꿈 기준 다중 노드 생성
* 들여쓰기 기반 parent-child 자동 생성
* Markdown list 자동 분해
* Heading 기반 tree 생성

##### ThinkWise식 붙여넣기 (MVS 구현 — 2026-07)

노드를 **선택만** 한 상태(편집 상태 아님)에서 Ctrl+V 하면 ThinkWise와
같이 **선택 노드의 하위 노드를 새로 만들고 클립보드 내용을 모두 그
노드에 직접 넣는다**. 만든 하위 노드가 바로 선택된다.

> 처음 구현은 여러 줄·기사를 "첫 줄 = 제목 + 나머지는 문단 노트"로
> 나눴으나, 사용자 피드백(2026-07)으로 **Case 구분 없이 무조건 노드에
> 모두 붙여넣기**로 변경했다. 노트는 만들지 않는다.

| 클립보드 내용 | 하위 노드에 들어가는 것 |
| --- | --- |
| 텍스트 (한 줄/여러 줄) | 노드 텍스트에 줄바꿈 포함 **전체 저장** (기본 원칙 그대로) |
| 웹 기사 등 서식 HTML | 텍스트 전체 = 노드 텍스트, 사진들은 **원문 위치 그대로 텍스트 중간에** (인라인 사진 `images[]` — 04-node-content.md §노드 인라인 사진) |
| 이미지 파일 (스크린샷 등) | 노드 텍스트 = "이미지", 사진을 노드 안에 표시 |

- "단일 노드 content로 저장" 원칙 유지 — 여러 줄이어도 노드를 여러
  개로 분해하지 않는다.
- 입력창(노트·검색 등)에 포커스가 있을 때는 각 입력창의 붙여넣기가
  우선한다. 노드 텍스트 **편집 중** 붙여넣기도 같은 원칙 — 텍스트는
  입력창에, 사진은 노드 사진으로 (04-node-content.md 참조).
- 검증: E2E e2e39 [A]~[E].

##### 노드 복사·붙여넣기 — 다른 맵·다른 창까지 (2026-08-05)

`Ctrl+C`(복사)·`Ctrl+X`(잘라내기)는 선택 서브트리를 **두 갈래**로 담는다
(`utils/nodeClipboard.ts`).

| 담는 곳 | 쓰는 때 | 특징 |
| --- | --- | --- |
| 메모리 스택 | 같은 탭 | 원본 그대로 — 가장 정확하고 큰 사진도 클립보드를 거치지 않는다 |
| 시스템 클립보드 텍스트 | **다른 탭·다른 맵·다른 창** | 첫 줄 `EMM-NODES::<토큰>` + 둘째 줄부터 서브트리 JSON |

붙여넣기는 토큰으로 메모리 스택을 먼저 찾고, 없으면 JSON 을 파싱한다.
JSON 은 시스템 클립보드를 거쳐 오므로 **믿을 수 없는 입력**으로 보고
`sanitizeNodes()`가 아는 필드만 통과시킨다:

* 사진·링크 주소는 `data:image/` 와 `http(s)` 만
* 노트의 `html`(리치 붙여넣기 원문)은 **버린다** — 텍스트만 남긴다
* **첨부파일은 옮기지 않는다** — 원본 맵의 저장소(blob:·서버 id)에 묶여
  있어 다른 맵에서는 열리지 않는다
* 노드 5000개·깊이 40 상한, 클립보드 텍스트 4MB 상한(넘으면 토큰만
  써서 같은 탭 전용이 된다)

> **다른 마인드맵 앱에서의 복사는 그 앱이 무엇을 클립보드에 넣느냐에
> 달려 있다.** ThinkWise 는 노드를 복사하면 text/plain 첫머리에
> `[--iThinkWise--]` 표시를 넣는다 — `utils/foreignClipboard.ts` 가
> 붙여넣기 경로(캔버스·노드 편집창)에서 이 표시를 걷어낸다.
>
> | 클립보드 | 결과 |
> | --- | --- |
> | 표시 + 내용 (노드 복사) | **표시만 지우고** 내용은 그대로 붙는다 — 하위 노드가 있으면 여러 줄이 한 노드 본문으로(붙여넣기 기본 정책) |
> | 표시만 (그림 복사) | 노드를 만들지 않고 캔버스에 안내(`canvas-notice`) — ThinkWise 가 클립보드에 **표준 이미지를 넣지 않아** 브라우저가 그림을 읽을 수 없다. 이미지 파일로 저장하거나 화면을 캡처해 붙여넣어야 한다 |
>
> 예전에는 그 표시가 그대로 노드 글자로 남았다 (2026-08-05 보고).

검증: e2e104 [3][4].

##### 다중 노드 생성 방식
여러 줄 텍스트를 노드 구조로 변환하려면
기본 Paste가 아니라 **다중 가지 추가(Bulk Branch Insert)** 기능을 사용한다.
...
##### 설계 이유
기본 Paste는 사용자의 **원문 보존을 최우선**으로 한다.
자동 구조 변환은 의도하지 않은 노드 분해를 유발할 수 있으므로,
명시적인 기능(다중 가지 추가)에서만 수행한다.
...
- 새 노드는 '새 노드' 텍스트로 생성·선택만 된다. 제목 편집 자동 진입은 없다(편집은 더블클릭).

#### 6.3 제목 편집 규칙
- 노드 제목은 빈 문자열 저장을 허용하지 않는다.
- 사용자가 빈 값 상태로 편집을 종료하면 **직전 텍스트를 유지**한다.
  - 기존 노드: 이전 제목 그대로 유지
  - 신규 노드: 기본 텍스트 '새 노드'가 그대로 잔류 (노드 자동 삭제 없음)
- 제목 길이 제한은 강제하지 않는다. (예외: Markdown 불러오기 시 `NODE_A4_CHARS` 2500자 초과 문단만 별도 규칙 적용 — 07-markdown-format-policy.md 참조.)
...
- [서버 연결 예정] 수동 좌표(manualPosition)는 현재 구현에 없다 — 수동 조정은 **크기(sizeW/sizeH)** 만 지원한다(90~900px). 자유배치 좌표 기록은 후속 과제.
...
> **[서버 연결 예정]** 현재 저장은 **맵 전체 JSON 스냅샷**(`PUT /maps/:id/document` → `map_documents.doc`)만 사용한다. 노드 단위 API·정규화 컬럼 쓰기는 없다. 아래 테이블/컬럼 구성은 협업 단계를 위한 설계안이다.
...
- 생성/수정/삭제 컬럼 예시:
  ...
  - nodes.title
  - nodes.collapsed
  - nodes.manual_position [서버 연결 예정] — 수동 좌표는 현재 구현에 없음(수동은 크기 sizeW/sizeH만)
...
> **[서버 연결 예정]** 현재는 노드 단위 API가 없고, 편집 결과는 **맵 전체 문서 스냅샷 저장**(`PUT /maps/:id/document`)으로만 반영된다. 아래 API 목록은 협업 단계 설계안이다.
...
- 요청:
  - title
  - sizeW / sizeH (수동 크기 — [서버 연결 예정] 수동 좌표 manualPosition은 현재 구현에 없음)
  - collapsed
- 응답:
  - 수정된 node
  - updated_at
...
- 새 노드는 “새 노드” 상태로 생성되고 선택된다(자동 편집 진입 없음).
- 사용자가 더블클릭으로 편집에 진입해 “Apache 설치”라고 입력하고 확정하면 저장된다.
...
#### 시나리오 3: 빈 제목으로 편집 종료
- 사용자가 새 노드를 추가한 뒤 더블클릭으로 편집에 진입했지만, 아무 글자도 입력하지 않고 ESC를 누른다.
- 시스템은 빈 값 저장을 하지 않고 직전 텍스트를 유지한다.
- 신규 노드는 기본 텍스트 '새 노드'가 그대로 잔류한다(노드 자동 삭제 없음).
- 기존 노드를 비운 경우에도 삭제하지 않고 이전 제목을 유지한다.
...
형제 노드 생성, 자식 노드 생성, 다중 자식 노드 생성 및 다중 가지 추가 시, **새로 생성되는 노드는 기준 노드의 `style` 객체 전체를 복사**한다. 단, `fontSize`는 새 노드의 실제 깊이(depth)에 맞는 기본값으로 재설정하고, `colorKey`(테마 색상 키)는 그대로 상속한다.
...
```typescript
// 노드 생성 로직 (현행 구현)
newNode.style = { ...baseNode.style };           // style 전체 복사 (shapeType, fillColor, borderColor 등)
newNode.style.fontSize = defaultFontSizeFor(depth); // 폰트 크기는 깊이별 기본값으로 재설정 (18/14/13)
newNode.colorKey = baseNode.colorKey;            // 테마 색상 키 상속
```
...
#### 17.2 맵 단위 Typography 데이터 구조 (설계 초안 — 미채택: 현행은 레벨별 size·family·align, 기본 크기 18/14/13)
...
#### 17.4 레벨별 폰트 스타일 기본값 (현행)

레벨별로 설정 가능한 항목은 **크기(size)·글꼴(family)·정렬(align)** 이며, 레벨별 굵기(weight) 설정은 없다.

| Level | Font Size |
|-------|-----------|
| Root (Main Topic) | 18px |
| Level 1 | 14px |
| Level 2+ | 13px |

> 레벨별 Font Weight 표(Bold/SemiBold/…)는 설계 초안이었으며 미채택.
...
| 구분 | 기능 | 포함 여부 |
|------|------|----------|
| MVP | 레벨별 폰트 크기 (기본 18/14/13) | ✔ |
| MVP | 레벨별 글꼴(font-family) | ✔ |
| MVP | 레벨별 정렬(align) | ✔ |
| — | 레벨별 폰트 굵기 | ✗ 없음 (레벨 설정은 size·family·align만) |
...
## 드래그 이동 드롭존 — 레이아웃 방향 인지 (2026-08-04)

노드를 끌어 다른 노드 위에 올리면 초록 바로 붙일 위치를 보여준다.
존 판정과 바 방향은 대상 노드의 **실효 레이아웃**을 따른다:

- **자식/부모 존** = 대상의 "자식이 자라는 방향" 축 — 가로 레이아웃
  (방사형·트리 오른쪽·계층형)은 좌/우 변, 세로 레이아웃(진행트리·
  트리 아래·시간배치)은 아래(위) 변.
- **형제 존(이전/다음)** = 부모의 자식 방향에 수직인 축 — 진행트리는
  형제가 가로로 배열되므로 **왼쪽 세로 바 = 이전 형제, 오른쪽 세로
  바 = 다음 형제**다. 가로 레이아웃은 위/아래 가로 바.

이전에는 side(좌/우)만 보고 판정해, 진행트리에서 아래 바(다음 형제)가
"하위"로 읽히고 오른쪽에는 아무 표시도 없던 불일치가 있었다(실사용
보고 — Canvas.tsx childDirOf/findDropZone).

### 노드 **안쪽 = 항상 '하위'** (2026-08-05)

위 축 규칙만으로는 **하위에 넣을 수 없는 노드**가 생겼다. 자식이
가로로 자라는 노드(tree-*)에서는 자식/부모 존이 좌·우에만 있어서,
사용자가 직관대로 **아래로 끌어도 좌우 형제 표시만** 떴다. "존이
2곳만 보이고 4곳이 안 보인다"도 같은 원인 — 축이 노드마다 달라
나머지 두 존에 닿지 못한 것이다.

그래서 판정을 **위치가 아니라 영역**으로 바꿨다:

| 커서 위치 | 존 |
|---|---|
| 대상 노드 **박스 안** | **child** — 레이아웃과 무관하게 항상 |
| 박스 바깥, 자식 축 방향 | child |
| 박스 바깥, 자식 축 반대 | parent (루트 제외) |
| 박스 바깥, 그 수직 축 | before / after |

표시도 갈랐다 — **child 는 노드 전체를 초록 테두리로 감싸고**(변에
붙은 얇은 바만으로는 형제 바와 구분되지 않았다), 자식이 자라는 변의
바를 함께 그려 어느 쪽에 붙을지 보여 준다. 형제·부모는 기존처럼 변
바 하나다.

> **형제로 넣으려면 박스 바깥으로** 나가야 한다(마진 30px). 안쪽은
> 어디를 지나든 하위다 — 검증 e2e101, 회귀 e2e95 [1]은 드롭 지점을
> 바깥 마진으로 옮겼다.

## 편집창이 내용만큼 자란다 (2026-08-09 사용자 요청)

> "노드의 내용을 여러 줄 편집하기에는 노드 기본 줄이 2줄뿐이라 불편하다.
> 입력이 끝나고 다시 열면 현재 줄 수만큼 보이니, **처음부터 편집 내용이
> 늘어나면 줄 수가 늘어나도록** 해 달라." → 이어서 "편집하다 보면
> **마지막 줄이 2/3만 보이는** 경우가 있다."

예전 편집 오버레이는 높이가 **노드 박스에 못 박혀** 있었다
(`height: ovr.height` + `overflow: hidden`). 줄이 늘면 내용이 박스를
넘쳐 잘렸고, 박스 높이가 줄 높이의 배수가 아니라 **반 줄에서 끊겼다** —
이것이 "마지막 줄 2/3" 의 정체다.

지금은 입력할 때마다 **저장 후 노드가 될 크기**를 같은 측정기로 구해
편집창을 그만큼 키운다.

```tsx
const liveH = sizeNodeForText(draftText, n.depth, {
  ...nodeSizingOpts(n),
  manualW: n.w,          // 폭 고정 → 같은 자리에서 줄바꿈
}).h;
// 오버레이 높이 = max(노드 높이, liveH * 줌배율)
```

- **레이아웃이 쓰는 그 함수**(`sizeNodeForText`)를 그대로 쓴다 — 편집 중
  보이던 모양이 저장 후에도 그대로다. 따로 계산하면 둘이 어긋난다.
- 폭은 `manualW: n.w` 로 고정한다. 폭까지 변하면 타이핑 중에 글자가
  이리저리 재배치되어 읽기 어렵다.
- 줄을 지우면 다시 줄어든다.
- 노드 박스보다 커진 부분은 **뒤에 SVG 도형이 없으므로**, 오버레이가
  노드와 같은 채움·테두리·모서리로 배경을 이어 그린다(도형 '없음'이면
  배경도 없다). 그래서 노드가 자란 것처럼 보인다.

## 겹치는 레이어 순서 (2026-08-09)

> "노드 내용 편집 중에 코드 블록 아이콘을 클릭하면 기존 노드 편집창이
> 창을 덮고 있다."

노드 편집 오버레이는 **문서 최상위 HTML**(`zIndex: 1000`)이다 — 줌/팬이
걸린 `foreignObject` 안 textarea 는 Chromium 이 선택 좌표를 잘못 계산해
선택이 새기 때문이다. 그런데 코드 블록 창은 `zIndex: 200` 이라 **편집
오버레이보다 아래**였다. 편집 중이던 노드의 글자와 서식 툴바가 창 위로
비쳐 화면이 겹쳐 보였다.

- 코드 블록 창을 **`zIndex: 1200`** 으로 올렸다 — 편집 중에 열리는 유일한
  모달이므로 앱에서 가장 위다.
- 창이 떠 있는 동안에는 **서식 툴바(B/I/S/U/H)를 감춘다.** 툴바는 노드
  위(창 밖)에 떠 있어 창을 가리고, 그때는 쓸 수도 없다.

**규칙**: 새 오버레이를 만들 때는 이 순서를 먼저 확인한다 — 다이얼로그
220~240 < 노드 편집 오버레이 1000 < 편집 중 모달 1200. 모달은 자기가
덮어야 할 것보다 반드시 위여야 하고, 모달이 뜨면 그 위에 떠 있던
부속 UI(툴바·팝오버)는 감춘다.
