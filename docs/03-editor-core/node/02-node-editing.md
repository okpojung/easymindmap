...
같이 **선택 노드의 하위 노드를 새로 만들고 거기에 클립보드 내용을
넣는다**. 만든 하위 노드가 바로 선택된다.

| 클립보드 내용 | 하위 노드에 들어가는 것 |
| --- | --- |
| 한 줄 텍스트 | 노드 텍스트 = 그 줄 (80자 초과 시 말줄임) |
| 여러 줄 텍스트 | 노드 텍스트 = 첫 줄, **전체 텍스트는 문단 노트**로 보관 |
| 웹 기사 등 서식 HTML | 노드 텍스트 = 첫 줄, 본문·사진은 **문단 노트에 서식·원문 위치째** 보관 (`sanitizeRichHtml`) |
| 이미지 파일 (스크린샷 등) | 노드 텍스트 = "이미지", 사진을 노드 안에 표시 |

- 위의 "단일 노드 content로 저장" 원칙은 유지된다 — 여러 줄이어도
  노드를 여러 개로 분해하지 않고 하위 노드 1개 + 문단 노트로 담는다.
- 입력창(노트·검색 등)에 포커스가 있을 때는 각 입력창의 붙여넣기가
  우선한다. 노드 텍스트 **편집 중** 붙여넣기는 04-node-content.md
  (기사 = 문단 노트, 사진만 = 노드 사진) 참조.
- 검증: E2E e2e39 [A]~[D].
