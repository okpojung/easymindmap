...
- **접힌 노드**: 숨은 서브트리를 알리기 위해 +(에디터) / +N(뷰어) 배지를
  항상 표시.
- **토글 위치 = 자식이 실제로 배치된 방향**: 펼쳐진 노드는 배치된 자식
  좌표의 평균 방향(아래/위/왼쪽/오른쪽)으로 계산한다 — 트리·진행트리의
  3레벨 이하에서 자식이 아래로 자라는데 토글이 오른쪽에 붙던 문제 수정.
  접힌 노드는 side 폴백. **에디터(CollapseControl)와 HTML 내보내기
  뷰어(.mm-toggle) 동일 규칙.**
