...
공간 감각과 일치시키기 위함 (예: 방사형 왼쪽 노드는 자식이 왼쪽으로 자라므로
왼쪽 +가 자식 추가여야 한다).

| node.side | 자식 | 부모 | 형제(앞) | 형제(뒤) |
|---|---|---|---|---|
| right (기본) | ➡ 우 | ⬅ 좌 | ⬆ 상 | ⬇ 하 |
| left | ⬅ 좌 | ➡ 우 | ⬆ 상 | ⬇ 하 |
| down (트리·아래) | ⬇ 하 | ⬆ 상 | ⬅ 좌 | ➡ 우 |

Root: 자식 추가만 — 자식이 자라는 방향에 표시. **방사형·양쪽**이면 좌·우
양쪽에 + 가 표시되고, 누른 쪽으로 새 브랜치가 배치된다(setBranchSide).
키보드 단축키(Space=자식 등)는 레이아웃과 무관하게 동일.

또한 방사형·양쪽에서 노드를 드래그해 루트(또는 반대쪽 빈 공간)에 놓으면
좌/우가 전환된다 (레벨 용어: domain-models.md §0 — 중심 주제가 1레벨):
- **2레벨(depth 1) 브랜치**: side만 바뀐다 (서브트리 유지).
- **3레벨(depth 2) 이하 노드**: 반대쪽에 붙을 부모가 없으므로 그쪽의
  **새 2레벨 브랜치**가 된다.
- 반대쪽의 특정 노드 위에 놓으면 그 노드의 자식/형제로 붙는다(기존
  드롭 규칙 그대로). **2레벨 브랜치의 상/하(형제) 드롭존에 놓으면
  새 형제는 그 대상 브랜치의 side를 따라간다** — side를 갱신하지
  않으면 배열 순서만 바뀌고 반대쪽에 그려져 "이동이 안 된 것"처럼
  보인다 (2026-07 수정, `moveNodeRelative` sibling 경로).
- 좌/우 판정 기준은 **루트 박스의 가로 중심**이다. 드래그 중 반대쪽 빈
  공간 위에 있으면 고스트 아래에 **"⬅ 놓으면 왼쪽으로 이동"** 안내
  배지가 떠서, 어디에 놓아야 전환되는지 즉시 알 수 있다 (안내가 없으면
  같은 쪽 = 놓아도 변화 없음).
...
### 노트 종류별 개별 인디케이터 (MVS 구현 — 2026-07)

노트 인디케이터(📝 단일)를 **종류별 배지**로 분리했다. 노드에 있는 노트
종류마다 인디케이터가 하나씩 표시되고, 클릭하면 그 종류의 노트만 담은
읽기 전용 뷰어 창이 열린다.
...
- 규격 정의: `nodeContent.ts`의 `NOTE_KIND_META` — 에디터 글리프
  (`NoteTypeGlyph`)와 HTML 내보내기 뷰어(`drawMarkerGlyph`)가 공유.
- 노드 폭 예약(`contentIndicatorCount`)도 노트 종류 수만큼 계산.
- 같은 종류가 여러 개(체크리스트)면 배지 위에 개수 표시.
- 폐기 타입(warning/tip)은 문단으로 집계 (하위호환).

### 첨부파일(클립) 인디케이터 배지화 — 다크 모드 시인성 (MVS 구현 — 2026-07)

첨부파일 인디케이터는 원래 진갈색 클립 선(`#4A3B28` stroke)만 그렸는데,
다크 모드의 어두운 노드 배경에서 거의 보이지 않았다. 노트 인디케이터
(NoteTypeGlyph)와 같은 방식의 **색상 배지**로 변경:

- **보라 배지(#7C3AED, 둥근 사각) + 흰 클립** — 라이트·다크 모두에서
  또렷하다. 다른 배지 색과 겹치지 않는다 (문단 회색 `#64748B` · 코드
  주황 `#B45309` · 표 파랑 `#1D4ED8` · 체크 초록 `#15803D` · 링크
  지구본 파랑+금색).
- 에디터 `IndicatorGlyph.tsx`(file 분기)와 HTML 내보내기 뷰어
  `exportHtml.ts drawMarkerGlyph`가 동일한 좌표·색으로 그린다
  (수정 시 양쪽을 함께 바꿀 것).
- 검증: E2E e2e39 [F][G] — 에디터(라이트·다크)와 뷰어에서
  `rect[fill=#7C3AED]` + 흰 클립 path 확인.

### 노트 뷰어 창 조작 (MVS 구현 — 2026-07)

에디터의 노트 뷰어 팝업과 HTML 내보내기 뷰어의 상세 패널 모두:

- **이동**: 제목줄(종류 배지색 + 종류 라벨 + 노드 제목) 드래그
- **크기 조절**: 우하단 모서리 드래그 (CSS `resize: both`, 최소 220×120)
- 기본 위치·크기(우측 상단 도킹, 300px/62%)는 유지 — 조작 전까지 동일.
  [서버 연결 예정] 크기 기본값은 system_settings / 사용자 설정으로 이관
  (기존 계획과 동일, db-schema §향후 관리 테이블 참조).

---

## MVS 구현 — 접기/펼치기 토글 동작 (2026-07 갱신)

### 토글 위치 — 레이아웃 종류로 결정 (2026-07 재갱신)

토글 위치는 **레이아웃 종류로 결정론적으로** 정한다. 이전의 "자식 좌표
평균 방향" 방식은 같은 레이아웃에서도 자식 수·서브트리 높이에 따라
노드마다 하단/오른쪽이 뒤섞이는 문제가 있었다 (트리·오른쪽에서 자식이
적은 노드만 토글이 오른쪽에 붙던 현상).

| 실효 레이아웃(자식 배치 기준) | 토글 위치 |
|---|---|
| 트리·오른쪽 / 트리·아래 / 진행트리 | **하단** |
| 계층형·오른쪽 / 방사형·오른쪽 / 자유배치 | 오른쪽 |
| 계층형·왼쪽 / 방사형·왼쪽 | 왼쪽 |
| 방사형·양쪽 / 타임라인 (방향이 데이터에 달림) | 자식 좌표 평균 (기존 방식 유지) |

에디터(Canvas — effByNode 오버라이드 체인)와 HTML 뷰어(drawNode —
node._eff)가 같은 표를 따른다.

### 모두 접기 / 모두 펼치기 (2026-07)

- **에디터**: 우상단 플로팅 툴바 '보기' 그룹에 +(모두 펼치기) /
  −(모두 접기) 버튼 — HTML 뷰어의 +/− 아이콘과 동일 동작. 모두 접기 =
  자식이 있는 모든 노드(2레벨 이하 전부)를 접어 루트+2레벨만 남긴다.
  실행 후 화면: 선택이 없으면(맵 전체) 맵 전체 맞추기, **선택이 있으면
  (그 가지만 펼치고/접고) 고른 노드를 100% 로 화면 중앙에**
  (`requestCenterNode(id, 100)`, 2026-09-21 사용자 보고 "노드를 선택하고
  모두 펼치기 하면 100% 로 보여야 하는데 전체 맵이 보이도록 아주 작아진다"
  — 검증 e2e297). (documentStore.collapseAll / expandAll /
  expandSubtree / collapseSubtree — 실행 취소 가능)
- **HTML 뷰어**: 접기/펴기를 한 번이라도 조작하면 **재배치 모드**로
  전환된다 — 에디터가 계산한 노드 크기·글꼴·줄바꿈은 그대로 유지한 채
  위치만 다시 계산(layoutBlock+arrange)해서, 에디터처럼 접으면 간격이
  줄고 펴면 다시 늘어난다. (이전에는 표시/숨김만 해서 접어도 간격이
  그대로였다. 최초 표시는 여전히 에디터 좌표 그대로 — assignFixed)
- 검증: E2E e2e45 — 에디터/뷰어 토글 전부 하단(트리·오른쪽), 모두
  접기 96→9노드·뷰어 전체 높이 6194→430(재배치), 개별 접기·펼치기
  재배치, 복원.

- **펼쳐진 노드**: 토글은 기본 숨김. 노드(또는 토글 자리)에 마우스를
  올리면 **곧바로 접기(−) 아이콘**이 나타나고 클릭 한 번으로 접힌다.
  (예전의 "작은 점(●) → 점에 다시 호버해야 (−)" 2단계는 클릭까지 두 번
  조준해야 해서 제거 — 맵의 깔끔함은 유지: 호버 전에는 아무것도 없음)
- **접힌 노드**: 숨은 서브트리를 알리기 위해 배지(칩)를 항상 표시.
- **토글 위치**: 위 "토글 위치 — 레이아웃 종류로 결정" 표를 따른다.
  **에디터(CollapseControl)와 HTML 내보내기 뷰어(.mm-toggle) 동일 규칙.**

### 접힘 칩 = 숨은 노드 수 배지 (2026-07)

- 접힌 노드의 칩에는 **숨은 하위 노드 수(N)** 를 표시한다 —
  **에디터·HTML 뷰어 동일** (이전에는 뷰어만 표시, 에디터는 + 아이콘).
  툴팁: `펼치기 — 숨은 노드 N개`.
- **숫자 색은 칩 밝기에 맞춘다** (2026-09-21 사용자 보고 "다크모드에서 접기
  숫자 시인성이 떨어진다"): `readableTextOn(t.primary)` — 라이트 primary
  `#D97706` 위엔 흰 글자, 다크 primary `#F59E0B`(밝은 주황) 위엔 어두운
  글자. 검증 e2e296 ③-b.
- 칩 크기는 자릿수에 맞춰 커진다: r = 8.5(1자리) / 10.5(2자리) /
  13(3자리 이상).
  - 에디터: `Canvas.tsx` — `descCount` useMemo(전체 자손 수 맵) →
    `CollapseControl`의 `count` prop.
  - 뷰어: `exportHtml.ts` — `countDescendants(node)`.
- **칩 자리 예약**: 뷰어 재배치(layoutBlock)에서 접힌 노드는 칩이
  놓일 방향으로 블록 크기를 넓혀 둔다 — 하단 칩(트리·진행트리)은
  `_bh += 26`, 좌/우 칩은 `_bw += 28`. 모두 접기 시 칩이 이웃 노드에
  깔려 안 보이던 문제의 근본 수정.
- **칩 최상위 레이어**: 뷰어는 접힘 칩을 노드 뒤에 그리지 않도록 전용
  `g.mm-chip-layer`(마지막 자식)에 그린다 — 어떤 노드 박스도 칩을
  가리지 못한다. (펼침 − 토글은 호버 시에만 보이므로 노드 g에 유지)
- 검증: E2E e2e46(트리·오른쪽 — 칩 숫자·겹침 0·elementFromPoint
  최상위·에디터 칩 숫자), e2e46b(방사형·오른쪽 / 트리·아래 동일 검증).
