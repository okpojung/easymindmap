...
1. **노드 텍스트 편집창(더블클릭)에 기사를 붙여넣는 경로**:
   ...
   (`node.image` — 항상 **텍스트 아래** 표시)으로 빼냈다. 즉 이 경로는
   구조적으로 사진이 항상 마지막이었다.
   → 수정: 편집 중 붙여넣은 HTML에 **이미지 + 40자 이상 본문**이 같이
   있으면(=기사) 노드 사진으로 빼지 않고 **문단 노트**에
   `sanitizeRichHtml` 결과(사진 원위치 포함)를 저장한다. 사진만 복사한
   경우는 기존처럼 노드 사진.
2. **숨은 중복 이미지**: lazy-load 구현에 따라 같은 주소의 이미지가
   숨김 상태로 한 번 더 복사되어(로드된 본 이미지 + 숨은 자리표시자)
   뒤쪽에 사진이 또 나타난 것처럼 보였다.
   → 수정: `sanitizeRichHtml`이 한 번의 붙여넣기 안에서 **같은 주소의
   이미지는 첫 위치만** 살린다 (기사에서 같은 사진을 두 번 싣는 일은
   없어 안전).

노드를 "선택만" 하고 붙여넣는 경우는 ThinkWise식 하위 노드 생성
(02-node-editing.md §ThinkWise식 붙여넣기)으로 처리되며, 기사 본문은
하위 노드의 문단 노트에 서식·사진 원위치째 들어간다.
...
- 검증: E2E(e2e39) — 네이버 기사 마크업 구조(요약 strong ·
  ...
  그대로 재현해 사진1 → 문단1 → 문단2 → 사진2 → 문단3 순서 유지,
  중복 1장 정리, 자리표시자 복원을 확인.
