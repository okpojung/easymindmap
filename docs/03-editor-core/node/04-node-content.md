...
### 18. 체크리스트 — 연속 입력과 토글 (2026-08-09)

**연속 입력.** 체크 항목을 적고 **Enter** 또는 **Shift+Enter** 를 누르면
**바로 아래에 다음 체크 항목**이 생기고 커서가 그리로 넘어간다.
`+체크` 를 매번 누르지 않아도 목록을 쭉 적을 수 있다.

- 새 블록은 맨 뒤가 아니라 **누른 항목 바로 다음**에 들어간다
  (`addNoteBlock(nodeId, type, text, extra, afterBlockId)` 의 5번째 인자).
- **내용이 빈 항목에서 Enter 는 아무 일도 하지 않는다** — 빈 항목이
  쌓이는 것을 막는다. 한글 조합 중(`isComposing`)에도 만들지 않는다.
- 새로 생긴 항목은 `autoFocus` 로 바로 입력받는다.

**토글할 수 있는 자리.** 체크는 세 군데에서 끄고 켤 수 있다 (모두 같은
맵 데이터를 고친다 — 한 곳에서 바꾸면 나머지에도 바로 보인다).

| 자리 | 대상 | 비고 |
|---|---|---|
| 노트·태그 탭 | 노트 체크리스트 블록 | `data-testid="note-check-toggle"` |
| 노트 뷰어 팝업 (노드의 ✓ 인디케이터) | 노트 체크리스트 블록 | 글자 편집은 여전히 탭에서만 |
| 맵/아웃라인/칸반의 노드 글자 | 노드 본문의 `- [ ]` 줄 | 원문 `[ ]`↔`[x]` 를 고친다 |

**클릭 판정은 글리프보다 넓다.** 13px 사각형만 판정하면 조금만 빗나가도
노드가 선택돼 "체크가 안 눌린다"로 보인다 — 사방 6px 을 넓혔다
(coding-conventions.md §5-1-5).

**표시 규칙은 네 자리가 함께 지킨다** (2026-08-09 2차 보고 — 자리마다
달라서 "엉망"이었다). coding-conventions.md **§5-1-6** 에 적어 뒀다:
완료해도 **취소선을 긋지 않는다** · ✓ 는 상자의 60% 이상 · 노드 글자를
제목으로 쓸 때는 `flattenNodeText()` 로 마커를 접는다 · 머리글에 늘 켜진
☑ 를 두지 않는다 · 종류 라벨을 제목과 겹치지 않는다.

내보낸 HTML 뷰어에서도 체크를 누를 수 있다 — 다만 그 결과는 **파일이
아니라 그 브라우저에** 저장된다 (import-export/20-export.md 참조).
