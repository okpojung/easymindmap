...
정리 — 사진 위치가 "원문 그대로" 보존되는 경로는 **노트 문단 블록에
직접 붙여넣기**(노트·태그 탭 문단 textarea)다. 노드에 붙여넣는 경로
(선택+붙여넣기 = ThinkWise식 하위 노드 생성, 편집 중 붙여넣기)는
"텍스트 전체 + 사진 1장(텍스트 아래)"으로 **모두 노드에** 들어간다
(02-node-editing.md §ThinkWise식 붙여넣기).
