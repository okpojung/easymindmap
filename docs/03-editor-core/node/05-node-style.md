...
| `colorKey` 가지 색 계열 — 명시 색이 없는 노드의 채움/테두리/글자색은 여기서 나온다(`resolveNodeColors`). 원본이 중심주제(`'root'`)면 뜨지 않고, 대상이 중심주제면 덮지 않는다 | 크기 `sizeW/H`(글 길이에 딸린 것)·배치 `layoutType`·`side` |
...
검증: 단위 `stylePainter.test.ts` 17항목 · 브라우저 e2e281 23항목.
