...
- `shapeType`: **12종** (없음/둥근/사각/캡슐/원/육각/다이아/평행/화살◀/화살▶/원통/별) — §16.1·§18·**§19** 참조. 모서리 둥글기는 `shapeType`으로 표현한다 (`borderRadius` 키 없음).
- `colorKey`: 테마 색상 키 — 노드의 `style` 밖 별도 필드.

> **글자색 우선순위 (2026-07-31)**: `searchHit 고정색 > style.textColor
> (사용자 지정) > 하이라이트 고정색(#1F1B16) > 채움 지정 시
> readableTextOn > 테마색`. 하이라이트(전체·부분 ==띠==)가 켜진 노드에서
> 지정 글자색이 고정 진한색으로 덮여 "라이트에서 빨간 글자가 안 나오던"
...
> 색(이전 템플릿의 레벨 스타일)만 새 템플릿 스타일로 교체한다 —
> 칸반에서 칠한 카드 색이 템플릿 '적용'으로 초기화되던 문제의 수정.
> "첫 노드=대표" 방식은 처음 칠한 노드 자신이 대표가 되는 함정이 있어
> 최빈값(다수결)을 쓴다 (e2e70).
>
...
- 주입 경로는 레벨별 폰트와 동일: Canvas가 레이아웃 계산 직전
  ...
  함께 적용된다.
- 선택지: 스타일 탭과 같은 **12종** (**없음**/둥근/사각/캡슐/원/육각/
  다이아/평행/화살◀/화살▶/원통/별).

### 16.2 HTML 뷰어 도형 파리티 (2026-08-08 — 사용자 보고 수정)

보고: *"에디터에서는 도형 없음이 되는데, HTML 뷰어에서는 도형이 있다."*

**원인은 뷰어가 아니라 내보내기에 있었다 — 두 가지다.**

1. **`shapeType`을 안 실었다.** 내보내기는 노드 `style`을 화이트리스트
   (`strike`/`underline`/`highlight`/`fillColor`/`borderColor`/`textColor`)
   로 추려 싣는데 `shapeType`이 빠져 있었다. 뷰어에는 `noShape` 분기
   코드가 있었지만 **값이 온 적이 없어 늘 false**였다. 화이트리스트를
   통과시키는 **바깥 조건에도** 넣어야 한다 — 조건에서 빠지면 도형만
   지정한 노드는 `style` 자체가 `undefined`가 되어 다시 사라진다.
2. **맵 설정을 뷰어가 모른다.** 뷰어는 `levelShapes`를 받지 않으므로,
   `textAlign`과 **같은 방식으로 실효 도형을 구워서** 보낸다:
   `node.style.shapeType ?? levelShape(depth)`. 이때 내보내기가
   `setLevelShapeConfig(map.settings?.levelShapes)`를 **먼저 주입해야**
   `levelShape(depth)`가 그 맵의 설정을 안다.

'없음'이면 뷰어는 도형을 그리지 않고 **글자만** 놓는다. 글자색은 두
모드 모두 안전한 본문색(`SKIN.fam.l2.text`)을 쓴다 — 흰 테두리 사각형
으로 우회하면 다크 모드에서 흰 박스가 보인다. 검색 강조일 때는 에디터와
동일하게 박스를 되살린다. (→ PR #232)

**함께 드러난 것 — 뷰어는 12종을 전부 사각형으로 그리고 있었다.** 값이
안 왔으니 분기할 일도 없어 가려져 있었을 뿐이다. 뷰어에 에디터
`NodeShape`와 **같은 좌표 공식**으로 12종을 모두 이식했다 (원/다이아/
육각/평행/화살◀▶/원통/별 + 사각형 계열의 모서리 반경). 뷰어의 도형
요소에는 `class="mm-box"`를 붙인다 — 코드·표 패널 배경도 같은 그룹의
`rect`라, 표식이 없으면 도형을 셀 때 섞인다. 사각형 계열의 기본 반경은
뷰어 기존 값(루트 13 / 그 외 9)을 유지한다.

> 원래는 다른 세션이 올린 PR #233이 같은 내용을 담았으나, 절반이
> #232와 겹쳐 main과 충돌했다. 겹치지 않는 **12종 파리티**만 다시
> 정리해 올렸다.

**검증: E2E e2e131** — 내보내기 메뉴를 실제로 눌러 받은 HTML을
`file://`로 열어 그려진 도형을 센다(13항목). 문자열 검사로는 이 버그를
못 잡는다 — 앞선 e2e129 ⑧이 `noShape`이 코드에 있나만 보고 통과시켰다.
회귀 실증도 해 뒀다: 내보내기에서 `shapeType` 한 줄을 빼면 8항목이
FAIL 한다. (test-catalog.md e2e131 — 함정 4가지 포함)
...
**단 '없음'(§19)만은 뷰어도 따른다** — "도형을 없앴는데 내보내면 박스가
생긴다"는 결과가 가장 눈에 띄기 때문이다.

### 19. 도형 '없음' — 글자만 (2026-08-08 사용자 요청)

> *"노드에 도형 없이 텍스트만 입력하는 경우도 있다"*

`ShapeType`에 **`'none'`** 추가 (총 12종). 채움·테두리를 **아무것도
그리지 않고 글자만** 놓는다. 노드 자체는 그대로라 **선택·연결선·
＋버튼·접기·드래그 이동은 전부 그대로** 동작한다 — 박스만 없다.

구현에서 걸리는 곳이 두 군데였다. 둘 다 "도형을 안 그린다"의 부작용이다.

| 무엇 | 왜 문제인가 | 어떻게 했나 |
|---|---|---|
| **글자색** | 도형이 없으면 글자가 **캔버스 배경 위**에 놓인다. 채움색에서 유도한 색(`readableTextOn`)이나 도형 위에 맞춘 팔레트 색(`colors.text`)을 그대로 쓰면 **다크 모드에서 배경에 묻힌다** | 사용자가 글자색을 직접 지정하지 않았으면 **테마 본문색**(`t.text` / 뷰어 `SKIN.fam.l2.text`)을 쓴다 — 라이트·다크 모두 안전 |
| **검색 강조** | 강조가 *노란 채움 + 붉은 테두리*라, 박스를 안 그리면 **강조가 통째로 안 보인다** | 검색 결과일 때만 `'none'`을 무시하고 둥근 박스를 되살린다 |

* 맵 설정의 **레벨별 기본 도형**에도 `없음 (글자만)`이 있다 — 예: 3레벨
  이하를 전부 글자만으로.
* 뷰어(HTML 내보내기)도 같은 규칙을 따른다(위 두 항목 포함).

검증: e2e129 [0]~[8].


### 20. 스타일 복사 — 붓 (2026-09-19 사용자 요청)

> *"하나의 노드를 선택 후 스타일복사 아이콘 버튼을 누르면 커서 옆에 붓이
> 표시되고, 그 상태에서 노드를 선택하면 내용을 뺀 스타일(도형·배경색·
> 글자맞춤·글자색 등)이 복사된다. 하나씩 눌러도 되고 드래그로 여러 노드를
> 골라도 된다. ESC 나 배경 클릭으로 해제된다."* (ThinkWise 의 서식 복사)

**흐름** — 우상단 플로팅 툴바 **노드** 그룹의 붓 버튼(`style-copy`).

| 단계 | 동작 |
|---|---|
| ① 노드 하나를 고르고 붓 버튼 | 그 노드의 겉모습을 **그 순간 떠서** 붓에 담는다 (`snapshotNodeStyle`). 버튼이 켜진 색으로 바뀌고, 캔버스 안에서 **커서 오른쪽 아래에 "🖌 스타일 복사" 붓**이 따라다닌다 (`style-brush-cursor`) |
| ② 노드 클릭 | 그 노드에 칠하고 **선택**한다. 붓 모드는 **그대로** — 다음 노드도 이어서 칠한다 |
| ② 빈 곳을 **드래그**(러버밴드) | 사각형에 걸린 노드 **전부**에 한 번에 칠한다 = **undo 한 단계** |
| ③ 해제 | `ESC`(선택은 그대로 둔다) · **빈 캔버스 클릭**(선택도 해제) · 붓 버튼 재클릭. 붓 아이콘이 사라진다 |

**무엇을 옮기나** — `apps/frontend/src/editor/canvas/stylePainter.ts`.

| 옮긴다 | 옮기지 않는다 |
|---|---|
| `style` 전부 — 도형·채움/테두리/글자색·테두리 굵기/선 종류·굵게/기울임/취소선/밑줄/형광펜. **통째로 바꾼다**(합치지 않는다): 원본에 없는 항목은 대상에서도 지워져 원본과 **같은 겉모습**이 된다 | `fontSize` — 스타일 탭에 조절이 없고 레벨 기본값(`inheritStyle`)으로만 정해지므로, 다른 레벨로 옮기면 그 레벨답지 않은 크기가 된다. 대상 것을 지킨다 |
| `textAlign` (원본에 없으면 대상에서도 지운다 = 레벨 기본) | 글(`text`)·노트·링크·첨부·사진·태그·**아이콘**·체크·잠금·접힘 — "내용" |
| `colorKey` 가지 색 계열 — 명시 색이 없는 노드의 채움/테두리/글자색은 여기서 나온다(`resolveNodeColors`). 원본이 중심주제(`'root'`)면 뜨지 않고, 대상이 중심주제면 덮지 않는다. **★ 원본에 colorKey 가 없으면 레벨 기본 계열을 채워 뜬다** — 1레벨 `l1A`, 2레벨 이하 `l2`(흰 바탕). 기본 흰색은 명시 색이 아니라 *colorKey 부재 + 깊이*에서 나오므로, 그대로 두면 흰 노드를 떠서 색 가지에 칠해도 가지 색이 남았다 (2026-09-19 사용자 보고 "배경색이 흰색은 적용이 안 된다" → 수정). 그래서 `snapshotNodeStyle(node, depth)` 에 깊이를 준다 | 크기 `sizeW/H`(글 길이에 딸린 것)·배치 `layoutType`·`side` |

* 붓에 든 값은 버튼을 누른 **순간**의 것 — 그 뒤 원본을 바꿔도 붓은 그대로.
* 원본이 기본 모습(스타일 없음)이면 "빈 붓"이다 — 칠하면 대상이 **기본
  모습으로 돌아간다**(스타일 지우개로 쓸 수 있다).
* 문서 변경은 `documentStore.applyStyleSnapshot(ids, snap)` 한 번 —
  클릭 한 번·러버밴드 한 번이 각각 undo 한 단계.
* 칸반 화면에는 붓 버튼이 없다(카드 클릭 흐름이 다르다). 아웃라인 화면도
  없다 — 캔버스 전용.

검증: 단위 `stylePainter.test.ts` 25항목 · 브라우저 e2e281 27항목(흰 노드 → 색 가지가 정말 `#FFFFFF` 로 그려지는 것 포함).
