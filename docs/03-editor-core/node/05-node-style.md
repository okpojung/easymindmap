...
1. **`shapeType` 을 안 실었다.** 내보내기는 노드 `style` 을 화이트리스트
   ...
   코드가 있었지만 **값이 온 적이 없어 늘 false** 였다.
   화이트리스트를 통과시키는 **바깥 조건에도** 넣어야 한다 — 조건에서
   빠지면 도형만 지정한 노드는 `style` 자체가 `undefined` 가 된다.
2. **맵 설정을 뷰어가 모른다.** 뷰어는 `levelShapes` 를 받지 않는다.
   그래서 `textAlign` 과 **같은 방식으로 실효 도형을 구워서** 보낸다:
   `node.style.shapeType ?? levelShape(depth)`. 이때 내보내기가
   `setLevelShapeConfig(map.settings?.levelShapes)` 를 **먼저 먹여야**
   `levelShape(depth)` 가 그 맵의 설정을 안다.

**함께 드러난 것 — 뷰어는 모든 도형을 사각형으로 그리고 있었다.** 값이
안 왔으니 분기할 일도 없어 드러나지 않았을 뿐이다. 뷰어에 에디터
`NodeShape` 와 **같은 좌표 공식**으로 12종을 모두 이식했다. 노드 도형
요소에는 `class="mm-box"` 를 붙인다 — 코드·표 패널 배경도 같은 그룹의
`rect` 라, 표식이 없으면 "도형이 남았다"고 잘못 읽는다.

> **검증 교훈**: 앞선 테스트(e2e129 [8])는 내보낸 HTML **문자열**에
> `noShape` 이 있나만 봐서 이 버그를 통과시켰다. "코드가 있다"와
> "그 코드가 동작한다"는 다르다. e2e131 은 내보낸 HTML 을 **실제
> 브라우저로 열어** 도형을 센다.
