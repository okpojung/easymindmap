...
**정책 2 — 리스트(List) 처리 (Smart Hybrid Mode):**  
Markdown 리스트는 그 성격에 따라 변환 방식이 달라진다.
- **순서 없는 목록 (`-`, `*`, `+`)**: 기본적으로 자식 Node로 분리하지 않고, 해당 Heading Node의 `nodes.text` 내부 콘텐츠로 유지한다.
- **순서 있는 목록 (`1.`, `2.`)**: 상위 Node의 자식 Node로 분리하여 절차나 단계를 시각적으로 구조화한다.
- **체크박스 목록 (`- [ ]`, `- [x]`)**: 상위 Node의 자식 TODO Node로 분리하여 실행 항목을 관리한다.
단, Import 시 `document` 모드를 명시적으로 선택한 경우에만 모든 리스트를 내부 콘텐츠로 강제 유지한다.
...
### 2.5 리스트 정책 (Smart Hybrid Mode)

- **순서 없는 목록 (Unordered List)**: child node로 자동 분해하지 않고 node 내부 콘텐츠로 유지한다. (부연 설명, 단순 나열)
- **순서 있는 목록 (Ordered List)**: 상위 노드의 자식 노드(Child Node)로 분리한다. (절차, 단계)
- **체크박스 목록 (Task List)**: 상위 노드의 자식 TODO 노드로 분리한다. (실행 항목)
...
- 문서형: Heading만 트리 구조로 사용하고, 문단/모든 리스트/코드는 node 내부 콘텐츠로 강제 유지
- 아웃라인형: Heading + 모든 List Item을 구조로 해석하여 자식 node 생성
- 하이브리드형(기본값): Heading 구조화 + 리스트 타입별 분리 적용 (순서없음=내용, 순서있음/체크박스=자식노드)
...
| Mode     | 설명                          |
| -------- | ----------------------------- |
| hybrid   | 리스트 타입별 분리 적용 (기본값) |
| document | 모든 리스트를 내부 콘텐츠로 유지 |
| outline  | 모든 리스트를 자식 노드로 구조화 |

---

### 4.4 기본 정책

- 기본 Import 모드는 `hybrid` (Smart Hybrid Mode)로 한다.
- 사용자가 명시적으로 선택한 경우에만 `document` 또는 `outline` 모드를 적용한다.
