...
**(나) 남은 위험 — §7.8 반영 후 상태 (2026-08-06)**
...
| **R4** | 자동저장으로 **지운** 내용을 되살리고 싶다 | 노드 삭제 후 시간이 지나 세션 undo 로 못 감 | **해결됨** — 개인 설정 간격(3·5·10분, 기본 5분)마다 히스토리 버전 | **최대 그 간격만큼**의 중간 상태 |
| **R5** | 로컬 첨부(`blob:`)가 세션 종료 후 깨짐 | 파일을 서버에 올리지 않고 링크만 잡은 맵 | **의도된 제약** (2026-08-05 사용자 결정) — 원본 파일은 사용자 PC 에 있다 | 맵 내 미리보기 링크만 |
| **R6** | 탭이 백그라운드에서 정지 | 모바일 등에서 앱 전환 | **해결됨** — `visibilitychange`(hidden) 에서 초안·자동저장을 **즉시** 보낸다 | 없음 |
...
**자동저장 지연의 상한**은 `MAX_WAIT_MS = 5000` 으로 **무제한 → 5초**다.
디바운스 1.5초는 그대로이며(손을 멈추면 1.5초), 5초는 **손을 안 멈출 때의
상한선**이다. 다만 §7.8 이후 **이 값들은 유실과 무관하다** — 서버 반영
속도와 트래픽을 조절하는 손잡이일 뿐이고, 안전은 로컬 초안이 맡는다.
