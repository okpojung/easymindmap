...
**(나) 남은 위험 — §7.7 반영 후 상태 (2026-08-05)**

| # | 위험 | 언제 일어나나 | 지금 상태 | 실제로 잃는 것 |
| --- | --- | --- | --- | --- |
| **R1** | 저장 직전에 브라우저·PC 가 강제 종료 | 크래시 · 전원 차단 · 강제 종료 (`beforeunload` 는 정상 종료에서만 뜬다) | **해결됨** — 1초마다 IndexedDB 초안, 다음 실행에서 "저장되지 않은 맵이 있습니다" | **마지막 1초 분량** |
| **R2** | 저장 실패가 계속됨 | 오프라인 · 서버 장애 · 쿼터 초과 | **해결됨** — 재시도 2회 후 배지 `error`, 편집분은 로컬 초안에 남는다 | 없음. 다만 **서버 반영은 사용자가 ☁ 저장을 눌러야** 한다 |
| **R3** | 편집권을 잃은 채 계속 편집 | 같은 맵을 **다른 창/기기**가 가져감 | **감지·안내까지** — 하트비트 `held=false` → 배지 `error` + 안내. 편집분은 로컬 초안에 남는다 | 서버 반영분 0. **'다른 이름으로 저장' 유도는 미구현** |
...
| **R6** | 탭이 백그라운드에서 정지 | 모바일 등에서 앱 전환 | **미구현** — 예약된 저장이 밀릴 수 있다. 로컬 초안도 같이 밀린다 | 정지 직전 몇 초 |

**남은 실제 노출은 두 줄로 요약된다.**

1. **마지막 1초** — 초안 기록 주기. 이보다 짧게 하려면 매 편집마다 IndexedDB 에 써야 하는데 큰 맵에서 부담이 된다(0.3초 안 → 1초로 정한 이유).
2. **R3·R6** — 각각 '다른 이름으로 저장' 유도와 `visibilitychange` 즉시 저장으로 없앨 수 있다. **아직 안 했다.**

**자동저장 지연의 상한**은 §7.7 의 `MAX_WAIT_MS = 5000` 으로 **무제한 → 5초**가 되었다. 디바운스 1.5초는 그대로이며(손을 멈추면 1.5초), 5초는 **손을 안 멈출 때의 상한선**이다.
