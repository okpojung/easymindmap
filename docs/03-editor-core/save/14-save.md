...
경우를 숨기지 않고 적는다 — 순위는 "실제로 일어날 확률"이다.

| # | 남은 위험 | 지금 상태 | 없애려면 |
| --- | --- | --- | --- |
| R1 | 디바운스 안에 브라우저/PC 가 강제 종료(크래시·전원) | **해결됨 (§7.7)** — 1초마다 IndexedDB 초안 + 다음 실행에서 복구. 남는 위험은 **마지막 1초** |
| R2 | 저장 실패(오프라인·서버 장애·쿼터 초과)가 이어질 때 | **대부분 해결됨 (§7.7)** — 로컬 초안이 남아 복구할 수 있다. 서버 반영은 여전히 손으로 ☁ 저장 |
| R3 | 편집권을 잃은 채(다른 창이 가져감) 계속 편집 | 하트비트가 잃음을 감지해 배지 `error` + 안내 (2026-08-05 추가). 그래도 그 사이 편집분은 서버에 못 간다 | '다른 이름으로 저장' 유도 + 로컬 초안 백업 |
| R4 | 자동저장으로 지운 내용(노드 삭제 등) | **해결됨 (§7.7)** — 개인 설정 간격(기본 5분)마다 히스토리 버전을 남긴다 |
| R5 | 로컬 첨부(`blob:`)가 있는 맵 | **의도된 제약** (2026-08-05 사용자 결정) — 원본 파일은 사용자 PC 에 있으므로 링크가 끊겨도 문제 없다고 본다 |
| R6 | 브라우저 탭이 백그라운드에서 정지(모바일 등) | 예약된 저장이 밀릴 수 있다 | `visibilitychange` 에서 즉시 저장 [미구현] |

> 위 표는 **§7.7 반영 후** 상태다. R1·R2·R4 를 막은 뒤 남은 것은
> **마지막 1초**(초안 기록 주기)와 R3·R6 뿐이다.

**지금 확실히 말할 수 있는 것**은 여기까지다 —
"**서버에 저장된 내용이 앱의 실수로 지워지거나 다른 문서로 덮어써지는
경로**"는 §7.1~§7.5 로 막았고 각각 재현 테스트가 있다. 반면 위 표는
**아직 서버에 못 간 편집**(마지막 몇 초·오프라인·미저장 새 맵)에 대한
것이고, R1·R2 를 실제로 없애려면 **로컬 초안 백업**이 필요하다.
