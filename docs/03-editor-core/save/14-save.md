...
| # | 남은 위험 | 지금 상태 | 없애려면 |
| --- | --- | --- | --- |
| R1 | **디바운스 1.5초 안에 브라우저/PC 가 강제 종료**(크래시·전원·강제 종료) | 경고조차 못 띄운다 (beforeunload 는 정상 종료에서만 뜬다) | **로컬 초안 백업**(localStorage 즉시 기록 + 시작 시 복구) [미구현] |
| R2 | 저장 실패(오프라인·서버 장애·쿼터 초과)가 이어질 때 | 1초·3초 재시도 후 배지 `error` + 창 닫기 경고. **사용자가 "나가기"를 누르면 유실** | 오프라인 큐 + 로컬 초안 백업 [미구현] |
| R3 | 편집권을 잃은 채(다른 창이 가져감) 계속 편집 | 하트비트가 잃음을 감지해 배지 `error` + 안내 (2026-08-05 추가). 그래도 그 사이 편집분은 서버에 못 간다 | '다른 이름으로 저장' 유도 + 로컬 초안 백업 |
| R4 | **자동저장으로 지운 내용**(노드 삭제 등) | 자동저장은 히스토리 버전을 남기지 않는다 — 세션 undo 말고는 복구 수단이 없다 (새로고침하면 끝) | 자동저장에도 **주기적 버전**(예: N분마다 1개) [미구현] |
| R5 | 로컬 첨부(`blob:`)가 있는 맵 | 세션이 끝나면 원본을 못 읽는다 — 다시 열면 첨부가 깨진다 (문서화됨) | 첨부를 항상 서버에 올린 뒤 참조 [일부 구현 — 10MB 초과분만 서버] |
