...
> **[NODE-15]** 태그 badge 표시 여부는 사용자 설정(`uiPreferences.showTagBadge`)으로 제어한다.
> OFF 시 badge UI 미렌더링. 태그 검색·필터 기능은 유지. 설정 → § 17 (Tag Badge 표시 ON/OFF 설정)
...
17\. Tag Badge 표시 ON/OFF 설정 (NODE-15)
================

### 17.1 배경

다수의 노드에 태그가 부착되면 badge UI가 맵 전체에 반복 표시되어 시각적 과부하가 발생할 수 있다.
특히 번역 인디케이터(🔤 등)와 함께 표시될 경우 노드 텍스트 영역이 과도하게 복잡해진다.

사용자 설정으로 tag badge 표시 여부를 전역 제어할 수 있다.

### 17.2 제어 정책

  설정 키: `uiPreferences.showTagBadge`  (docs/02-domain/map-model.md > UiPreferences)
  저장 위치: users.ui_preferences_json JSONB  (docs/02-domain/schema.sql)
  기본값: `true` (표시)

  | showTagBadge | 동작 |
  |---|---|
  | true (기본) | 노드 하단/우측에 tag badge 표시 |
  | false | badge UI 미렌더링. 태그 데이터 및 검색/필터 기능 유지 |

  * badge를 숨겨도 태그 데이터(node.tags)는 서버에 보존된다.
  * 태그 검색(`tag:AI`), Tag Explorer, Tag Filter 기능은 설정에 무관하게 동작한다.
  * **시각 표시만 OFF** — 태그 기능 자체를 비활성화하는 것이 아님.
...
function shouldShowTagBadge(
  ...
  if (!uiPrefs.showTagBadge) return false;  // [NODE-15] 사용자 설정 OFF
...
### 17.4 설정 UI 위치
  ...
  "표시 설정" 패널 (사이드바 또는 메뉴 > 설정 > 인디케이터) 내에
  번역 인디케이터 ON/OFF 토글과 함께 배치한다.
  ...
  * "태그 badge 표시" OFF 시 tooltip 문구:
    "태그 badge를 숨깁니다. 태그 검색과 필터 기능은 계속 사용 가능합니다."

### 17.5 연관 문서

  - docs/03-editor-core/node-indicator.md § 25 (태그 badge 표시 제어 로직)
  - docs/03-editor-core/node-indicator.md § 26 (설정 UI 와이어프레임)
  - docs/02-domain/map-model.md > UiPreferences 타입
  - docs/02-domain/schema.sql > users.ui_preferences_json
