...
1. **잠금 — 앱과 같은 잠금을 탄다.** `getDocument` 에 편집 세션 키
   (`mcp:<user>`)를 줘서 `map_edit_locks` 를 앱처럼 잡는다.
   - 사람이 앱에서 편집 중(하트비트 60초 안)이면 **거절** — "앱에서 맵을
     닫거나 1분 뒤에". 편집 중인 내용을 덮어쓰지 않기 위해서다(§2-2 가
     미뤄 둔 이유가 이것이었다).
   - 죽은 잠금(60초 넘게 조용)은 앱과 같은 규칙으로 넘겨받는다.
   - 협업맵은 앱도 잠그지 않으므로 잠그지 않는다. 권한(role)은
     MapsService 가 판정한다 — 읽기 권한이면 거절.
   - 성공하든 실패하든 **끝나면 잠금을 푼다**(`editRelease`). 조각이
     잘못됐으면 잠금을 잡기 **전에** 거절한다.
