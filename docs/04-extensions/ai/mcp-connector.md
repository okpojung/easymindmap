...
| `import_github_docs` | GitHub API + `POST /v1/maps` + `PUT …/document` | **GitHub 저장소의 문서 폴더(`docs/`)를 새 맵으로** — 폴더 트리 = 가지, 문서 = 노드(제목·GitHub 링크·마지막 커밋 시각 노트), 문서 안 `##`·`###` = 하위 노드, 표·문단·코드 = 노트. "OOO 저장소 문서를 emm 새 맵으로" — §9.14 (2026-09-21) |
| `update_map_from_github` | GitHub API + `GET` + `PUT …/document` | 그 맵을 저장소의 지금 상태에 맞춘다 — 사라진 문서 노드 삭제 · 새 문서 추가 · 바뀐 문서 노드 재생성(절은 제목으로 짝지어 id 유지). `node` 로 폴더·문서·**절 하나**까지 좁힌다. "OOO 맵을 업데이트 해줘" · "OOO 맵의 OOO 노드를 업데이트 해줘" · "현재 선택한 노드의 내용을 업데이트 해줘" — §9.14 |
...
- **삭제·계정 조작** (`DELETE /v1/maps/:id`, `DELETE /v1/account` …) —
  ...
  **둘째 예외 — `update_map_from_github`** (2026-09-21 사용자 결정, §9.14):
  GitHub 문서로 만든 맵을 저장소에 맞추려면 노드를 더하고·바꾸고·지워야
  한다. 경계는 셋이다 — ⑴ **이 도구가 스스로 만든 노드만** 손댄다(GitHub
  링크가 달린 문서·폴더 노드와 그 아래 생성 id 의 절 노드), 사용자가 손으로
  붙인 노드·노트는 남긴다 ⑵ 이 도구로 만들지 않은 맵(루트 노트에 `출처:`
  가 없다)은 거절한다 ⑶ 히스토리 버전으로 남아 앱에서 되돌린다.
...
> 이 표는 홈페이지 **`/ai`**(`apps/site/src/pages/Ai.tsx`)에도 손님 말로
> 옮겨 두었다(2026-09-15, e2e267) — 열쇠 수명·새면 어떻게 되나·끊는 곳까지
> 같은 내용이다. **고치면 두 곳을 같이 고친다.** 여기가 원본이다.
...
| 인자 | 뜻 |
|---|---|
| `parent` | 노드 **이름**(`"할 일"`) 또는 **경로**(`"2분기 > 협업"`). 비우거나 `root` 면 첫 중심주제 아래 새 최상위 가지. 맵에 중심주제가 여럿이면 **중심주제 이름**(`"중심주제2"`)이나 `"중심주제2 > 가지"` 로 그 중심을 고른다(2026-09-16) |
...
| 인자 | 뜻 |
|---|---|
| `nodes` | 체크박스가 **들어 있는 노드**의 이름/경로 목록. 찾는 규칙은 `append_to_map` 의 `parent` 와 같다(`findByPath`) — `"selected"` 는 앱에서 선택한 노드, `root` 는 첫 중심주제, 중심주제 이름을 적으면 그 중심(2026-09-16) |
...
### 9.14 GitHub 저장소 문서 → 맵 · 저장소 변경 반영 — `import_github_docs` · `update_map_from_github` (2026-09-21)

**목적**: 사용자 요청 — ⑴ *"github 저장소를 지정하면 문서 폴더(대부분 /doc)
아래 문서들을 새 맵으로"* — 폴더 트리를 그대로, 파일 제목은 노드 하나에
GitHub 주소를 링크로, 파일 안 목차 2레벨까지 노드로(중요 표·내용은 노트),
파일 노드에 최종 업데이트 시각을 노트로 ⑵ *"OOO 맵을 github 수정사항을
반영하여 업데이트"* — 노트의 시각과 github 의 시각을 비교해 사라진 노드는
지우고, 새 문서·내용은 더하고, 바뀐 것은 고친다 ⑶ *"OOO 맵의 OOO 노드를
업데이트"* · *"현재 선택한 노드의 내용을 업데이트"* — 노드 단위(폴더 · 파일 ·
파일 안의 2레벨 목차 절)로.
...
| 자리 | 파일 |
|---|---|
| GitHub 호출(저장소 기본 브랜치 · 트리 · 파일별 마지막 커밋 · raw 원문) | `apps/api/src/mcp/github-client.ts` |
| 순수 로직(저장소 지정 해석 · 절 나누기 · 맵 만들기 · 갱신 계획·적용) | `apps/api/src/mcp/github-docs.ts` |
| 도구 정의·실행(저장·잠금은 `openForWrite`·`saveVersion` 그대로) | `apps/api/src/mcp/mcp-tools.ts` |
| 단위 시험 | `apps/api/test/github-docs.test.mjs` (81항목, `test:mcp`) |

**맵 모양**

```
r 문서                              ← 루트: 링크 = 폴더 주소, 노트 = `출처: github:o/r@main:docs (가져옴 …)`
├─ guide/                           ← 폴더 노드: 링크 = …/tree/main/docs/guide
│  ├─ 설치 안내                     ← 문서 노드: 제목 = 첫 `#`(없으면 파일 이름), 링크 = …/blob/main/docs/guide/a.md
│  │  │  노트: `최종 업데이트: 2026-09-02T… · 커밋 c111111 · 파일 a111111` + 머리말의 문단·표
│  │  ├─ 요구 사항                  ← `##` 절: 노트 = 그 절의 문단(목록)·표·코드
│  │  │  └─ 선택                    ← `###` 절
│  │  └─ 실행
│  └─ b
├─ 개요 (README.md — 폴더의 맨 앞)
└─ Z
```

**정한 규칙과 이유**

1. **맵이 출처를 사람이 읽는 글로 기억한다.** 루트 노트 한 줄(`출처: github:…`),
   폴더·문서 노드의 GitHub 링크, 문서 노드의 `최종 업데이트:` 노트. 숨은
   메타 필드를 두면 앱에서 안 보이고 mmd 왕복에서 사라진다 — 글과 링크는
   둘 다 살아남는다. 갱신 도구는 **링크 주소**로 노드와 파일을 짝짓는다
   (같은 저장소·같은 브랜치의 blob 주소만). 경로에 공백이 있어도 된다 — 링크는
   `encodeURIComponent` 로, 출처 노트는 고정 접미 ` (가져옴 …)` 앞까지 통째로
   읽는다(#531 Codex 지적).
2. **바뀌었는지는 blob sha 로 판정하고, 없을 때만 커밋 시각으로.** 노트에
   `파일 <blob sha 7자리>` 를 함께 적어 두면 트리 조회 한 번(1회 호출)으로
   무엇이 바뀌었는지 안다 — 파일마다 커밋을 물을(N회) 필요가 없다. 옛 맵
   (sha 없음)은 저장된 시각과 마지막 커밋 시각을 비교한다(사용자 요구 그대로).
3. **`##`·`###` 만 절이다.** 사용자가 "목차의 2레벨까지" 라 했다. `####` 이하는
   그 자리의 **굵은 글**로 노트에 남긴다. 코드 펜스 안의 `#` 은 견출이 아니다.
   front matter 는 걷어낸다. `#` 이 없는 문서는 파일 이름이 제목이다.
4. **노트는 "중요한 것"만 — 상한이 있다** (`NOTE_LIMITS`: 노드당 노트 8개,
   표 4·코드 2, 문단·목록 합계 1,200자, 표 2,500자, 코드 1,500자). 문서 전체를
   옮기는 것이 목적이 아니다 — 원문은 링크로 한 번에 간다. 표는 언제나
   들어간다(사용자가 "중요 표" 를 첫째로 꼽았다).
5. **갱신은 노드 단위로 합친다 — id 를 지킨다.** 바뀐 문서의 노드는 **같은 id,
   같은 자리**에 새 내용을 놓고, 절 노드는 **제목으로 짝지어** id 를 유지한다
   (`mergeChildren`). 그래야 앱의 선택·접힘·다른 맵의 참조가 살아남는다.
   **사용자가 손으로 붙인 노드·노트는 남긴다** — 생성 id(`gh-`·`dir-`·`nt-`·
   `lk-`)가 아닌 것은 이 도구의 것이 아니다. 사라진 문서 때문에 빈 폴더 노드도
   지운다(사용자 노드가 남아 있으면 안 지운다).
6. **범위는 넷** — `node` 를 비우면 맵 전체, 폴더 노드면 그 폴더 아래 문서들,
   문서 노드면 그 문서, 문서 안의 절 노드면 **그 절만**(문서의 갱신 시각 노트는
   손대지 않는다 — 문서 전체를 맞춘 것이 아니라서). `"selected"` 는 앱에서
   선택한 노드(`FocusService`, §9.9). 절이 저장소 문서에서 사라졌으면 지우지
   않고 알린다 — 문서 노드로 갱신하면 그때 지워진다.
7. **트리는 문서 폴더 아래만 재귀로 받는다**(`treeUnder` — 폴더를 한 층씩 내려가
   그 폴더의 트리 sha 를 찾은 뒤 그 아래만). 저장소 전체를 재귀로 받으면 큰
   저장소에서 GitHub 이 목록을 자르고(`truncated`), 잘린 목록으로 갱신을
   계획하면 빠진 문서가 전부 "사라진 것" 이 되어 **멀쩡한 노드를 지운다**(#531
   Codex 지적). 그래도 잘리면 두 도구 모두 **멈추고** 폴더를 좁히라고 한다.
8. **익명 GitHub 호출은 시간당 60회** — 트리·저장소 정보 몇 회 + 문서마다 커밋
   1회라 **문서 50개**까지만 받는다(원문은 raw 주소라 한도에 안 든다). 그 위는
   API 서버 환경변수 `GITHUB_TOKEN`(읽기 권한 fine-grained 토큰이면 된다,
   시간당 5,000회)을 넣거나 `path`·`max_files` 로 좁힌다. 한도에 걸리면 GitHub 이
   알려 준 재시도 시각을 문장에 넣는다. 토큰은 **서버 환경변수로만** 받는다 —
   도구 인자로 받으면 대화 기록에 남는다.
9. **§2-3 의 둘째 예외다.** 지우는 것은 이 도구가 만든 노드뿐이고, 이 도구로
   만들지 않은 맵은 거절하며(루트 노트에 `출처:` 없음), 히스토리 버전으로
   남는다. 저장·잠금·공개·협업 판정은 `append_to_map` 과 같은 함수
   (`openForWrite`·`saveVersion`)를 탄다.

**검증**(e2e290): 단위 `test/github-docs.test.mjs` **81항목** — 저장소 지정
해석(주소·`.git`·`tree/브랜치/경로`·거절 3) · 문서 폴더 판정·파일 고르기
(대소문자·바이트순·blob sha) · 절 나누기(front matter · 펜스 안 `#` · 표 · 목록 ·
`####` → 굵은 글 · 인용문 · 사진 줄 버림 · 제목 없음 · `##` 없는 `###`) · 맵 만들기
(출처 노트 왕복 · 폴더 먼저·README 먼저 · 1레벨 색·방향 · 링크·라벨 · 갱신 노트
왕복 · 머리말 노트 · `##`→자식·`###`→손자 · 커밋 모름 · id 유일 · 절 안으로 안
내려감 · 공백 경로 출처·링크 왕복) · 클라이언트(가짜 fetch — 폴더를 한 층씩
내려가 그 아래만 재귀 · 없는 폴더 404 · 뿌리) · 계획(추가·수정·삭제·그대로 · sha 없으면 커밋 시각 · 폴더·문서 범위 ·
범위 판정 5종 · 절 제목 사슬) · 적용(원본 불변 · id 유지 · 없어진 절 삭제·새 절
추가 · 사용자 노드·노트 보존 · 빈 폴더 정리 · 새 폴더 사슬 · 절 범위만 갱신 ·
사라진 절 알림 · 다른 저장소 링크 무시). JSON-RPC 시험은 도구 여덟과 이름
예외를 못 박는다. **진짜 GitHub·DB·Claude 대화로는 재 보지 않았다** — 이
컨테이너는 GitHub API 로 나가는 연결이 막혀 있다. dev 에 배포한 뒤 사용자가
"okpojung/easymindmap 저장소 문서를 emm 새 맵으로 만들어 줘" → "그 맵을
업데이트 해줘" 로 한 번 확인하는 것이 남았다(docs 폴더는 문서가 수백 개라
`GITHUB_TOKEN` 이 있어야 한다 — 없으면 `path:"docs/user-guide"` 처럼 좁힌다).
...
1. ChatGPT(웹) ▸ 프로필 ▸ **설정** ▸ **보안 및 로그인** ▸ **개발자 모드** 켜기
   (2026-09-22 사용자 화면 실측 — "위험도 높음" 배지. 안내 글들이 말하는
   *앱 및 커넥터 ▸ 고급 설정* 은 옛 배치다)
2. **설정 ▸ 플러그인** 페이지 오른쪽 위 **[+]**(= 만들기). 안내 글들의
   *앱 및 커넥터 ▸ [만들기(Create)]* 도 같은 것의 옛 이름이다. 사용자가
   "[만들기] 를 못 찾겠다" 고 한 것이 이 이름 차이 때문이었다(2026-09-22)
3. 채우는 칸
...
### 12.7 실측 2 — **계정 연결 성공** · 그런데 "액션이 없습니다" (2026-09-22)

§12.6 을 재배포하고 플러그인을 새로 만들어 [다른 계정 연결] → **연결됨**
("서비스 바로 님의 emm 계정 · Primary", 사용한 인증 OAuth). 토큰 교환 500 은
풀렸다 — 겉면이 `openid` 를 뗀 것이 통했다.

그런데 플러그인 화면의 **액션**이 여전히 "아직 사용할 수 있는 앱 액션이
없습니다" 다. 후보는 셋인데 **서버에 근거가 하나도 없다** — API 가 MCP 요청을
로그에 남기지 않아서 `tools/list` 가 왔는지, 401 이었는지, `GET` 405 뒤에
포기했는지 알 길이 없다.

| 후보 | 근거가 될 로그 |
|---|---|
| ① 액션 목록은 플러그인을 만들 때(로그인 전, 401) 읽어 둔 것 — [새로 고침] 을 눌러야 다시 읽는다 | 연결 뒤 `tools/list` 가 **아예 없음** |
| ② ChatGPT 가 `GET /v1/mcp`(SSE) 405 를 못 견디고 포기 | `GET → 405` 만 있고 POST 없음 |
| ③ 토큰은 받았는데 우리 가드가 거절 | `MCP OAuth 토큰 거절 …` warn |

그래서 **요청 로그를 넣었다** (`mcp.controller.ts`, e2e302): POST 마다 한 줄
`MCP [UA] ← initialize, notifications/initialized, tools/list → 200` (배치면
쉼표로, `tools/call` 은 도구 이름까지) · `GET` 등은 `→ 405` 한 줄 · 가드의
토큰 거절은 debug → **warn**(배포 로그에 찍히도록). 본문·토큰은 적지 않는다.

확정 절차: 재배포 → ChatGPT 플러그인 화면 [새로 고침] → 안 되면 새 대화에서
`emm` 을 켜고 "내 emm 맵 목록 보여줘" → API 컨테이너 로그에서 `MCP [` 줄을
본다(사용자 가이드 12 §2-D 의 API 로그 스크립트).

### 12.8 ★ 실측 3 — **붙었다. 액션 8개가 보인다** (2026-09-22)

§12.7 의 후보 ① 이 맞았다. 플러그인 화면 정보 옆 **[새로 고침]** 을 누르자
액션 목록이 채워졌다 — `append_to_map` 이 **"쓰기"** 배지(우리 `readOnlyHint:
false`, §12.2)와 함께, 설명·입력 스키마까지 그대로 보인다. 액션 목록은 플러그인을
만들 때(로그인 전 401) 한 번 읽어 두는 것이라 연결 뒤 새로 고쳐야 했다.
API 재배포(§12.7 의 로그)는 이 확인에 필요하지 않았다.

이로써 **ChatGPT 경로(§9.2 ②-E)가 열렸다** — 2026-09-22 하루에 세 실측:

| 실측 | 막힌 곳 | 푼 것 |
|---|---|---|
| 1 (§12.5) | 계정 연결 → "연결에 문제" | 원인은 토큰 교환 500(`openid`) — GoTrue 로그가 확정 |
| 2 (§12.7) | 연결됨 · 액션 없음 | 화면 캐시 — [새로 고침] |
| 3 (이 절) | — | 액션 8개 |

#### ChatGPT 화면이 남긴 힌트 하나 — "출력 스키마 권장"

각 액션에 **⚠ 출력 스키마 권장** 배지가 붙는다. MCP 의 `outputSchema`(도구가
돌려주는 `structuredContent` 의 JSON Schema)를 우리 도구가 선언하지 않아서다.
없어도 동작한다 — 우리 결과는 `content[].text` 한 줄이고 ChatGPT 는 그것을
그대로 읽는다. 선언하려면 도구마다 결과를 구조화해야 하므로(맵 id·이름·노드 수
등) **별도 작업**으로 남긴다. 지금은 하지 않는다.

#### 남은 확인 — 대화에서 실제로 부르기

플러그인 화면의 액션과 대화에서의 호출은 다른 단계다. 새 대화 ▸ [+] ▸ 더 보기 ▸
`emm` ▸ "내 emm 맵 목록 보여줘" 로 `list_maps` 가 불리고, "…를 emm 맵으로
저장해줘" 로 확인 창 → `create_map` → 문서함에 맵이 생기면 끝이다. API 로그
(§12.7)에 `MCP [ChatGPT…] ← tools/call:list_maps → 200` 이 남는다.

### 12.9 실측 4 — 대화에서는 **안 불린다** · 서버에 흔적이 없다 (2026-09-24)

§12.8 로 액션 8개까지 봤지만 **대화에서 `list_maps` 가 불리지 않았다.** 모델은
*"현재 제가 사용할 수 있는 도구에는 EMM 맵 목록 조회 기능이 없습니다"* 라고
답했다 — 즉 ChatGPT 가 모델에 도구를 **주지 않았다.**

| 시각(UTC) | 어디 | 무엇 |
|---|---|---|
| 09-24 11:51 | API | #555 로그 판으로 재배포 |
| 09-24 15:29 | API | `claude-code/2.1.281 (remote)` 가 initialize → GET 405 → tools/list 200 (Claude Code 는 405 를 견딘다 — §12.4 하나 확정) |
| 09-24 16:00 · 16:01 | GoTrue | ChatGPT 재연결 두 번: authorize 302 → authorizations 200 → **token 200** |
| 09-24 16:00~17:00 | API | **ChatGPT 의 `/v1/mcp` 요청 0건** — 연결 직후에도, 채팅 때도 |

GoTrue DB 도 봤다: ChatGPT 가 DCR 로 만든 클라이언트 둘(09-22 07:02 · 11:33)은
`public/none` 이지만 `grant_types` 에 `refresh_token` 이 있다. 갱신을 못 하는
구조는 아니다.

#### 아직 가르지 못하는 것 — 그래서 로그 구멍을 막았다

"ChatGPT 가 안 불렀다" 와 "왔는데 **조용히 401** 로 돌아갔다" 를 구분할 수 없었다.
가드는 JWT 검증 실패만 warn 으로 남기고, **토큰 없음·PAT 폐기·형식 오류·OAuth
미설정** 은 소리 없이 401 이었다. 겉면 메타데이터·authorize 도 로그가 없었다.
e2e307 에서 넷을 고쳤다 — 가드의 **모든 거절**을 `MCP 거절 [UA] 토큰=<없음|PAT|JWT|기타>:
이유` 로, PRM·AS 메타데이터 읽기를 `메타데이터 [UA] ← …` 로, 겉면 authorize 를
`겉면 authorize [UA] scope="…"` 로. 토큰 값·state·PKCE 는 적지 않는다.

#### 다음 실험 — 재배포 뒤 둘

1. 플러그인 화면 **[새로 고침]** → API 로그에 `tools/list` 가 찍히나. 찍히면
   ChatGPT → 우리 서버 연결은 살아 있고, 문제는 **대화에서 도구를 모델에 넘기지
   않는** ChatGPT 쪽 단계다(개발자 모드 플러그인의 대화 선택·권한 설정을 본다).
2. 새 채팅에서 "내 emm 맵 목록 보여줘" → `MCP 거절 …` 이 찍히면 그 이유가
   원인이고, 아무것도 없으면 ChatGPT 가 대화에서 서버를 부르지 않는 것이다.

> 09-22 에는 [새로 고침] 이 `tools/list` 를 불러 액션이 채워졌다(로그 없던 판이라
> 간접 증거). 그 뒤 우리 쪽 변경은 #555(로그)뿐이다 — 요청 처리 경로는 같다.
