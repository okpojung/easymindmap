...
> 실패 규칙·검증 항목까지 적는다. 상태: **설계 확정 · 구현 대기**
...
| 단계 | 내용 | 검증(e2e) |
| --- | --- | --- |
| **C1** | 서버 — 세션/조각/완료/취소 + 스트림 저장 + 쿼터 예약 | 조각 3개짜리 파일 왕복, 조각 누락 시 409, 남의 `uploadId` 404 |
| **C2** | 프런트 — `uploadInChunks` + 재시도 + 취소 | 조각 PUT 을 1회 실패시켜도 완료됨(멱등 재시도), 취소 시 tmp 정리 |
| **C3** | UI — 진행률·"올리는 중" 표시, 드롭/메뉴 공통 | 큰 파일 드롭 시 진행률이 오르고 완료 후 노드에 첨부 |
| **C4** | 정리 — 만료 tmp GC, 상한 1GB 로 상향, 문서 갱신 | 만료 세션이 지워짐, 1GB 파일 왕복(수동 1회) |
| **C5** *(선택)* | 새로고침 후 이어받기(`uploadId` 보관) | 새로고침 뒤 남은 조각만 전송 |

> **주의 — 테스트에서 1GB 를 만들지 않는다.** `partSize` 를 환경변수로
> 낮춰(예: 64KB) 조각 여러 개가 나오게 하면 같은 경로를 **작은 파일로**
> 검증할 수 있다. CI 시간과 디스크를 아끼는 정석이다.
