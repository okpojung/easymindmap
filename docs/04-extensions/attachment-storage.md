...
> · 상태: **P1 구현 완료(2026-08-02, B9) · P2~P4 설계 · §12 청크 업로드 설계 확정(2026-08-06)**
...
| 플랜 | 저장 용량(문서+첨부 합산) | 파일 1개 최대 | 상태 |
|---|---|---|---|
| Free | **1 GB** | 1GB(쿼터가 먼저 걸림) | ✅ `quota_bytes` 기본값 |
| Basic(기본 구독) | **10 GB** | 〃 | ✅ 값은 적용 · **자동 승격은 없음**(아래) |
| Team | 사용자당 20 GB (워크스페이스 합산) | 〃 | 초안 — 미구현 |
| 내 드라이브 사용 시 | **무제한(과금 대상 아님)** | 브라우저 한계 | 초안 — 미구현(P4) |

### 8.1 지금 실제로 어떻게 정해지나 (2026-08-06)

**한도는 `public.users.quota_bytes` **한 컬럼**이 전부다.** 요금제 컬럼은
아직 없다 — 즉 **결제·플랜에 따라 자동으로 올라가는 경로가 없고**, 상향은
그 값을 직접 UPDATE 하는 것이 유일한 방법이다.

| 대상 | 한도 | 어떻게 |
|---|---|---|
| **신규 가입** | 1GB (Free) | 컬럼 기본값 `1073741824`. 코드가 `INSERT INTO public.users (id)` 만 하므로 항상 기본값이 붙는다 |
| **2026-08-06 12:00 UTC 이전 가입 계정** | **10GB (Basic)** | `schema.sql` 의 일회성 UPDATE (아래) |

```sql
-- schema.sql §15 — 초기 사용자 Basic 승격
UPDATE public.users
   SET quota_bytes = 10737418240            -- 10GB
 WHERE created_at < TIMESTAMPTZ '2026-08-06 12:00:00+00'
   AND quota_bytes < 10737418240;
```

**고정 시각을 쓴 것이 핵심이다.** `schema.sql` 은 배포마다 다시 적용되는데,
조건이 `NOW()` 기준이거나 아예 없으면 **재적용할 때마다 그 사이에 가입한
무료 사용자까지 10GB 로 올라간다.** 고정 시각이면 몇 번을 돌려도 대상이
늘지 않는다. `quota_bytes < ...` 조건은 이미 손으로 더 크게 준 계정을
**끌어내리지 않기** 위한 것이다.
...
```sql
-- 이메일로 찾아서 10GB
UPDATE public.users u SET quota_bytes = 10737418240
  FROM auth.users a WHERE a.id = u.id AND a.email = '<이메일>';

-- 현재 상태 확인
SELECT a.email, pg_size_pretty(u.quota_bytes) AS 한도, u.created_at
...
> 실패 규칙·검증 항목까지 적는다.
>
> **상태: C1~C4 구현 완료 (2026-08-06).** C5(새로고침 후 이어받기)만
> 남았다. 설계에서 **바뀐 점 두 가지**는 §12.5-1 에 적어 두었다.
...
* **진행률** = `보낸 조각 수 / 전체 조각 수`. 조각이 8MB 라 1GB 면 88단계 —
  막대가 충분히 부드럽다. (요청 내부 진행률까지 볼 필요 없다)
...
| 파일 1개 상한 | **1GB** (플랜별 조정) | 사용자 요청. 계정 쿼터(무료 1GB)가 먼저 걸리므로 유료 플랜과 함께 조정 |
...
| **C5** *(선택)* | 새로고침 후 이어받기(`uploadId` 보관) | ⏸ 미구현 | — |
