...
> ### ⚠️ 2026-08-16 — 이 문서의 **전송·충돌 전제는 낡았다**
>
> 아래 §5.2·§5.3·§14 는 **Supabase Realtime · Redis · LWW** 를 전제한다.
> **셋 다 지금 코드에 없거나 바뀌었다** — Supabase 는 Auth 전용이고,
> Redis 는 쓰지 않으며, 충돌 해소는 **CRDT(Yjs)** 로 정해졌다.
>
> **동기화 모델은 [`27-sync-model.md`](./27-sync-model.md) 가 기준이다.**
> 이 문서에서 여전히 유효한 것은 **권한·초대·스레드·API 계약**이다.
>
> 협업은 **유료 기능**이라 구현은 private 저장소에 있다
