...
A-5  findOrphans 를 쓰는 수리 함수를 documentStore 에 만든다
     repairOrphans(): 뿌리에 닿지 못하는 노드를 뿌리로 올리고, 몇 개를
     올렸는지 돌려준다. 지우지 않는다
```

> **A-5 를 지금 하는 이유**: 협업이 없어도 값어치가 있다. 잘못된 임포트나
> 옛 문서 때문에 이미 고아가 생긴 맵이 있으면 **그 가지는 화면에서 통째로
> 안 보인다.** 사용자는 데이터가 사라졌다고 생각한다.
...
- **`repairOrphans` 를 언제 부르는가.** 문서를 열 때마다 훑으면 큰 맵에서
  비용이 든다. 저장 직전에만 부를지, 사용자가 부를 때만 할지 정하지 않았다.
