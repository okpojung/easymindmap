...
1. 기능 개요
맵을 Read-only 대시보드로 설정하면, 외부 시스템이 노드의 텍스트 값을 변경했을 때 설정된 주기에 따라 맵이 자동으로 화면을 리프레시하여 항상 최신 값을 표시하는 기능.
...
view_mode / refresh_interval DB 컬럼
매우 낮음 (이미 설계 완료)


GET /maps/:id/snapshot API
낮음


setInterval polling 로직
낮음
