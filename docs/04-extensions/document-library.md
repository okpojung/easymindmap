...
> **최종 업데이트**: 2026-08-05 — 관리 아이콘을 이모지(✏📂🗑) → **SVG 버튼**으로(작아서 식별 불가 보고) + 열 머리글을 값과 같은 쪽으로 정렬('관리'는 버튼 가운데). 2026-08-04(2차): 목록 상세 컬럼(생성일·수정일·노드수·크기·첨부수·첨부용량) + 전 컬럼 오름/내림 정렬. 같은 날 1차: 읽기 전용(다른 세션 편집 중) 사본 저장 분기 · '다른 이름으로 저장' · 무변경 저장 스킵 추가.
>
...
| 6 | 목록은 **이름·생성일·수정일·노드수·크기·첨부수·첨부용량** 7컬럼 표시 + 머리글 클릭 오름/내림 정렬 (2026-08-04 확장). **기본 정렬 = 이름 오름차순**(2026-08-05 — 폴더에 들어갔을 때 사람이 찾는 순서는 가나다순이다) | `GET /maps?sort=&order=` |
| 6-1 | 폴더·맵 **행에는 호버 툴팁을 붙이지 않는다** — 행 강조(`.mm-list-row`)가 선택 대상을 이미 보여 준다. 아이콘만 있는 관리 버튼은 예외(툴팁이 유일한 설명) | `coding-conventions.md §5-1` |
...
- **무변경 저장 스킵**: 내용이 직전 저장과 같으면 저장을 건너뛴다
  (`unchanged` — 히스토리 버전·`updated_at` 을 만들지 않는다).

### 다른 이름으로 저장 (2026-08-03)

- **서버 맵과 연결된 상태에서만 노출**되는 메뉴 — 새 폴더·이름으로
  **새 맵을 저장**한 뒤 그 맵의 탭으로 전환한다.
- 기본 이름은 **'<제목> 사본'**.
...
- **끊기 전에 먼저 저장한다** (2026-08-07 — 첨부 유실 보고).
  확인 게이트("현재 맵 '…'을 닫고 진행할까요?")에서 지금 맵이 서버
  맵이면 버튼이 **`저장하고 계속`** 이 되고, `saveCurrentMap()` 이
  끝난 **뒤에야** 문서를 갈아 끼운다. 저장이 실패하면 **닫지 않는다**.

  왜 필요했나 — 자동저장은 **주기(기본 5분)·미저장 편집 50건·탭 전환**
  에만 올린다(useCloudAutosave, 2026-08-06 개편). 대용량 첨부를 붙이고
  곧바로 `새 맵 만들기 > 계속` 을 하면 그 사이 어느 조건도 걸리지 않아
  **서버에는 첨부 이전 스냅샷**만 남았다. 사용자가 "처음엔 첨부가 없이
  열렸는데 다시 해 보니 있더라"고 한 것도 같은 이야기다 — 두 번째엔
  주기가 지났을 뿐이다. 상단 `✕ 맵 닫기`(saveAndCloseMap)는 원래
  저장하고 닫았으므로, 이 문을 그 규칙에 맞춘 것이다. 검증: e2e124 [3].
...
- **트리 목록** (2026-08-07 재구성 — 사용자 방안 3). 폴더를 눌러도 그
  폴더로 **들어가지 않고 그 자리에서 펼쳐진다**. `cwd`·breadcrumb 는
  없앴다 — 폴더 위치와 파일 이름을 한 화면에서 같이 보고, 폴더를 들락
  날락하지 않기 위해서다.
  · 기본 **모두 접힘** · `⊞ 모두 펼치기 / ⊟ 모두 접기`
  · **`↻ 새로고침`** (2026-09-21 사용자 요청 — "공유받은 문서를 보려고
    Ctrl+F5 를 눌렀다"). "내 문서" 바로 다음 버튼. 같은 `load()` 를 다시
    불러 **폴더·내 맵·공유받은 맵 세 목록**을 한 번씩 더 읽는다(브라우저
    새로고침이 필요 없다). 읽는 동안 `↻ 읽는 중…` 으로 잠긴다. 검증 e2e293
  · `＋ 새 폴더` = 홈에 만들기. 폴더 줄의 `＋` = 그 폴더 안에 만들기.
    **입력창은 만들어질 자리에 뜬다** — 홈이면 목록 맨 위, 폴더면 그
    폴더 바로 아래에 한 단계 들여써서(2026-08-08 사용자 보고: 목록 맨
    위 막대는 "TTTT 안"이라 쓰는데 커서가 남은 다른 줄이 hover 로
    강조돼 화면이 두 가지를 말했다). 입력 중에는 hover 강조를 끄고
    대상 폴더만 표시하며, 접힌 폴더는 펼친다.
    `Esc` 는 **안쪽부터** — 입력 취소 → 검색어 지우기 → 문서함 닫기
    (예전에는 입력 중 Esc 가 문서함을 통째로 닫았다).
    규칙: coding-conventions.md §5-1-2·§5-1-3
  · 들여쓰기는 `depth * 16px`
- **전체 검색** — 지금 폴더가 아니라 **문서함 전부**를 훑는다. 맞는
  것이 든 폴더는 자동으로 펼쳐지고(`keepFolder`: 이름이 맞거나 하위에
  맞는 것이 있으면 남긴다), 그 안에서도 **맞는 맵만** 남는다.
...
- 열 머리글 **폴더/파일명 · 생성일 · 수정일 · 노드 · 크기 · 첨부 ·
  ...
  (`fontVariantNumeric: tabular-nums`)라 자릿수가 세로로 맞는다.
- **오른쪽 맞춤은 유형·생성일·수정일까지** (2026-08-09 요청). 값(단독맵
  배지·날짜)이 열 오른쪽 끝에서 끝나는데 머리글만 왼쪽에 있어 세로선이
  어긋나 보였다. 폴더 줄의 `폴더`·`맵 N개` 도 같은 선에 세운다 —
  `맵 N개` 는 **생성일 칸 하나만** 쓴다(두 칸을 차지하면 오른쪽 맞춤이
  수정일 선까지 밀린다).
- **합계는 머리글 바로 아래 고정 한 줄**(2026-08-09 3차 요청,
  `browser-totals`). 예전에는 숫자 4열만 **이름 위**에 합계를 얹었는데,
  그 열만 두 줄이 되어 머리글 높이가 들쭉날쭉했고 합계가 어느 열 것인지도
  헷갈렸다. 지금은 각 값이 **그 열 아래**에 놓여 열 이름이 곧 설명이다.
  정렬을 어떻게 바꿔도 자리를 지킨다(목록의 일부가 아니다).

  | 열 | 합계 |
  |---|---|
  | 폴더/파일명 | `합계` (검색 중이면 `합계 (검색 결과)`) |
  | 유형 | **폴더 수** — 하위 폴더까지 전부 |
  | 생성일 | **맵 수** (`맵 8(개)`) — 폴더 줄이 '맵 N개'를 쓰는 칸이라 같은 말로 |
  | 수정일 | 비움 (합계가 없는 열) |
  | 노드 · 크기 · 첨부 · 첨부 용량 | 각 열의 합 |

- **단위는 괄호로 떼되 숫자와 붙여 쓴다**: `1,448(개)` · `309.8(KB)` ·
  `1.01(GB)`. 숫자와 단위가 완전히 붙어 있으면(`309.8KB`) 자릿수를 눈으로
  세기 어렵고, 반대로 `309.8 (KB)` 처럼 띄우면 좁은 열에서 두 줄로 접힌다
  (`nowrap` 도 함께 건다). 목록 안의 값은 예전대로 붙여 쓴다 — 한 줄에
  여러 열이 늘어서므로 짧을수록 읽기 쉽다.
- **머리글은 전 열이 같은 색**(강조색 primary) — 2026-08-09 요청 두 번을
  거쳐 정해졌다. ⑴처음에는 `textMuted`(회색)라 이름이 잘 안 읽혔고
  ("왜 희미한 색상이지"), ⑵본문색으로 올린 뒤에도 **정렬 중인 열만**
  강조색이라 기본 정렬인 `폴더/파일명` 만 또렷하고 나머지는 상대적으로
  죽어 보였다("모두 폴더/파일명 글자색과 동일하게").
  지금은 **정렬 상태를 화살표로만** 구분한다 — 정렬 중이면 ▲/▼, 아니면
  ⇅. 색까지 다르게 할 필요가 없다.
  합계는 본문색 + 흐림(opacity .75) 제거, 값 줄(날짜·숫자)은
  `textSubtle → textMuted`, 폴더 줄의 `폴더` 는 같은 줄 폴더 이름과 같은
  본문색이다.
- 합계 기준: 그냥 볼 때 **전체**(접혀 있어도), 검색 중 **결과**.
...
**계정이** 바뀔 때마다(EditorPage, `[authUserId, guest]` 효과) **문서·서버
링크·undo 히스토리를 전부 비운다**:

> ★ 의존성은 `session` **객체가 아니다** (2026-09-04). 토큰 갱신마다 새
> 세션 객체가 오는데, 그것을 "로그인 전환" 으로 보면 로그인 1시간 뒤 첫
> API 호출이 편집 중인 문서를 지운다 — 실제로 그렇게 '문서 없음' 이
> 저장·내보내기까지 이어졌다. 상세 [14-save.md §7.12](../03-editor-core/save/14-save.md).
...
### 8.6 검색 범위 — 퍼블리싱 · 공유받은 맵까지 (2026-09-07 사용자 요청)

> "내문서에서 검색시 내문서함에 있는 맵만 검색되는것 같은데, 검색범위를
> 퍼블리싱, 공유받은맵 모두다 검색대상에 포함해줘"

**있던 것과 없던 것.**

| 자리 | 전 | 후 |
|---|---|---|
| 내 트리 | 제목 + 맵 안 (서버 `GET /maps?q=`) | 그대로 |
| 퍼블리싱 자리 | **원래부터 같은 검색**이었다 — 퍼블리싱 맵도 내 맵이라 같은 서버 결과에 들어오고, 화면이 `publishId` 로 자리만 나눈다(§7) | 그대로 (검증만 추가) |
| 공유받은 맵 | 처음 받아 둔 **100개 안에서 이름만** 화면이 걸렀다 — 그 바깥은 이름조차 못 찾았고, 맵 안의 글은 아예 안 봤다 | **서버 `GET /maps/shared?q=`** 가 제목 + 맵 안을 찾는다. 배지 `맵(N건)` 도 같다 |

**서버** (`MapsService.listShared`) — 내 목록 `list()` 와 **같은 형태**로
붙였다. 내용 조건은 MATERIALIZED CTE(§8.2 의 실측 근거 그대로 — trigram
인덱스를 타게), CTE 안에서 **참가자 표(`map_members`)와 조인**해 내가
참가자가 아닌 문서는 훑지 않는다(`list()` 의 소유자 조인과 같은 이유:
2글자 검색이 남의 문서 전체를 훑는 것을 막는다). `search_text` 는
검색할 때만 고른다 — 델타 미적용 서버에서 공유 목록이 죽지 않게.

**화면** — 검색 중 내 맵과 공유 맵을 **나란히** 묻는다(`Promise.all`).
공유 검색이 실패해도 내 검색은 살아야 하므로 따로 잡고, 그때만 받아 둔
목록에서 **이름**으로 물러선다(배너는 내 검색이 실패했을 때만).

**검증: E2E e2e230** (서버 15 + 화면 15). 서버 수정을 되돌리면 서버
4항목이 FAIL(공유 맵을 맵 안의 글로 못 찾고 `matchCount` 도 없다).
