...
- **오른쪽 맞춤은 유형·생성일·수정일까지** (2026-08-09 요청). 값(단독맵
  ...
  수정일 선까지 밀린다).
- **합계는 단위를 괄호로 뗀다** (2026-08-09 요청): `1,448(개)` ·
  `309.8(KB)` · `1.01(GB)`. 숫자와 단위가 붙어 있으면 자릿수를 눈으로
  세기 어렵고, 열마다 단위가 달라 값끼리 비교도 헷갈렸다. 목록 안의
  값은 예전대로 붙여 쓴다(`309.8KB`) — 한 줄에 여러 열이 늘어서므로
  짧을수록 읽기 쉽다.
  · 숫자와 `(` 사이는 **띄우지 않는다**(같은 날 2차) — 띄우면 좁은 열에서
    `1,448` / `(개)` 두 줄로 접혀 머리글 줄만 높아졌다. `nowrap` 도 함께.
