...
- **합계는 단위를 괄호로 뗀다** (2026-08-09 요청): `1,448 (개)` ·
  ...
  짧을수록 읽기 쉽다.
- **머리글은 본문색** (2026-08-09 — "왜 희미한 색상이지"). 예전에는
  `textMuted` 라 회색으로 잠겨 이름이 잘 안 읽혔다. 머리글은 표를 읽는
  기준선이라 흐리면 표 전체가 흐려 보인다. 일부 열만 올리면 나머지가 더
  흐려 보이므로 **전 열을 함께** 올렸고, 합계도 흐림(opacity .75)을 뺐다.
  값 줄(날짜·숫자)도 `textSubtle → textMuted` 로 한 단계 올렸다.
  라이트 15:1 · 다크 13:1 대비.
