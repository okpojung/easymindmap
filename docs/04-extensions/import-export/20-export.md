...
### ★ 표가 노드 밖으로 삐져나왔던 일 (2026-09-12)

사용자가 퍼블리싱 링크를 열어 보고 알렸다 — **표가 노드 상자를 뚫고
나왔다. 에디터에서는 멀쩡했다.** 내보낸 HTML 파일도 같은 코드라 **똑같이
깨져 있었다**(공개 뷰어와 내보내기는 `buildStandaloneHtml` 하나를 쓴다).

**왜 갈라지나** — 표는 **상자 폭을 에디터가 굽고(`pos.w`) 격자는 뷰어가
그린다.** 둘이 같은 규칙으로 재지 않으면 어긋난다. 어긋난 곳이 둘이었다.

| | 에디터 | 뷰어(예전) |
|---|---|---|
| 셀 폭 | `measureTextApprox(stripInlineMarks(셀))` | `measureText(셀 **원문**)` |
| 마커 문자 | **숨기고 서식만** | **글자로 그대로 그림** |

백틱·별표까지 세니 열이 넓어졌고, 게다가 `measureText` 는 노드 본문용
근사라 **숫자·대문자 계수가 다르다**(0.56 단일 vs 0.62·0.72). 코드 표시가
많은 표에서 특히 크게 벌어진다 — 사용자가 캡처한 표가 **44px** 넘쳤다.

**고친 것** — 표 셀에만 에디터와 같은 공식(`cellApproxW`)을 쓰고, 마커는
`parseInlineSegs` 로 **숨기고 서식만** 입힌다. `measureText` 자체는
건드리지 않는다 — 노드 본문 폭 계산이 함께 흔들린다.

> ⚠️ **이 스크립트 안에서는 정규식에 유니코드 이스케이프를 쓰지 말 것.**
> 뷰어 코드는 **문자열로 실려 나가서 백슬래시가 한 겹 더 남는다.**
> 처음에 `/[\u3000-…]/` 로 적었다가 뷰어에는 `/[\\u3000-…]/` 가 실려
> **한글이 CJK 로 안 잡혔고 열이 45%쯤 좁아졌다.** 코드포인트로 직접
> 비교한다(`isCjkCell`). 백틱도 같은 이유로 못 쓴다.

검증은 **진짜 크로미움**으로 뷰어를 띄워 잰다 (e2e255).
...
| 노드 내 Markdown 표 | 뷰어가 노드 텍스트에서 표를 같은 규칙으로 감지해 SVG 격자로 그림 (에디터 좌표 모드에서만 — `pos.lines`에는 표 원문이 제외되어 있음). ★ **열 폭·셀 글자를 에디터와 같은 규칙으로 잰다** — 2026-09-12 에 어긋나 표가 노드 밖으로 삐져나왔다(아래) |
| 접힌 노드 아래의 자식 | **크기(`pos`)를 전부 굽는다** (2026-09-04 사용자 보고). 레이아웃 엔진은 접힌 노드의 자식을 배치에서 빼므로(`pruneCollapsed`) 예전에는 그 아래 노드에 `pos` 가 없었고, 뷰어에서 펼치는 순간 재배치(`reflowFixed`)가 크기 없는 블록을 더해 **NaN 좌표로 맵이 통째로 사라졌다**(진행트리 '모두 펼치기' → 백지). 이제 전부 펼친 사본을 한 번 더 배치해 빠진 노드의 크기·줄바꿈·글꼴을 채운다 — 위치는 뷰어가 다시 계산하므로 크기가 전부다. 뷰어 쪽도 `pos` 없는 자식은 스스로 잰다(`measure` 폴백) — 두 겹이라 한쪽이 빠져도 백지가 되지 않는다 (e2e187) |
| 접기 → 다시 펼치기 | **내보낼 때의 접힘 상태로 돌아오면 구운 좌표(`pos`)를 다시 쓴다** (2026-09-18 사용자 보고, e2e279). 예전에는 접기 칩을 한 번이라도 누르면 `DYN` 이 영영 켜져, 모두 펼쳐도 뷰어 자체 배치(`reflowFixed` — 간격 배율·오버라이드 밀기를 모른다)로 남았다. 이제 render 마다 "지금 접힘 상태가 내보낼 때와 다른가"(`_c0` 비교)로 정한다 — 일부가 접혀 있는 **동안**만 재배치. 첫 중심의 배치 기준과 상단 표시는 `root.layoutType` 이 아니라 **`mapLayout`(편집기의 맵 레이아웃)** 을 먼저 본다 — 엔진이 첫 중심을 그것으로 놓았고 `root.layoutType` 은 옛 값이 남을 수 있다(사용자 파일: root=tree-right · 편집기=진행트리 → 접었다 펴면 트리·오른쪽으로 놓였다) |
...
- **아웃라인 분할·아웃라인 모드/맵 모드 (2026-07)** — 에디터와 파리티:
  ...
  - **행 본문은 블록을 그대로 그린다** (2026-09-04 사용자 보고) — 에디터
    아웃라인(`RichTextHtml.NodeRichText`)과 같은 규칙: 코드 펜스 → 언어
    라벨 + ⧉ 복사 + 모노 줄(격자 배치), 파이프 표 → 격자 표 + ⧉ 복사,
    `- [x]` → 체크 글리프(표시만), 인라인 마크(**굵게** 등) → 마커 숨김.
    원문 순서(앞 텍스트 → 표/코드 → 뒤 텍스트)를 지킨다. 예전에는
    검색 결과용 한 줄 요약(`flattenText` — "⧉코드"·"⊞표" 로 접기)을
    아웃라인 행에도 써서, 에디터에서 보이던 코드·표가 뷰어 아웃라인에서는
    칩 하나로만 보였다. 접기 규칙은 **검색 결과 목록에만** 남는다
    (`richTextEl` · `.mm-ol-code` · `.mm-ol-table` — e2e185).
  - **양방향 동기** (2026-09-04 사용자 보고) — 맵에서 노드를 클릭하면
    아웃라인의 그 행이 선택 표시(`.mm-ol-row.sel`, 주황 왼쪽 띠)되고
    **그 행으로 스크롤**된다(에디터 `OutlineScrollSync` 와 같은 규칙 —
    `scrollIntoView nearest`). 예전에는 아웃라인 → 맵만 됐다. 스크롤은
    **선택이 바뀐 때만** 한다 — 접기/펴기·검색으로 아웃라인이 다시
    그려질 때 화면을 끌고 가면 보던 자리를 잃는다. 아웃라인 행 클릭도
    선택(`SEL`)을 함께 세워 두 행이 따로 강조되지 않는다(e2e186).
