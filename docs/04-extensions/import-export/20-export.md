...
메타데이터(mapMeta.ts)는 `{ format:'easymindmap-map', version, generator:
'EasyMindMap', exportedAt, editor:{layoutType, spacingX/Y}, map:원본 맵 전체 }`.
스타일·노트·링크·사진·태그·맵 설정(레벨별 폰트·레이아웃)까지 담긴다.
