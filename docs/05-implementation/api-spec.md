...
#### 협업 확장점 2개 (2026-08-15, **자리만 뚫려 있다**)

| # | 메서드 | 경로 | 설명 |
|---|---|---|---|
| 32 | GET | `/v1/collab/status` | **무인증.** 협업이 켜져 있는가 → `{enabled, reason, features[]}`. **로그인 없이 답하는 이유** — 화면이 메뉴를 그릴지 말지 정하는 데 쓰므로 로그인 전에도 알아야 한다. 유료 모듈이 없으면 `enabled:false` + **왜 꺼졌는지**(그 문장을 화면이 그대로 보여 준다) |
| 33 | POST | `/v1/collab/maps/:mapId/join` | 협업 세션 참가 → `{mapId,endpoint,token,expiresAt}`. 유료 모듈이 없으면 **404**(본문에 같은 이유) |

> **이 저장소에는 협업 구현이 없다.** 유료 기능이라 별도 private 저장소에
> 있고, 여기에는 계약·스텁·경로만 있다
> (`docs/04-extensions/open-core-boundary.md`). **공개판에서는 32번이
> `enabled:false`, 33번이 404 인 것이 정상 동작**이지 오류가 아니다.
>
> 33번이 조용히 성공하지 않는 이유 — 빈 세션을 돌려주면 화면은 협업이
> 켜진 것처럼 굴다가 아무 일도 일어나지 않는다. 그게 제일 나쁘다.
>
> 경로를 **공개 코어**에 두는 이유 — 유료 모듈이 경로까지 만들면, 모듈이
> 없을 때의 404 가 "그런 기능이 원래 없다"인지 "유료라서 없다"인지
> 구분되지 않는다.
