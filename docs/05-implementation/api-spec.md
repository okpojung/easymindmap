...
| 19j | GET | `/v1/account/logins` | **내 로그인 기록** — 경로에 id 를 받지 않는다(토큰 주인 것만). 응답은 22h 와 같다 (2026-08-13). `events[].ip`·`events[].device` 는 **우리 `login_events` 기록에서 로그인 줄에만** 붙는다 (±60초 · 일대일). 못 맺으면 null — 2026-08-14 이전 로그인이 그렇다 |
