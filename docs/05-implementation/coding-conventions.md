...
### 5-1-5. 작은 글리프는 **보이는 것보다 넓게 누를 수 있어야** 한다 (2026-08-09)

체크박스 글리프는 13px 남짓이다. 그만큼만 클릭 판정을 두면 조금만
빗나가도 **뒤에 있는 것(노드)** 이 눌려, 사용자에게는 "체크가 안 된다"로
보인다. 실제로 그렇게 접수된 버그였다.

규칙: **가로·세로 20px 미만인 조작 요소는 클릭 판정을 사방으로 넓힌다**
(대략 6px). 넓히되 **겉보기 배치는 그대로 두어야** 한다 — 넓힌 만큼 글자가
밀리면 안 된다.

| 자리 | 방법 |
|---|---|
| SVG | 글리프 뒤에 투명 `<rect>`(`fill: rgba(0,0,0,0.001)`)를 깔고 그 `<g>`가 클릭을 받는다 |
| HTML(인라인) | `position: relative` + 자식 `position: absolute; inset: -6px` |
| HTML(블록) | 래퍼에 `padding: 6px; margin: -6px` |
...
### 5-1-6. 같은 개념은 **어디서나 같은 모습** — 규칙을 한 곳에 적는다 (2026-08-09)
...
| 맵 노드 글자 (SVG) | 본문의 `- [ ]` 줄 |
| 아웃라인·칸반 (HTML) | 같은 줄 |
| 인스펙터 노트·태그 탭 | 체크리스트 블록 |
| 노트 뷰어 팝업 · 내보낸 뷰어 상세 패널 | 같은 블록 |

네 곳이 **따로 구현**되어 있다. 그래서 한 곳만 손보면 규칙이 조용히
갈라진다 — 실제로 갈라졌다(2026-08-09 사용자 보고):

* 뷰어에서 체크하면 **취소선**이 그어지는데 맵 노드는 안 그어졌다
* 인스펙터의 ✓ 는 9px 이라 초록 칠에 묻혀 "녹색 네모"로만 보였다
* 노트 팝업 제목에 원문 마커(`- [x] 할링`)가 그대로 새어 나왔다

**합의된 표시 규칙** (바꾸려면 네 곳을 함께 바꾼다):

1. **완료해도 취소선을 긋지 않는다.** 완료 표시는 ✓ 하나로 충분하고,
   글자가 지워진 것처럼 보이면 읽기 나쁘다.
2. **✓ 는 상자의 60% 이상**으로 그린다. 칠만 바뀌면 "켜졌다"로 안 읽힌다.
3. **노드 글자를 제목으로 쓸 때는 `flattenNodeText()`** 로 마커를 접는다
   (`- [x] 할링` → `☑ 할링`). 뷰어는 같은 규칙의 `flattenText()`.
4. **머리글에 늘 켜져 있는 ☑ 를 두지 않는다** — 바로 아래의 진짜 체크
   상자와 헷갈린다(블록 종류 아이콘은 `☰` 같은 중립 글리프로).
5. **종류 라벨을 제목과 같은 줄에 겹치지 않는다** — 그 종류의 인디케이터를
   눌러서 연 창이면 라벨은 중복이다.

**작업 규칙**: 같은 개념을 **새 자리에 그릴 때는 기존 자리를 먼저 보고
맞춘다.** 다르게 하고 싶으면 기존 자리도 같이 바꾼다 — 한쪽만 바꾸는 것은
"새 기능"이 아니라 **규칙을 깨는 일**이다.

> 왜 이런 일이 났나: 뷰어의 체크를 **누를 수 있게 만든 순간**(2026-08-09),
> 그동안 아무도 누르지 않아 드러나지 않던 표시 차이가 한꺼번에 보였다.
> **읽기 전용을 쓰기 가능으로 바꾸면, 그 개념이 그려지는 모든 자리가
> 함께 시험대에 오른다** — 그때는 자기가 만진 자리만 보지 말고 전부 훑는다.

### 5-1-7. 팝업(대화상자)은 **오른쪽 위 ×, 제목 고정, 본문만 스크롤** (2026-09-08)

> 사용자 결정 둘. ⑴ 계정 팝업마다 아래 [닫기]는 있는데 **오른쪽 위 × 가
> 없어** 닫는 자리를 찾아 내려가야 했다. ⑵ AI 설정처럼 긴 팝업은 스크롤하면
> **제목까지 밀려 올라가** 무슨 창인지 잃어버렸다.

규칙 — 계정 메뉴 계열을 비롯한 **모든 팝업**은 `components/ui/DialogFrame.tsx`
를 쓴다. 손으로 overlay·panel 을 다시 그리지 않는다.

| 항목 | 규칙 |
|---|---|
| 닫기 | 오른쪽 위 **×**(`{testId}-x`) 와 아래 [닫기](`DialogCloseButton`) 둘 다 — 같은 일을 한다. 바깥 클릭·Esc(§5-1-3)도 닫는다 |
| 제목 | 머리(`{testId}-head`)에 두고 **스크롤되지 않는다**. 부제(subtitle)도 머리 |
| 본문 | `{testId}-body` 만 `overflowY: auto`. 패널은 `maxHeight: 88vh` 의 세로 flex |
| 발 | [닫기]·[저장] 같은 버튼은 `footer` 로 — 본문과 함께 밀리지 않는다 |
| 진행 중 | 삭제 중처럼 닫으면 안 될 때는 `closeDisabled` — × 와 바깥 클릭이 함께 잠긴다 |

검증은 e2e247 — AI 설정 창 본문을 끝까지 내려도 제목·× 의 화면 위치가 그대로다.
새 팝업을 만들 때 이 틀을 쓰지 않으면 §5-1-6("같은 개념은 어디서나 같은 모습")
위반이다.

#### ★ 아직 `DialogFrame` 이 아닌 팝업은 **`DialogXButton`** 을 쓴다 (2026-09-18)

> 사용자 지적: *"팝업창 오른쪽에 모두 × 를 넣기로 규칙에 넣었는데 아직
> 반영이 안 된 팝업이 있다."* 실제로 전수 확인하니 `DialogFrame` 을 쓰는
> 것은 `UserMenu` **하나뿐**이었고, 나머지 여섯(퍼블리싱·맵 저장·폴더
> 고르기·다중 추가·코드 블록·표)과 [저장 안 됨] 경고에는 × 가 없었다.

`DialogFrame` 으로 옮기는 것이 정답이지만, 여섯 팝업은 저마다 본문 구성이
달라 한 번에 옮기면 되돌리기 어려운 회귀가 난다. 그래서 **× 버튼만 먼저
한 벌로** 뽑았다 — `DialogFrame` 이 쓰던 그 버튼을 `DialogXButton` 으로
내보내고, 여섯 팝업이 그것을 그대로 쓴다. 생김새·크기(28×28)·자리(오른쪽
위 10px)·`{testId}-x` 규칙이 전부 같아진다.

```tsx
<div style={{ position: 'relative', /* ← 이게 있어야 × 가 패널에 붙는다 */ … }}>
  <DialogXButton t={t} testId="publish-panel-x" onClose={onClose} />
  <div style={{ …, paddingRight: 34 /* ← 제목이 × 밑으로 안 들어가게 */ }}>제목</div>
```

| 지킬 것 | 왜 |
|---|---|
| 패널에 `position: 'relative'` | 없으면 × 가 화면(뷰포트) 기준으로 날아간다 |
| 제목 줄에 `paddingRight: 34` | 긴 제목이 × 밑으로 들어간다 |
| testId 는 `{패널 testId}-x` | 시험이 팝업마다 같은 규칙으로 찾는다 |

**떠 있는 카드(팝오버)도 같다** — 문서함의 상세 정보 카드처럼 고정
(pinned)되는 카드에는 × 를 둔다. 마우스를 떼면 사라지는 호버 카드에는
두지 않는다(닫을 일이 없다).

**남은 빚**: 여섯 팝업을 `DialogFrame` 으로 옮기는 일. 옮기면 제목 고정·
본문만 스크롤까지 함께 얻는다. 검증은 e2e273.

##### ★★ **× 만 옮기면 절반만 옮긴 것이다** (2026-09-21 — 그 빚이 터진 자리)

> 사용자 지적: *"왜 퍼블리싱 팝업창 오른쪽에 X (닫기) 아이콘이 없나."*
> **× 는 있었다 — 보이지 않았을 뿐이다.**

`DialogXButton` 만 붙이고 이 절의 위 표에 있는 **나머지 절반(패널
`maxHeight` + 본문만 스크롤)** 을 안 옮긴 것이 원인이었다. 퍼블리싱 창은
미리보기 그림까지 담아 **974px** 인데, 바깥이 세로 가운데 정렬이라 창이
화면보다 길면 **위아래로 똑같이 넘친다.** 넘친 위쪽에 제목과 × 가 있고,
바깥이 `position: fixed` 라 **스크롤해서 닿을 수도 없다.**

실측 (`data-testid="publish-panel"` 의 화면 좌표):
...
**그래서 × 를 넣을 때는 반드시 셋을 함께 넣는다.**

```tsx
<div style={{
  position: 'relative',
  maxHeight: 'calc(100vh - 32px)',
  boxSizing: 'border-box',        // ★ 없으면 패딩이 그 위에 더해져 넘친다
  display: 'flex', flexDirection: 'column',
}}>
  <DialogXButton … />            {/* 패널에 absolute — 굴러도 제자리 */}
  <div style={{ flexShrink: 0 }}>제목</div>
  <div style={{ overflowY: 'auto', minHeight: 0 }}>…본문…</div>
  <button style={{ flexShrink: 0 }}>닫기</button>
</div>
```

| 함정 | 증상 |
|---|---|
| `boxSizing` 을 빼면 | `maxHeight` 는 content-box 에 걸려 **패딩 40px + 테두리가 더 붙는다** — 화면 900 에 패널 910 (실측) |
| `minHeight: 0` 을 빼면 | flex 자식이 **줄지 않아** `maxHeight` 가 통째로 무시된다 |

**다른 팝업은 지금 걸리지 않는다** — 확인했다. 맵 저장·[저장 안 됨] 경고는
짧고, 다중 추가·코드 블록은 textarea 가 `rows={9}`·`rows={10}` 으로 묶여
있어 화면을 넘지 않는다. **퍼블리싱만 그림(500×262)이 들어가 자랐다.**
검증은 e2e287.

#### ★ 떠 있는 카드의 자리는 **셈을 따로 시험한다** (2026-09-18)

`window.innerHeight - 250` 처럼 **높이를 상수로 추측하지 않는다.** 문서함
상세 정보 카드가 그렇게 짜여 있어, 내용이 길어진 퍼블리싱 맵에서 아래가
잘렸다. 실제 높이를 재고(`useLayoutEffect` — 그려지기 전에 돈다)
`utils/popupPosition.ts` 의 `clampTop`·`clampLeft` 로 접는다.

눈으로는 "내 맵에서는 멀쩡하던데" 로 넘어가기 쉬운 자리라 **순수 함수로
빼서 단위 시험**한다(`popupPosition.test.ts`, `test:unit`).
