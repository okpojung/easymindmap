...
#### ★ 이 스크립트는 무중단이 아니다 — 중단 시간 (2026-09-13 확정)

dev 의 Coolify 는 새 컨테이너를 **옆에** 띄워 헬스를 통과시킨 뒤 라우팅을
옮기므로 끊기지 않는다. 운영의 `docker compose up -d` 는 컨테이너를 **그
자리에서 교체**하므로 옛 것이 내려가고 새 것이 헬스를 넘길 때까지 빈다.
NPM 이 컨테이너 포트를 직접 보므로 그 사이 **502** 다. 아래는 실측이
아니라 설계 + dev 실측치(종료 2.1초)로 계산한 **추정**이다 — 운영 첫 배포
때 재서 고친다.

| 단계 | 끊기나 | 추정 |
|---|---|---|
| ② 이미지 pull | 아니오 — 옛 컨테이너가 서비스 중 | 20~60초 |
| ③ 스키마 델타 | 아니오 — 표가 먼저, 옛 코드는 계속 돈다 | 수 초 |
| ④ API 교체 (SIGTERM → 정리 종료 → 기동 → DB 연결 → 헬스 200) | **예 — 502** | **5~15초** |
| ④ 프런트(nginx) 교체 | 예 | **1~2초** |
| ⑤ 헬스 실패 → 자동 롤백 (이전 이미지는 디스크에 있음) | 예 | 헬스 대기(30~60초) + 교체 5~15초 ≈ **40~75초** |
| 헬스는 통과했는데 기능이 틀림 → 사람이 이전 태그로 실행 | 예 (교체 5~15초) | 중단은 5~15초, **잘못된 버전이 도는 시간 = 알아채는 시간** |
| ③ 델타 실패 | **아니오** — 스크립트가 멈추고 옛 컨테이너가 그대로 | 0 |

**그 5~15초에 사용자가 겪는 것** (코드로 확인, 2026-09-13):

- 새로 여는 사람 → 502. 프런트가 502 본문을 JSON 으로 읽지 못해 화면에는
  **"요청 실패 (502)"** 가 뜬다(`apiClient.ts`). 배포 중인지 알 길이 없다 → §11.4-A ⓐ.
- 에디터를 열어 둔 사람 → 화면은 그대로. 편집은 **1초 안에 IndexedDB 초안**으로
  남는다(`useLocalDraft`). 그 순간의 자동저장은 실패하지만 **끝까지 재시도**한다
  (`useCloudAutosave` `RETRY_DELAYS` 1·3·10·30초, 마지막 값 무기한 —
  2026-08-06 R2). 두 번 넘게 실패하면 배지가 'error' 로 바뀌었다가 성공하면
  돌아온다. **잃는 편집은 없다** — 고칠 것은 문구다(§11.4-A ⓑ).
- 협업맵 → 소켓이 1001 로 끊기고 스스로 다시 붙는다. 끊긴 동안 REST 저장은
  보류하고(STALE 방지, 2026-09-08) 다시 붙으면 방 위에 얹는다.

**무중단이 필요해지면** — 복제본을 늘리는 롤링은 **안 된다**(협업 방이
인스턴스 메모리에 있다 — private `deploy.md` §6.1). 맞는 것은 **블루/그린**:
`api-blue`·`api-green` 두 서비스를 두고 노는 쪽에 새 태그를 띄워 헬스를
넘긴 뒤 프록시 업스트림을 바꾸고, 옛 쪽은 정지 유예 안에 편집을 저장하고
내려간다. 전환은 reload 한 번, 원복은 포트를 되돌리는 것뿐. 백로그 **B20 ⑦** —
유료화 직전(PITR 과 같은 시점). 그 전에는 **새벽에 배포한다.**

#### 11.4-A 배포 중 사용자 안내 — 502 대신 "점검 중" (B20 ⑧)

**ⓐ NPM 이 502 를 가로챈다** — 운영 VM 에 손댈 것 없이 NPM(192.168.0.74) 의
Proxy Host **Advanced** 탭만 고친다. 프런트 호스트와 api 호스트에 **다르게**
넣는다(브라우저에는 HTML, API 클라이언트에는 JSON).

```nginx
# 프런트 호스트 (예: app.easymindmap.org) — Advanced
proxy_intercept_errors on;
error_page 502 503 504 =503 /emm-maintenance.html;
location = /emm-maintenance.html {
    root /data/nginx/custom;      # NPM 컨테이너의 /data 볼륨 — 호스트의 NPM data 폴더
    internal;
    add_header Cache-Control "no-store";
    add_header Retry-After 15;
}
```

```nginx
# api 호스트 (예: api.easymindmap.org) — Advanced
proxy_intercept_errors on;
...
location @maintenance {
    ...
    add_header Cache-Control "no-store";
    add_header Retry-After 15;
    return 503 '{"code":"MAINTENANCE","message":"시스템 점검 중입니다 (버전 업그레이드 배포). 잠시 후 다시 접속해 주세요."}';
}
```

`emm-maintenance.html` 은 한 장짜리 정적 파일(제목 · "버전 업그레이드 배포
중입니다 · 약 1분 뒤 자동으로 다시 시도합니다" · `<meta http-equiv="refresh"
content="10">`)이다. 로고·CSS 는 파일 안에 넣는다 — 프런트 컨테이너가 내려간
동안이라 외부 자산을 못 가져온다. 이렇게 하면 프런트는 고치지 않아도
...
지운다. 재시도 로직은 이미 있으므로 **문구 한 줄과 해제 한 줄**이다.
배지 'error' 는 그대로 둔다(사실이다).

**ⓒ 사전 공지** — 관리자 콘솔 설정에 `notice.text` 한 칸, `GET /v1/notice`
(인증 없음 · 캐시 60초), 프런트가 60초마다 폴링해 상단 띠로 표시. 배포
스크립트가 시작할 때 "23:00 배포 · 약 1분 중단" 을 넣고 끝나면 비운다.
ⓐⓑ 뒤에, 별도 PR.
