...
> 들어 있어 **그대로 붙여넣으면 끝**이어야 한다.
>
> **어디서 실행하나** — docker 가 도는 호스트의 SSH 터미널이다. dev 는
> **`ubuntu@em-dev`** (Coolify 가 띄운 앱·DB 컨테이너가 여기 있다). 보고서의
> 델타 블록 바로 위에 이 서버 이름을 적는다. 정상 출력은
> `✅ DB=<컨테이너> 계정=postgres DB이름=postgres` → `ALTER TABLE` →
> `COMMENT` → 검증 SELECT 한 줄이다 (2026-09-08 `users.avatar` 적용 실측). 저장소가 있는 서버에서는
...
### 1.5-0-★. 긴 블록이 **조용히 잘린다** (2026-09-04 실측)

이 문서의 스크립트는 대부분 50줄이 넘는다. SSH 터미널에 그대로 붙여넣으면
**끝까지 들어가지 않는 경우가 있다.** `bash <<'SCRIPT'` 는 받은 데까지만
읽고 **정상 종료**하므로, 오류 한 줄 없이 "앞부분만 실행"된다.

실제로 그렇게 겪었다 — `✅ easymindmap → 앱=… DB=…` 한 줄만 찍히고
프롬프트로 돌아왔다. 델타 SQL 은 **한 문장도 실행되지 않았는데** 화면만
보면 성공처럼 보인다.
...
| 확인 | 잘렸다는 뜻 |
|---|---|
| 스크립트 끝의 **검증 출력**(`표 : OK` 등)이 안 보인다 | 잘렸다 — 다시 실행한다 |
| `/v1/health` 의 `missingTables` 가 그대로다 | 안 들어갔다 |

**긴 블록을 피하는 법** — 델타는 저장소에 파일로 있다
(`apps/api/database/deltas/`). 공개 저장소이므로 서버에서 바로 받는다.

```bash
R=https://raw.githubusercontent.com/okpojung/easymindmap/main/apps/api/database/deltas
curl -fsSL "$R/<파일이름>.sql" | docker exec -i "$DB" psql -U "$PGUSER" -d "$PGDB" -v ON_ERROR_STOP=1
```

붙여넣는 것은 **두 줄**이고, 내용은 파일에서 온다 — 잘릴 자리가 없다.
...
### 1.5-0-I. 무료 게시 + MCP 토큰 — `published_maps` · `api_tokens` 델타 (2026-09-04)

공개 #382(무료 게시) · #381(MCP 커넥터). 두 표를 한 번에 넣는다.

> **적용 전에도 앱은 죽지 않는다.** 표가 없으면 **그 기능만** 꺼지고 화면이
> 이유를 미리 말한다 — [공개] 대화상자는 "아직 준비되지 않았습니다", MCP
> 토큰 화면은 `ready:false`. 맵 열기·저장은 그대로다.
>
> **적용 뒤 재기동이 필요 없다.** 서버가 60초 안에 표를 다시 확인한다
> (`apps/api/src/common/table-ready.ts`).

**긴 블록을 붙여넣지 않는다** — 델타는 저장소의 파일이고, 서버는 그것을
받아서 실행한다. 이유는 §1.5-0-★ 다.
...
[ -z "$API" ] && { echo "❌ api 컨테이너를 찾지 못했습니다."; docker ps --format '{{.Names}}'; exit 1; }
docker inspect "$DB" >/dev/null 2>&1 \
  || { echo "❌ DB 호스트($DB)가 이 서버의 컨테이너가 아닙니다."; exit 1; }
echo "✅ 앱=$API  DB=$DB  계정=$PGUSER  데이터베이스=$PGDB"

R=https://raw.githubusercontent.com/okpojung/easymindmap/main/apps/api/database/deltas
curl -fsSL "$R/2026-09-04-published-maps-and-api-tokens.sql" \
  | docker exec -i "$DB" psql -U "$PGUSER" -d "$PGDB" -v ON_ERROR_STOP=1
SCRIPT
```

검증 네 줄(`published_maps 표/정책 · api_tokens 표/색인`)이 전부 `OK` 면
성공이다. 그다음 `/v1/health` 의 `missingTables` 에서 `published_maps` 와
`api_tokens` 가 사라진다.

**되돌리려면** — 게시 기록까지 지운다(공개 중인 링크가 전부 죽는다).
링크만 죽이려면 화면의 [공개 중단] 을 쓴다.

```bash
docker exec -i "$DB" psql -U "$PGUSER" -d "$PGDB" \
  -c "DROP TABLE IF EXISTS public.published_maps;"
```
...
그 사용자가 **`https://pro-dev.mindmap.ai.kr` → 우상단 아바타**를 누르면
...
(탈퇴 한 번으로 협업맵 참여자의 작업까지 사라지던 것을 DB 가 막는다),
**B** `plan_quotas.version_days·max_pinned` + `map_document_versions.pinned·label·pinned_by·pinned_at`
+ 부분 인덱스, **C** `map_ownership_transfers` 표. B·C 는 표·컬럼만이고
읽는 코드가 아직 없다.

**순서가 중요하다 — 이번만은 코드가 먼저다.** A 의 RESTRICT 는 **새 탈퇴
코드**(단독맵을 먼저 지우고 users 를 지운다)와 짝이다. 옛 코드가 도는
서버에 RESTRICT 만 먼저 넣으면 **모든 탈퇴가 500** 이 된다. 반대로 새
코드는 옛 스키마(CASCADE)에서도 그대로 돈다(e2e197 구 스키마 21/21).

```
① private 저장소(easymindmap-pro)의 코어 SHA 를 8caa2c3 이상으로 올리고 api-pro 재배포
     ↳ 개발 서버는 유료판만 돈다(§0 머리말) — 공개 저장소 병합만으로는 코드가 오지 않는다
② 새 코드가 떴는지 확인: /v1/health 가 schema:"outdated" 와 함께
     missingTables:["map_ownership_transfers"] · outdatedConstraints:["maps_owner_id_fkey: CASCADE (기대 RESTRICT)"]
     를 보이면 새 코드다 (옛 코드는 이 키들을 모른다 — 그냥 ok 로 나온다)
③ 아래 델타 적용 → /v1/health 가 schema:"ok"
```
...
동작은 아바타 메뉴 → 회원탈퇴 → `회원탈퇴` 입력 → 탈퇴로 본다 — 협업맵
개설자면 "함께 쓰는 맵 N개의 개설자입니다 …" 안내와 맵 목록이 뜨고 계정이
남는다. 단독맵만 있는 계정은 예전처럼 탈퇴된다.

**되돌리기** (새 코드가 아직 안 왔는데 이미 적용해 버렸을 때): RESTRICT 만
CASCADE 로 되돌리면 옛 코드의 탈퇴가 다시 돈다. B·C 는 그대로 둬도 된다.
...
> ✅ **2026-09-07 확인 완료** — ⑥~⑩ 이 모두 맞춰졌다. 302 의 목적지가
> `pro-dev` 로 바뀌었고 번들에도 실렸다. 배포본을 브라우저로 돌려
...
로그(`/var/log/emm-backup.log`)에는 **cron 실행만** 쌓인다 — 손으로 돌린
것은 터미널에만 찍힌다. 실행마다 `── <시각> 시작 DB=…` 줄로 시작하고
실패 줄(`❌`)에도 시각이 붙으므로, `tail` 로 본 실패가 **어느 실행**의
것인지는 그 시각으로 가른다 (2026-09-18: 옛 스크립트로 돈 03:10 실패가
새 스크립트를 받은 뒤에도 마지막 줄로 남아 있어 헷갈렸다).
...
**결정 (2026-09-21 사용자):** 첨부 파일이 있는 NAS(hng1)가 **아니라
다른 NAS(hng2)** 의 공유 폴더 `hng2` 아래 하위 폴더
**`easymindmap_db_backup`** 으로 보낸다. 별도 공유 폴더를 만들지 않는다.
다른 장비라서 오프사이트로서의 의미가 있다 — 첨부 NAS 가 죽어도 DB 백업은
남는다. 지금은 dev DB 를 보내지만, **운영이 서면 dev 는 보내지 않고 운영
DB 만** 이곳에 둔다. 그래서 그 아래를 환경별로 나눈다 — `dev/` · `prod/`.
운영이 서면 dev 의 cron 에서 `OFFSITE_DIR` 을 빼고 `dev/` 는 지운다.

| | NAS | 내보내기 | 서버 마운트 | 옵션 |
|---|---|---|---|---|
| 첨부 파일 (§1.5-A) | hng1 `192.168.0.210` | `/volume2/hng1/easymindmap/dev` | `/mnt/nas/emm-files` | `hard` |
| **DB 백업** | **hng2 `192.168.0.220`** | **`/volume2/hng2/easymindmap_db_backup`** | **`/mnt/nas/emm-db-backup`** | **`soft`** |

(주소는 sanitize 규약값이다 — 실제 값은 서버의 `/etc/fstab` 과 서버 관리
문서에만 둔다.)

```
hng2/                          ← NAS hng2 의 기존 공유 폴더
└── easymindmap_db_backup/
    ├── dev/   .emm-offsite  all-YYYYMMDD-HHMM.sql.gz …   ← 지금
    └── prod/  .emm-offsite  …                             ← 운영이 서면
```

##### ① NAS hng2 쪽 — `hng2` 공유 폴더의 NFS 권한 (DSM)

**제어판 ▸ 공유 폴더 ▸ `hng2` ▸ 편집 ▸ NFS 권한** 에 dev 서버 IP 줄이
있어야 한다(없으면 생성) — 권한 **읽기/쓰기**, Squash **매핑 없음**(cron 이
root 로 쓴다), **"마운트된 하위 폴더 접근 허용"** 체크(하위 폴더만 따로
...
```
192.168.0.220:/volume2/hng2/easymindmap_db_backup  /mnt/nas/emm-db-backup  nfs  vers=4.1,_netdev,noatime,soft,timeo=150,retrans=3  0  0
```
