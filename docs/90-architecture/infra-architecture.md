...
> **최종 업데이트**: 2026-08-07
> **버전**: v1.5 — **브라우저가 여는 호스트(`dev`·`api-dev`)에는 Access
> List 를 걸지 않는다** + §7.3 허용 IP 목록을 **실측으로 정정**(SSL-VPN
> 풀은 실제 규칙에 없었다). 외부 공개 준비는 백로그 B14.
> v1.4 — `api-dev` 호스트는 Access List 를 걸지 않는다
...
> **운영 도메인의 단일 기준은 `docs/00-project-overview/vision.md`**
> "서비스 도메인 계획" 이다 (2026-08-09 — 주도메인 `easymindmap.org`,
> 앱 `web.`·홈페이지 `www.`, 기존 `mindmap.ai.kr` 은 301 포워드).
> 이 문서의 표는 **내부 배치**(어느 VM 의 어느 포트로 넘기는가)를
> 다루므로 도메인은 `example.com` 자리표시자를 그대로 쓴다.
...
Allowed IPs:
  [사내 LAN 대역 /24]         # 아래 ⚠️ 참조 — 실제로는 이 한 줄뿐이다

Pass Auth: ✅ (Allow)
```

> ⚠️ **[2026-08-07 실측 정정]** 이 절은 오래도록 허용 목록에
> `개발 PC 단일 IP` + `[SSL-VPN 클라이언트 풀]` 이 들어 있다고 적고
> 있었지만, **실제 nginx 규칙에는 사내 LAN 대역 `/24` 한 줄뿐이었다**
> (`grep -A8 auth_basic /data/nginx/proxy_host/<id>.conf` 로 확인).
> **SSL-VPN 풀은 규칙에 없다.** 이 문서를 믿고 "VPN 이면 통과한다"고
> 판단하면 틀린다 — 실제로 그렇게 오진했다.
>
> (FortiGate 가 VPN 트래픽을 사내 대역으로 NAT 한다면 결과적으로 통과할
> 수는 있다. 그건 Access List 가 아니라 **방화벽 구성**에 달린 별개
> 조건이므로, 필요하면 실측으로 확인한다.)

> ⚠️ **어디에 걸 것인가 — 브라우저가 여는 호스트에는 걸지 않는다.**
> `Satisfy: Any` 는 *IP 가 안 맞으면 Basic 인증*을 요구한다. 두 가지가
> 모두 깨진다.
>
> | 대상 | 왜 깨지나 |
> | --- | --- |
> | **API 호스트**(XHR) | 브라우저는 **preflight(OPTIONS)에 인증 정보를 싣지 않는다** → 401 → CORS error → 통째로 막힘 (§7.7) |
> | **프런트 호스트**(문서) | 처음 오는 사용자에게 **브라우저 Basic 인증 팝업**이 뜬다 → 앱에 들어오지도 못함 (§7.6) |
>
> 프런트는 **한 번 인증한 브라우저가 자격을 캐시**하므로 개발자 본인
...
Cache Assets: ❌ 끄기
SSL:      Let's Encrypt + Force SSL ✅
Access:   Publicly Accessible            ← ★ 외부 테스트를 열려면 (아래)
```

> ### ★ 외부에 열 것인지 먼저 정한다 (2026-08-07)
>
> | 목적 | Access List |
> | --- | --- |
> | **외부 사람이 테스트해야 한다** | **`Publicly Accessible`** — `api-dev` 와 **함께** 풀어야 한다 |
> | 사내 전용으로 유지한다 | `IPSec-VPN-Only` 유지 (그러면 외부 접속은 VPN 으로만) |
>
> **프런트만 잠가 두면 새 사용자는 앱 화면조차 못 본다** — 브라우저
> Basic 인증 팝업이 먼저 뜬다. 개발자 본인 브라우저는 자격을 캐시하고
> 있어 멀쩡해 보이므로 **자격 캐시가 없는 새 브라우저로 확인할 것.**
>
> 열어도 무방비가 아니다 — 앱 자체가 **로그인 게이트**로 막고, 그 뒤는
> JWT 가드·소유자 검증·RLS 가 지킨다. 다만 **공개 범위를 넓히기 전에**
> 레이트 리밋·가입 통제가 필요하다 (백로그 B14).
