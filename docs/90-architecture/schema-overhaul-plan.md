...
| # | 변경 | 성격 | 지금 안 하면 |
|---|---|---|---|
| **A** | `maps.owner_id` CASCADE 제거 | **파괴적** | 협업 켜는 순간 데이터 손실 버그가 실재화된다 |
| **B** | 버전 보관 컬럼 추가 | 가산 | 나중에 백필이 필요해진다 |
| **C** | 소유권 이전 테이블 | 가산 | A 의 해법이 없다 |
| **D** | 첨부 저장 경로 분리 | 인프라 | 첨부가 차면 API 서버가 죽는다 |
...
```
A  개설자 탈퇴
   ☐ 협업맵 개설자로 탈퇴 시도 → 목록과 함께 막힌다 (500 이 아니라 안내)
   ☐ 단독맵만 가진 사용자 → 정상 탈퇴된다
   ☐ 협업맵에 참여자로만 있는 사용자 → 정상 탈퇴되고 map_members 에서 빠진다
   ☐ DB 에서 직접 DELETE FROM users 시도 → RESTRICT 로 막힌다
...
C  소유권 이전
   ☐ 같은 맵에 두 번째 pending 제안 → 유니크 인덱스가 막는다
...
전체
   ☐ 기존 e2e 회귀 없음
   ☐ apply-schema.mjs 를 빈 DB 에 돌려 한 번에 통과한다
```
...
- **A 를 `RESTRICT` 로 할지 `SET NULL` 로 할지.** `SET NULL` 이면 탈퇴는
  되지만 주인 없는 맵이 남는다. 그 맵의 요금제 기준이 사라지므로
  (13a-version-retention.md §4.5 는 개설자 요금제를 따른다)
  `RESTRICT` 를 권하지만, 탈퇴를 막는 것이 사용자 경험상 강하다는 반론이 있다.
- **이전 제안 만료 14일**이 적절한가.
