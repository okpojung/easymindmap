...
노드를 **싱글 클릭**하면 해당 노드의 **상·하·좌·우 4방향에 `+` 아이콘(인디케이터)이 표시**되고,  
...
- 선택 노드와 기존 부모 사이에 새 노드를 **중간 삽입**한다.
...
인디케이터의 **물리적 표시 방향**은 고정(상/하/좌/우)이지만,  
**의미(동작)** 는 레이아웃 방향과 무관하게 아래처럼 **항상 고정**한다.
