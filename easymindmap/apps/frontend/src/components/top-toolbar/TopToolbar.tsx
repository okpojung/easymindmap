// TopToolbar — global controls: logo+breadcrumb, undo/redo, layout picker,
// save state, collab avatars, AI/share/export/user menu.
// Search and per-node action icons are intentionally NOT placed here; they live
// in the left sidebar (search) and the canvas floating toolbar (node actions).
...
export function TopToolbar({
  t, collabs, mapTitle, layoutType, onLayoutChange, saveState = 'saved',
}: Props) {
  const saveStateInfo = ({
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 280 }}>
        <div style={{
          ...
          background: t.primarySoft, border: `1px solid ${t.primaryBorder}40`,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.15, minWidth: 0 }}>
          ...
          <div style={{
            ...
            {mapTitle}
            <span style={{ opacity: 0.5, display: 'flex' }}><I.ChevronDown size={14} /></span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 2 }}>
        <IconBtn t={t} title="되돌리기 (Ctrl+Z)"><I.Undo size={17} /></IconBtn>
        <IconBtn t={t} title="다시 실행 (Ctrl+Y)" disabled><I.Redo size={17} /></IconBtn>
      </div>

      <div style={{ width: 1, height: 28, background: t.divider }} />

      <LayoutPicker t={t} layoutType={layoutType} onChange={onLayoutChange} labels={LAYOUT_LABELS} />
      ...
      <div style={{
        display: 'flex', alignItems: 'center', gap: 6,
        fontSize: 12, color: saveStateInfo.color, fontWeight: 500,
      ...
      <IconBtn t={t} title="내보내기"><I.Download size={16} /></IconBtn>
