...
// ★ 아래 셋은 회귀 테스트다 — 위 '코드 펜스 안쪽' 이 못 잡던 자리다.
// 펜스가 **첫 줄**이면 루프가 i=1 부터 돌아 그 펜스를 놓쳤고, 코드블록
// 안쪽을 바깥으로 착각해 **원문에서 밑줄 한 줄이 사라졌다.**
check(
  '② ★ 문서 첫 줄이 펜스 — 안쪽을 건드리지 않는다',
  conv(F, '', 'title', '=====', F),
  [F, '', 'title', '=====', F],
);
check(
